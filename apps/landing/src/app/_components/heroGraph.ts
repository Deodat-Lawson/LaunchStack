...
function createGraph(): Graph {
    ...
    const edges: Edge[] = EDGE_SEED.map(([a, b]) => ({ a: nodeById[a]!, b: nodeById[b]! })).filter(
        (e): e is Edge => !!e.a && !!e.b
    );
...
function render(
    ...
    for (const c of Object.values(graph.clusterMap)) {
        const fake: { x: number; y: number; z: number; sx?: number; sy?: number; scale?: number } =
...
export function createHeroGraph(
