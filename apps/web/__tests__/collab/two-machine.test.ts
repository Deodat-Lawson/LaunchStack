...
describe("meeting across separate machines", () => {
  ...
  afterAll(async () => {
    ...
    const dispatcher = (globalThis as unknown as Record<symbol, { close?: () => Promise<void> }>)[
  ...
  it("runs one meeting with each agent hosted by a different process", async () => {
    ...
    const nodes = (await console_.health()).nodes as Array<{ nodeId: string; connected: boolean }>;
    expect(nodes.map((n) => n.nodeId).sort()).toEqual(["machine-b", "machine-c", "operator-console"]);

    const finished = await console_.control(meetingId, { action: "run" }, { timeoutMs: 90_000 });
  ...
  it("lets a human on a third machine take over mid-meeting", async () => {
    ...
    const meetingLine = await hub.waitForLine(/\[collab-hub\] meeting \{.*\}/, 60_000);
    const { meetingId, channelId } = JSON.parse(meetingLine.slice(meetingLine.indexOf("{"))) as {
      meetingId: string;
      channelId: string;
    };
    await hub.waitForLine(/\[collab-hub\] listening/, 30_000);
    ...
    expect(messages.some((m) => m.kind === "system" && m.text.includes("took the floor"))).toBe(true);
    expect(messages.filter((m) => m.kind === "chat" && m.author.kind === "agent")).toHaveLength(2);
  });
});
