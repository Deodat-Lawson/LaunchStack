...
function computeLayout(nodes: GraphNode[]): {
    ...
    for (const [name, group] of byLabel.entries()) {
        ...
        group.forEach((n, idx) => {
            const theta = (idx / Math.max(group.length, 1)) * Math.PI * 2 + name.length * 0.3;
            laid.push({
                ...n,
    ...
    const byId = Object.fromEntries(laid.map(n => [n.id, n])) as Record<number, LaidNode>;
    const labelSummary = Object.fromEntries(
        labels.map(l => [l, { center: centers[l]!, count: byLabel.get(l)!.length }])
    );
...
export function GraphView({ documentId }: GraphViewProps) {
    ...
    const filteredNodeIds = useMemo(() => {
        ...
        return new Set(layout.laid.filter(n => n.name.toLowerCase().includes(q)).map(n => n.id));
    }, [layout, query]);
    ...
    return (
        ...
        >
            ...
            {/* Status banners */}
            {loading && <CenterNote>Loading knowledge graph…</CenterNote>}
                ...
                >
                    ...
                    >
                        ...
                        {layout.laid.map(n => {
                            ...
                            const r =
                                6 +
                                Math.min(Math.sqrt(n.mentionCount) * 2, 12) +
                                (isSelected ? 4 : 0);
                            return (
                                <g
                                    key={n.id}
