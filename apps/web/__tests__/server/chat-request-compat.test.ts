...
describe("deprecated chat request selectors", () => {
    ...
    it("rejects a provider selector outright — providers no longer exist", () => {
        expect(validateDeprecatedChatSelection({ provider: "openrouter" }, resolved)).toEqual({
            ok: false,
            status: 400,
            message:
    ...
    it("rejects a model override rather than silently ignoring it", () => {
        expect(validateDeprecatedChatSelection({ model: "request/override" }, resolved)).toEqual({
            ok: false,
            status: 400,
            message:
