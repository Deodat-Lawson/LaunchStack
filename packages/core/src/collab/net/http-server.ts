    ...
    void (async () => {
      try {
        ...
        if (body === null) {
          ...
          res.end(JSON.stringify({ ok: false, code: "bad_request", error: "Body too large" }));
          return;
        }

        const rawPath = req.url ?? "/";
        const canonical = mountPath && rawPath.startsWith(mountPath) ? rawPath.slice(mountPath.length) : rawPath;

        if (!canonical.startsWith(COLLAB_API_PREFIX)) {
          res.writeHead(404, { "content-type": "application/json" });
          res.end(JSON.stringify({ ok: false, code: "not_found", error: `Unknown path ${rawPath}` }));
          return;
        }
        ...
        const hubRequest: HubHttpRequest = {
          ...
          path: canonical,
          headers: lowercaseHeaders(req.headers as Record<string, string | string[] | undefined>),
          body,
        };
