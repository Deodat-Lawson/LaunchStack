...
import type { PageContent, NormalizedDocument, ExtractedTable } from "./ocr-processing/types";
...
export interface SourceAdapter {
    ...
    process(input: string | Buffer, options?: SourceAdapterOptions): Promise<StandardizedDocument>;
}
...
export function toNormalizedDocument(doc: StandardizedDocument): NormalizedDocument {
    ...
    return {
        ...
        metadata: {
            ...
            provider: (providerMap[doc.metadata.provider] ?? "NATIVE_PDF") as
                | "AZURE"
                | "LANDING_AI"
                | "NATIVE_PDF"
                | "DATALAB",
