...
jest.mock("~/server/collab/personas", () => ({
  ...
  createPersona: (_companyId: bigint, input: { key: string } & Partial<StoredPersona>) => {
    if (mockCtx.personas.some((p) => p.id === input.key)) {
      return Promise.reject(
        new Error('duplicate key value violates unique constraint "collab_persona_company_key_idx"'),
      );
    }
  ...
  updatePersona: (_companyId: bigint, dbId: string, patch: { key?: string } & Partial<StoredPersona>) => {
...
describe("collab agents routes", () => {
  ...
  it("rejects unauthenticated callers", async () => {
    ...
    expect(
      (await PATCH(jsonRequest({ role: "x" }, "PATCH"), { params: Promise.resolve({ personaId: "p" }) })).status,
    ).toBe(401);
    expect(
      (await DELETE(jsonRequest({}, "DELETE"), { params: Promise.resolve({ personaId: "p" }) })).status,
    ).toBe(401);
  });
  ...
  it("reports where a worker should point and what Slack still needs", async () => {
    ...
    expect(body.network).toEqual({ enabled: true, hubId: "app-hub", hubPath: "/api/collab/hub" });
  ...
  it("reports the network as disabled when no hub secret is configured", async () => {
    ...
    const body = (await (await GET()).json()) as { network: { enabled: boolean; hubId: string | null } };
    expect(body.network).toMatchObject({ enabled: false, hubId: null });
  });

  it("lists worker nodes with their connection state", async () => {
    mockCtx.nodes = [
      { nodeId: "gpu-box-1", label: "GPU box", personaIds: ["eng"], lastSeenAt: 1, queuedEvents: 0, connected: true },
  ...
  it("still renders the roster when node bookkeeping fails", async () => {
    ...
    mockCtx.nodes = [
      { nodeId: "gpu-box-1", personaIds: [], lastSeenAt: 1, queuedEvents: 0, connected: true },
    ];
  ...
  describe("POST /api/collab/agents", () => {
    ...
    it("reports a duplicate handle as a conflict, not a server error", async () => {
      ...
      expect(((await response.json()) as { error: string }).error).toMatch(/"finance" already exists/);
    });
    ...
    it("rejects a model route the deployment does not define", async () => {
      ...
      expect((await POST(jsonRequest({ ...VALID, key: "ok1", route: "reasoning" }))).status).toBe(201);
    });
  ...
  describe("PATCH /api/collab/agents/[personaId]", () => {
    ...
    it("clears a node assignment with an explicit null", async () => {
      ...
      expect(((await response.json()) as { persona: StoredPersona }).persona.nodeId).toBeNull();
    });
  ...
  describe("DELETE /api/collab/agents/[personaId]", () => {
    it("archives rather than deleting, so past transcripts still resolve", async () => {
      ...
      expect(response.status).toBe(200);
      expect((await response.json()) as { archived: boolean }).toMatchObject({ archived: true });
