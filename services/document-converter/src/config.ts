...
export type ConfigurableDefaultProvider = (typeof CONFIGURABLE_DEFAULT_PROVIDERS)[number];
...
const envSchema = z.object({
    ...
    DOCLING_SERVE_TIMEOUT_MS: positiveInt("DOCLING_SERVE_TIMEOUT_MS", DEFAULT_DOCLING_TIMEOUT_MS),
    ...
    OCR_DEFAULT_PROVIDER: optionalString
        .superRefine((value, ctx) => {
            ...
            if (!(CONFIGURABLE_DEFAULT_PROVIDERS as readonly string[]).includes(upper)) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
        ...
        .transform(value =>
            value === undefined ? undefined : (value.toUpperCase() as ConfigurableDefaultProvider)
        ),
