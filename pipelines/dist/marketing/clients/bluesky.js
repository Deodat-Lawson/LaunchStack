class BlueskyClient {
    ...
    async searchTrendingPosts(query, maxResults) {
        try {
            ...
            const searchData = await this.makeAuthenticatedRequest("app.bsky.feed.searchPosts", {
                q: query,
    ...
    async getTrendingFeed(maxResults) {
        try {
            // Get trending posts from the "What's Hot" algorithm feed
            const feedData = await this.makeAuthenticatedRequest("app.bsky.feed.getFeed", {
