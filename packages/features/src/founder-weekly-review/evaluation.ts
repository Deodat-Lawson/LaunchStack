...
function futureReleaseMentioned(text: string, periodEnd: string): boolean {
    ...
    for (const [month, number] of Object.entries(monthNumbers)) {
        if (new RegExp(`\\b${month}\\b`, "i").exec(lower) === null) continue;
        const candidate = new Date(Date.UTC(year, number - 1, 1));
        if (candidate > end) return true;
    }
    const yearMention = /\b(20\d{2})\b/.exec(lower);
...
export function evaluateFounderWeeklyReview(
    ...
    try {
        FounderWeeklyReviewEvidenceSnapshotSchema.parse(evidenceSnapshot);
        FounderWeeklyReviewV2PayloadSchema.parse(report);
    } catch {
        canonicalSchemaValid = false;
        failures.push({
            category: "malformed_payload",
            explanation: "Snapshot or review does not satisfy its canonical schema.",
        });
    }
    if (!canonicalSchemaValid)
        return {
            ...
            deterministic: {
                ...
                citationValidity: 0,
                citationCoverage: 0,
                sourceSemanticViolationRate: 1,
                unsupportedClaimRate: 0,
                unsupportedShippedClaimRate: 0,
                duplicateClaimRate: 0,
                evidenceCoverage: 0,
                emptySectionCorrectness: 0,
            },
            failures,
        };
    let citations = 0,
    ...
    for (const [sectionName, section] of Object.entries(report.sections)) {
        ...
        for (const item of sectionItems) {
            ...
            if (normalizedClaim) {
                claimCount++;
                const sectionClaim = `${sectionName}:${normalizedClaim}`;
                if (claims.has(sectionClaim)) {
                    duplicateClaims++;
                    failures.push({
                        category: "duplicate_claim",
                        section: sectionName,
                        claim,
                        explanation: "Repeated claim within the same section.",
                    });
                }
                claims.add(sectionClaim);
            }
            const ids = Array.isArray(item.sourceIds)
                ? item.sourceIds.filter((id): id is string => typeof id === "string")
                : [];
            if (
                ["observed_fact", "contradictory_evidence", "recommendation"].includes(
                    String(item.kind)
                )
            ) {
                if (ids.length) citedClaims++;
            }
            if (claim && ids.length) {
                ...
                const combined = cited
                    ...
                    .join(" ");
                supportChecks++;
                if (overlap(claim, combined))
                    cited.forEach(source => covered.add(source!.sourceId));
                else if (item.kind !== "recommendation" && item.kind !== "contradictory_evidence") {
                    unsupported++;
                    failures.push({
                        ...
                        claim,
                        explanation:
                            "Soft lexical diagnostic: claim has limited overlap with cited evidence.",
                    });
                }
            }
            for (const id of ids) {
                ...
                if (!source) {
                    failures.push({
                        ...
                        section: sectionName,
                        claim,
                        explanation: `Unknown sourceId: ${id}`,
                    });
                    continue;
                }
                validCitations++;
                ...
                semanticChecks++;
                const allowed =
                    sectionName === "whatCustomersSaid"
                        ? sourceType === "customer_feedback"
                        : sectionName === "whatChanged" || sectionName === "whatShipped"
                          ? sourceType === "document_change" ||
                            (sourceType === "workspace_document" &&
                                ids.some(
                                    other => evidence.get(other)?.sourceType === "document_change"
                                ))
                          : true;
                if (!allowed) {
                    semanticViolations++;
                    failures.push({
                        ...
                        section: sectionName,
                        claim,
                        explanation: `${sourceType} cannot independently support ${sectionName}.`,
                    });
                }
            }
            if (sectionName === "whatShipped" && ids.length > 0) {
                shippedChecks++;
                const citedSources = ids
                    .map(id => evidence.get(id))
                    .filter(
                        (source): source is FounderWeeklyReviewEvidenceSnapshot["items"][number] =>
                            source !== undefined
                    );
                ...
                if (!/launch|ship|release|deploy|complete|implement/i.test(citedText)) {
                    ...
                    failures.push({
                        ...
                        section: sectionName,
                        claim,
                        explanation: "Cited evidence has no explicit shipping signal.",
                    });
                }
                if (
                    citedSources.some(source => source.sourceType === "document_change") &&
        ...
        if (
            ...
            typeof section === "object" &&
            "state" in section &&
            section.state === "no_evidence" &&
            "items" in section
        )
            failures.push({
                category: "invalid_empty_section",
                section: sectionName,
                explanation: "No-evidence sections must not include items.",
            });
    }
    const hard = failures.some(failure =>
        [
            "malformed_payload",
            "invalid_citation",
            "invalid_source_type",
    ...
    const qualityEvidence = evidenceSnapshot.items.filter(
        item => item.sourceType === "document_change" || item.sourceType === "customer_feedback"
    );
    const authoritativeFailures = failures.filter(
        failure => failure.category !== "unsupported_claim"
    );
    return {
        passed: authoritativeFailures.length === 0,
        hasHardFailure: hard,
        deterministic: {
            canonicalSchemaValid,
            citationValidity: citations ? validCitations / citations : 1,
            citationCoverage: claimCount ? citedClaims / claimCount : 1,
            sourceSemanticViolationRate: semanticChecks ? semanticViolations / semanticChecks : 0,
            unsupportedClaimRate: supportChecks ? unsupported / supportChecks : 0,
            unsupportedShippedClaimRate: shippedChecks ? unsupportedShipped / shippedChecks : 0,
            duplicateClaimRate: claimCount ? duplicateClaims / claimCount : 0,
            evidenceCoverage: qualityEvidence.length
                ? qualityEvidence.filter(item => covered.has(item.sourceId)).length /
                  qualityEvidence.length
                : 1,
            emptySectionCorrectness: failures.some(
                failure => failure.category === "invalid_empty_section"
            )
                ? 0
                : 1,
        },
