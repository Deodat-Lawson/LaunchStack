...
describe("reconcile", () => {
    it("creates a record for a new fact key", () => {
        const incoming = [assertion("hq.city", "Berlin", anchor(1, 2), "2026-01-01T00:00:00Z")];
    ...
    it("updates an existing fact and pushes the old value onto supersededValues", () => {
        const existing = [record("hq.city", "Berlin", anchor(1, 2), "2026-01-01T00:00:00Z")];
    ...
    it("keeps the newest value when the incoming assertion is older, storing it as history", () => {
        const existing = [record("hq.city", "Munich", anchor(1, 2), "2026-03-01T00:00:00Z")];
        const result = reconcile(
            existing,
    ...
    it("applies a batch for one key oldest to newest", () => {
        ...
        expect(result.facts[0]!.supersededValues.map(s => s.value)).toEqual(["Berlin", "Munich"]);
    });
    ...
    it("reports cross-source conflicts over the merged current set", () => {
        ...
        expect(result.conflicts[0]!.values.map(v => v.value).sort()).toEqual(["berlin", "munich"]);
    });
    ...
    it("does not report a conflict when the ledger record's version is superseded", () => {
        ...
        const existing = [record("hq.city", "Berlin", anchor(1, 1), "2026-01-01T00:00:00Z")];
        const result = reconcile(
            existing,
            [assertion("hq.city", "Munich", anchor(9, 20), "2026-02-01T00:00:00Z")],
            { currentVersionIds: current }
    ...
    it("handles independent keys in one batch", () => {
        ...
        expect(result.facts.map(f => f.factKey)).toEqual(["hq.city", "ceo.name", "founded.year"]);
    ...
    it("never mutates its inputs", () => {
        ...
        expect(result.facts[0]!.supersededValues.map(s => s.value)).toEqual(["Bonn", "Berlin"]);
    });
    ...
    it("rejects a ledger with duplicate fact keys", () => {
        ...
        expect(() => reconcile(dup, [], { currentVersionIds: current })).toThrow(RangeError);
    });
});
