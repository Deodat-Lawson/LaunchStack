...
describe("automation policy is server-controlled", () => {
  ...
  it("only relaxes the review gate through server configuration", () => {
    expect(
      resolveAutomationPolicy({ EMAIL_AUTOMATION_ALLOW_UNREVIEWED: "true" })
        .requireReviewPass,
    ).toBe(false);
  ...
  it("falls back to a sane recipient cap when misconfigured", () => {
    expect(
      resolveAutomationPolicy({ EMAIL_AUTOMATION_MAX_RECIPIENTS: "25" })
        .maxRecipients,
    ).toBe(25);

    for (const value of ["0", "-5", "abc", "1.5", ""]) {
      expect(
        resolveAutomationPolicy({ EMAIL_AUTOMATION_MAX_RECIPIENTS: value })
          .maxRecipients,
      ).toBe(200);
    }
  });
  ...
  it("rejects any unknown field rather than silently ignoring it", () => {
    for (const extra of [
      ...
      expect(AutomatedRunSchema.safeParse({ ...baseBody, ...extra }).success).toBe(
        false,
      );
    }
  });
