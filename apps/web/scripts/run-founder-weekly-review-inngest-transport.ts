...
async function main(): Promise<void> {
    ...
    try {
        // The harness isolates by database, not by search_path: generated
        // migrations qualify their foreign keys as "public"."…", so a schema
        // sandbox would resolve them to the wrong schema. Point the children at
        // the throwaway database itself.
        const schemaUrl = new URL(localUrl);
        schemaUrl.pathname = `/${testDb.databaseName}`;
