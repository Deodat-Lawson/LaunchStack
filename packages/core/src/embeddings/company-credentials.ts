 ...
 *
 * Credentials are read exclusively from `company_embedding_credentials`.
 * Legacy plaintext columns on `company` were dropped by migration 0011.
 */
 ...
 *
 * Reads from the encrypted credentials table only. If the ciphertext is
 * present but cannot be decrypted (bad key, corrupt data) we return null for
 * that field and log — silently swallowing would mask rotation bugs.
 */
 ...
 *
 * Writes to the encrypted table only.
 */
