    ...
    async process(
        ...
        const inputType = Buffer.isBuffer(input)
            ...
            : typeof input === "string" && (input.startsWith("http") || input.startsWith("/"))
              ...
              : "raw_string";
        console.log(
            `[TextAdapter] Processing: file=${options?.filename ?? "unknown"}, mime=${options?.mimeType ?? "none"}, input=${inputType}`
        );
        ...
        console.log(
            `[TextAdapter] Done: ${pages.length} pages, type=${isMarkdown ? "markdown" : "text"} (${elapsed}ms)`
        );
