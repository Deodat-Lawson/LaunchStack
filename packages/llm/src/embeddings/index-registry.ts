...
import { GEMINI_EMBEDDING_MODEL } from "../types";
...
export type EmbeddingProvider = "openai" | "ollama" | "huggingface";
...
const registryConfigSlot = createSlot<EmbeddingIndexRegistryConfig>("embeddings/indexRegistry");

export function configureEmbeddingIndexRegistry(config: EmbeddingIndexRegistryConfig): void {
...
function buildDynamicIndexes(config?: CompanyEmbeddingConfig): EmbeddingIndexConfig[] {
    ...
    if (
        ...
        ollamaModel &&
        ollamaDimension &&
        isSupportedDimensionTableDimension(ollamaDimension)
    ) {
        indexes.push({
    ...
    if (
        registry.huggingface?.embeddingModel &&
        huggingFaceDimension &&
        isSupportedDimensionTableDimension(huggingFaceDimension)
    ) {
        indexes.push({
...
export function getEmbeddingIndexRegistry(config?: CompanyEmbeddingConfig): EmbeddingIndexConfig[] {
    return [LEGACY_OPENAI_INDEX, GEMINI_EMBEDDING_INDEX, ...buildDynamicIndexes(config)];
}
...
export function getDefaultEmbeddingIndexKey(config?: CompanyEmbeddingConfig): string {
    ...
    return (
        resolveEffectiveEmbeddingConfig(config).embeddingIndexKey ?? LEGACY_OPENAI_INDEX.indexKey
    );
}
...
export function resolveEmbeddingIndex(
    ...
    if (!index) {
        throw new Error(`Embedding index "${targetKey}" is not registered or not enabled.`);
    }
