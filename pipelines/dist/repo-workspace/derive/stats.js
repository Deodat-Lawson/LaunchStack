...
export function computeRepoStats(files) {
    let totalBytes = 0;
    const languageAgg = new Map();
    const directoryAgg = new Map();
    ...
    const languages = [...languageAgg.entries()]
        .map(([language, agg]) => ({ language, ...agg }))
        .sort((a, b) => b.bytes !== a.bytes ? b.bytes - a.bytes : a.language < b.language ? -1 : 1)
        .slice(0, MAX_LANGUAGES);
    const largestDirectories = [...directoryAgg.entries()]
...
export function renderRepoStats(stats) {
