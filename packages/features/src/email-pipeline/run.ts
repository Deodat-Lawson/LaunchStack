...
import { createCampaign, isSuppressed, saveRecipients, saveSends, sentEmails } from "./db";
import { EMAIL_MODELS, EMAIL_PROMPT_VERSION } from "./models";
import type { EmailTemplate, Recipient, SendMode, SendResult, TemplateReview } from "./types";
...
export async function runEmailCampaign(args: {
    ...
    if (persist) {
        ...
        // Persist the list alongside the campaign so a send can be reconstructed
        // later — including each recipient's `vars`.
        await saveRecipients(campaignId, args.recipients);
    }
