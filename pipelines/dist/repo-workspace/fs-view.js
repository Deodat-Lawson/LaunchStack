...
export function matchesGlob(filePath, glob) {
    ...
    if (glob.endsWith("*"))
        return filePath.startsWith(glob.slice(0, -1));
...
export function createDirectoryView(rootDir, sha) {
    ...
    const readFile = async (relPath, maxBytes) => {
    ...
    const searchText = async (pattern, options) => {
        ...
        for (const file of files) {
            if (matches.length >= maxResults)
                break;
            if (!matchesGlob(file.path, options?.glob))
                continue;
...
//# sourceMappingURL=fs-view.js.map