...
function main() {
    ...
    for (const result of report.results) {
        ...
        console.log(
            `  ${icon} [${status}] ${result.scenarioName} (${result.domain}) — score: ${result.overallScore}, ${result.duration}ms`
        );

        for (const metric of result.metrics) {
            console.log(
                `      ${metric.name}: ${metric.score}/${metric.maxScore}${metric.details ? ` (${metric.details})` : ""}`
            );
        }
