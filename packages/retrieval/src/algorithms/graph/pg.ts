...
export class GraphRetriever extends BaseRetriever {
    ...
    async _getRelevantDocuments(
        ...
        console.log(
            `[GraphRetriever] Step 1: Extracted ${queryTerms.length} query terms: [${queryTerms.slice(0, 10).join(", ")}]`
        );
        ...
        console.log(
            `[GraphRetriever] Step 2: Found ${matchedEntityIds.length} matching entities (${Date.now() - step2Start}ms)`
        );
        ...
        for (let hop = 0; hop < this.maxHops; hop++) {
            ...
            console.log(
                `[GraphRetriever] Step 3: Hop ${hop + 1} expanded ${prevSize} → ${allEntityIds.size} entities (+${neighbors.length} neighbors)`
            );
        }
        console.log(
            `[GraphRetriever] Step 3: Total entity expansion: ${matchedEntityIds.length} → ${allEntityIds.size} (${Date.now() - step3Start}ms)`
        );
        ...
        console.log(
            `[GraphRetriever] Step 4: Found ${sectionIds.length} section IDs (${Date.now() - step4Start}ms)`
        );
    ...
    private async findMatchingEntities(terms: string[]): Promise<number[]> {
        ...
        const conditions = terms.map(term => ilike(kgEntities.name, `%${term}%`));

        const results = await getDb()
            .select({ id: kgEntities.id })
            .from(kgEntities)
            .where(and(eq(kgEntities.companyId, BigInt(this.companyId)), or(...conditions)))
     ...
     */
    private async getSectionIdsForEntities(entityIds: number[]): Promise<number[]> {
        if (entityIds.length === 0) return [];
