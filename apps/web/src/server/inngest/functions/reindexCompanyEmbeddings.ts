...
export const reindexCompanyEmbeddingsJob = inngest.createFunction(
    ...
    async ({ event, step, logger }) => {
        ...
        const chunkCount = await step.run("count-chunks", () => countCompanyChunks(companyId));
        ...
        while (true) {
            const batch = await step.run(`load-chunks-${batchIdx}`, async () =>
                loadCompanyChunks(companyId, cursor, BATCH_SIZE)
            );
 ...
 */
export async function markReindexFailed(companyId: number, error: unknown): Promise<void> {
    const message = error instanceof Error ? error.message : String(error);
    try {
        await failReindex(companyId, message);
    } catch (dbErr) {
        // Never throw out of an error handler.
        console.error(`[reindex] failed to record failure for company ${companyId}:`, dbErr);
    }
}
