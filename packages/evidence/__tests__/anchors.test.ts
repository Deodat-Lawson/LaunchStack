...
describe("anchorKey", () => {
  ...
  it("ignores chunkId and quote", () => {
    expect(
      anchorKey({ ...pageAnchor, chunkId: 991, quote: "some quoted text" }),
    ).toBe(anchorKey(pageAnchor));
  });
  ...
  it("throws on invalid anchors instead of producing malformed keys", () => {
    ...
    expect(() =>
      anchorKey({ ...pageAnchor, sourceId: 0 }),
    ).toThrow(RangeError);
    expect(() =>
      anchorKey({
        sourceId: 12,
...
describe("parseAnchorKey round-trips", () => {
  ...
  it.each(anchors.map((a) => [anchorKey(a)] as const))(
    "key -> anchor -> key for %s",
    (key) => {
...
describe("span validation", () => {
  it("rejects inverted ranges", () => {
    ...
    expect(
      isValidAnchorSpan({ kind: "time", startSeconds: 31.2, endSeconds: 12.5 }),
    ).toBe(false);
  ...
  it("accepts equal endpoints", () => {
    ...
    expect(
      isValidAnchorSpan({ kind: "time", startSeconds: 3, endSeconds: 3 }),
    ).toBe(true);
  ...
  it("rejects non-integer pages/offsets and negative or non-finite seconds", () => {
    ...
    expect(
      isValidAnchorSpan({ kind: "time", startSeconds: -0.1, endSeconds: 5 }),
    ).toBe(false);
    expect(
      isValidAnchorSpan({
        kind: "time",
  ...
  it("requires positive integer ids on the full anchor", () => {
    ...
    expect(
      isValidCitationAnchor({ ...pageAnchor, sourceVersionId: 1.5 }),
    ).toBe(false);
  });
});
