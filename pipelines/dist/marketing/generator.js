...
import { MarketingPipelineOutputSchema } from "./types.js";
 ...
 * ────────────────────────────────────────────────────────────── */
function contentTypeTemplate(type) {
    switch (type) {
        case "thread":
...
export async function generateVariants(args) {
    ...
    const settled = await Promise.allSettled(args.strategies.map(async (strategy) => {
    ...
    for (const failure of failures) {
        console.warn(`[marketing-pipeline] variant generation failed for ${failure.variantId}:`, failure.reason);
    }
