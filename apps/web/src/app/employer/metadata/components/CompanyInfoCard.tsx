...
function ConfBar({ confidence }: { confidence: number }) {
    ...
    const cls = bucket === "high" ? m.confBarHigh : bucket === "med" ? m.confBarMed : m.confBarLow;
    const label =
        bucket === "high"
            ? "High confidence"
            : bucket === "med"
...
function SourceChip({ fact }: { fact: AnyMetadataFact }) {
    ...
    return (
        <span className={m.src} title={first.doc_name}>
            ...
            {first.doc_name}
            {extra > 0 && <span style={{ color: "var(--ink-4)", marginLeft: 2 }}>+{extra}</span>}
        </span>
    );
}
...
function Field({ row, fact, onFieldSave }: FieldProps) {
    ...
    const fieldClass = [m.field, row.fullWidth ? m.fieldFull : "", editing ? m.fieldEditing : ""]
        .filter(Boolean)
        .join(" ");
                ...
                {editing ? (
                    ...
                    <>
                        {isMissing ? (
                            ...
                            <div className={`${m.fieldValue} ${m.fieldValueBlock}`}>{value}</div>
