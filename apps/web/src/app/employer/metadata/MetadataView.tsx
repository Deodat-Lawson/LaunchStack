...
type SectionId = "identity" | "people" | "services" | "markets" | "legal" | "provenance";
...
function TableOfContents({ metadata, activeSection, onChange }: TableOfContentsProps) {
    ...
    return (
        <aside className={m.toc} aria-label="On this page">
            ...
            <div className={m.tocFoot}>
                Hover any field and click <kbd>✎</kbd> to edit it inline. Sources are linked from
                each fact.
            </div>
        </aside>
    );
        ...
        <main style={{ minWidth: 0 }}>
            ...
            {!hasMetadata || !metadata ? (
                ...
                <>
                    ...
                    >
                        <CompanyInfoCard company={metadata.company} onFieldSave={onFieldSave} />
                    </SectionPanel>
                    ...
                    {metadata.people.length > 0 && (
                        ...
                        >
                            <PeopleSection people={metadata.people} onFieldSave={onFieldSave} />
                        </SectionPanel>
                    )}
...
function PageHeader({
    ...
    return (
        <header className={`${m.pageHeaderGrid} ${embedded ? m.pageHeaderEmbedded : ""}`}>
            <div>
                ...
                <p className={s.sub} style={{ marginTop: 8, maxWidth: 600 }}>
                    Who you are, in machine-readable form. Auto-extracted from your uploads, edited
                    by hand whenever the AI missed something — and ready to feed into every
                    workflow.
                </p>
            </div>
...
function SummaryStrip({ metadata, updatedAt }: SummaryStripProps) {
    const completeness = useMemo(() => {
        ...
        const filled = EXPECTED_COMPANY_FIELDS.filter(f => !!metadata.company[f.key]).length;
        ...
        const peoplePenalty =
            metadata.people.length > 0 ? (peopleWithoutRoles / metadata.people.length) * 0.15 : 0;
        const pct = Math.max(0, Math.min(100, Math.round((baseScore - peoplePenalty) * 100)));
    ...
    const peopleManual = metadata.people.filter(p => p.name.priority === "manual_override").length;
                        ...
                        <div className={m.smDetail}>
                            ...
                            {completeness.peopleWithoutRoles > 0 && (
                                <>
                                    ...
                                    {completeness.peopleWithoutRoles === 1 ? "" : "s"} without role
                                </>
                            )}
                        </div>
                    </div>
...
function MissingInfoBanner({ metadata, onDismiss, onJump }: MissingInfoBannerProps) {
    const missingFields = EXPECTED_COMPANY_FIELDS.filter(f => !metadata.company[f.key]);
        ...
        <div className={m.banner} role="status">
            ...
            <div className={m.bannerBody}>
                ...
                <p className={m.bannerText}>
                    The extractor couldn&apos;t find a confident answer for these. Click a chip to
                    jump and edit, or upload more documents and re-extract.
                </p>
    ...
    return (
        <div className={`${s.panel} ${m.empty}`}>
            ...
            <p className={m.emptyText}>
                Upload some documents, then run an extraction. The AI will pull company info,
                people, services, and legal entities. You can edit anything it got wrong.
            </p>
            <button
                type="button"
