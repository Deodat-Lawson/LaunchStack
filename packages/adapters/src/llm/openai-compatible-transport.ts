...
export function buildModelKwargs(
  options: Pick<
    CreateCompatibleChatOptions,
    "behavior" | "maxOutputTokens" | "requestPatch"
  >,
  ...
  const field =
    behavior.parameters.maxOutputTokensField ?? DEFAULT_MAX_OUTPUT_TOKENS_FIELD;
  if (
...
export function createOpenAiCompatibleChat(
  options: CreateCompatibleChatOptions,
): BaseChatModel {
  ...
  const capField =
    parameters.maxOutputTokensField ?? DEFAULT_MAX_OUTPUT_TOKENS_FIELD;
