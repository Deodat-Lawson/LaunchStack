 ...
 * host at startup via configureSecretBox() — apps/web/src/server/engine.ts
 * registers it from EMBEDDING_SECRETS_KEY. The slot is authoritative: this
 * module never reads process.env (ADR-002); an unconfigured slot surfaces
 * as MissingSecretsKeyError at the first encrypt/decrypt.
 */
