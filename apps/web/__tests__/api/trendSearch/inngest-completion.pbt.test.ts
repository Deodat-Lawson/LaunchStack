...
jest.mock("@launchstack/pipelines/trend-search/run", () => ({
    runTrendSearch: jest.fn(),
}));

jest.mock("@launchstack/pipelines/trend-search/db", () => {
    const actual = jest.requireActual("@launchstack/pipelines/trend-search/db");
...
import * as trendSearchDb from "@launchstack/pipelines/trend-search/db";
