...
const pair = (
    ...
    previousVersionId = 1,
    currentVersionId = 2,
    currentDate = "2026-02-02T00:00:00.000Z"
): VersionPair => ({
    documentId,
...
function groupFor(before: string, after: string, id = 1, versionPair = pair()) {
    const raw = buildRawDocumentChanges(versionPair, [
        modified(id, before, after, { documentId: versionPair.documentId }),
    ]).rawChanges;
...
function analyzed(
    ...
    return {
        ...
        materiality: {
            category,
            priority,
            confidence: category === "uncertain" ? "uncertain" : "strong",
            signals: [`${category}_test`],
        },
    };
}
...
describe("deterministic document-change materiality", () => {
    ...
    it("uses explicit precedence when multiple signals occur", () => {
        const result = analyzeDocumentChangeGroup(
            groupFor(
    ...
    it("keeps punctuation, capitalization, and paraphrase changes uncertain while retaining negation", () => {
        expect(analyzeDocumentChangeGroup(groupFor("Launch.", "Launch!"))).toMatchObject({
            category: "uncertain",
        });
        expect(
            analyzeDocumentChangeGroup(groupFor("Product roadmap", "product roadmap"))
        ).toMatchObject({ category: "uncertain" });
        expect(
            analyzeDocumentChangeGroup(
                groupFor("Reliable export retries", "Export retries are reliable")
            )
        ).toMatchObject({ category: "uncertain" });
        expect(
            analyzeDocumentChangeGroup(groupFor("supports exports", "does not support exports"))
        ).toMatchObject({
    ...
    it.each([
        ...
        expect(assessment.factualComparisons).toEqual(
    ...
    it("builds one stable bounded group item with copied spans and complete audit membership", () => {
        ...
        const alignments: ChunkAlignment[] = [
            modified(1, "Product owns retry telemetry.", "Platform owns retry telemetry.", {
                structurePath: "/ownership",
                structureTitle: "Ownership",
            }),
            modified(2, "Product owns alerting.", "Platform owns alerting.", {
                structurePath: "/ownership",
                structureTitle: "Ownership",
            }),
            {
                changeType: "removed",
                previousChunk: chunk(3, 1n, "Product owns paging.", {
                    structurePath: "/ownership",
                    structureTitle: "Ownership",
                }),
                alignmentMethod: "unmatched",
            ...
            {
                changeType: "added",
                currentChunk: chunk(4, 2n, "Platform owns paging.", {
                    structurePath: "/ownership",
                    structureTitle: "Ownership",
                }),
                alignmentMethod: "unmatched",
        ...
        expect(item).toEqual(
            buildCondensedDocumentChangeEvidence(
                versionPair,
                result.selectedGroups[0]!.group,
                result.selectedGroups[0]!.materiality
            )
        );
        ...
        expect(new Set(auditGroup.rawChangeIds)).toEqual(
            new Set(result.audit.rawChanges.map(change => change.rawChangeId))
        );
        expect(result.audit.rawChanges.find(change => change.changeType === "modified")).toEqual(
        ...
        expect(promptItem.metadata).toMatchObject({
            category: "ownership_change",
            materialityMethod: "deterministic",
            materialityConfidence: "strong",
            structureTitle: "Ownership",
            rawChangeCount: 4,
        });
    ...
    it("preserves concise single-change before/after semantics", () => {
        const versionPair = pair();
        const result = materializeDocumentChanges([
            { pair: versionPair, alignments: [modified(1, "planned", "shipped")] },
        ]);
        expect(result.items).toHaveLength(1);
        expect(result.items[0]).toMatchObject({
            sourceType: "document_change",
            metadata: expect.objectContaining({ category: "status_change", rawChangeCount: 1 }),
        });
    ...
    it("selects by materiality within documents while preserving document diversity and final chronology", () => {
        ...
        for (const documentId of [1n, 2n, 3n, 4n]) {
            candidates.push(
                analyzed("ownership_change", documentId, 1, "2026-02-09T00:00:00.000Z")
            );
            ...
            candidates.push(
                analyzed("requirement_change", documentId, 5, "2026-02-05T00:00:00.000Z")
            );
            candidates.push(analyzed("uncertain", documentId, 6, "2026-02-04T00:00:00.000Z"));
            candidates.push(
                analyzed("editorial_rewrite", documentId, 7, "2026-02-03T00:00:00.000Z")
            );
        }
        const selected = selectMaterialDocumentChangeGroups([...candidates].reverse());
        const counts = new Map<string, number>();
        for (const entry of selected.selectedGroups)
            counts.set(
                entry.group.documentId.toString(),
                (counts.get(entry.group.documentId.toString()) ?? 0) + 1
            );
        ...
        expect(
            selected.selectedGroups.some(
                entry => entry.materiality.category === "editorial_rewrite"
            )
        ).toBe(false);
        expect(
            selected.selectedGroups.filter(
                entry => entry.materiality.category === "ownership_change"
            )
        ).toHaveLength(4);
        expect(selected.selectedGroups.map(entry => entry.pair.currentCreatedAt.getTime())).toEqual(
            [...selected.selectedGroups]
                .map(entry => entry.pair.currentCreatedAt.getTime())
                .sort((a, b) => a - b)
        );
    });
    ...
    it("enforces eight per document and pair and prefers newer equal-category events", () => {
        ...
        expect(selected.truncatedGroups[0]!.pair.currentCreatedAt.toISOString()).toBe(
            "2026-02-01T00:00:00.000Z"
        );
        expect(selected.warnings).toContainEqual(
            expect.objectContaining({ code: "document_change_budget_truncated" })
        );
    });
    ...
    it("condenses the 40-chunk enterprise scenario while retaining every raw audit record", () => {
        ...
        for (let index = 0; index < 20; index++) {
            ...
            alignments.push({
                ...
                previousChunk: chunk(index + 1, 1n, content, {
                    documentId: 44n,
                    pageNumber: index + 1,
                    structurePath: `/unchanged-${index}`,
                }),
                currentChunk: chunk(1001 + index, 2n, content, {
                    documentId: 44n,
                    pageNumber: index + 1,
                    structurePath: `/unchanged-${index}`,
                }),
        ...
        for (let index = 0; index < 5; index++)
            alignments.push(
                modified(
                    ...
                    `wrapped line ${index}`,
                    `  wrapped\r\n line\u00a0${index}  `,
                    { documentId: 44n, structurePath: `/noop-${index}` }
                )
            );
        alignments.push(
            modified(201, "Product owns retry telemetry.", "Platform owns retry telemetry.", {
                documentId: 44n,
                structurePath: "/ownership",
                structureTitle: "Ownership",
            })
        ...
        alignments.push(
            modified(202, "Launch deadline is Q3.", "Launch deadline is Q4.", {
                documentId: 44n,
                structurePath: "/deadline",
                structureTitle: "Deadline",
            })
        );
        alignments.push(
            modified(203, "The release is planned.", "The release is launched.", {
                documentId: 44n,
                structurePath: "/status",
                structureTitle: "Status",
            })
        );
        alignments.push(
            modified(204, "Retries are optional.", "Retries are required.", {
                documentId: 44n,
                structurePath: "/requirements",
                structureTitle: "Requirements",
            })
        );
        const long =
            " Detailed source wording about the operating model and customer rollout remains copied for audit verification.".repeat(
                4
            );
        for (let index = 0; index < 6; index++)
            alignments.push(
                modified(
                    300 + index,
                    `Old rewrite fragment ${index}.${long}`,
                    `Rephrased rewrite fragment ${index}.${long}`,
                    { documentId: 44n, structurePath: "/rewrite", structureTitle: "Large rewrite" }
                )
            );
        for (let index = 0; index < 5; index++)
            alignments.push(
                modified(
                    400 + index,
                    `- Editorial source fragment ${index}.${long}`,
                    `* Editorial source fragment ${index}.${long}`,
                    { documentId: 44n, structurePath: "/editorial", structureTitle: "Editorial" }
                )
            );

        const result = materializeDocumentChanges([{ pair: versionPair, alignments }]);
        const shuffled = materializeDocumentChanges([
            { pair: versionPair, alignments: [...alignments].reverse() },
        ]);
        const snapshot = FounderWeeklyReviewEvidenceSnapshotSchema.parse({
            schemaVersion: "founder-weekly-review-evidence/v2",
            capturedAt: "2026-02-28T00:00:00.000Z",
        ...
        expect(result.diagnostics).toMatchObject({
            ...
            rawAuditCount: 15,
            condensedEvidenceCount: 6,
        });
        expect(result.analyzedGroups.map(group => group.materiality.category)).toEqual(
            expect.arrayContaining([
                "ownership_change",
                "deadline_change",
                "status_change",
        ...
        expect(result.selectedGroups.map(entry => entry.group.groupId)).toEqual(
            shuffled.selectedGroups.map(entry => entry.group.groupId)
        );
        expect(result.diagnostics.rawExcerptCharacters).toBeGreaterThan(
            result.diagnostics.condensedPromptFacingCharacters * 1.5
        );
...
describe("Founder Weekly Review evidence snapshot v2", () => {
    ...
    it("parses v1 and v2 without mutating v1", () => {
        const result = materializeDocumentChanges([
            { pair: pair(), alignments: [modified(1, "planned", "launched")] },
        ]);
        const v2 = {
            ...v1,
            schemaVersion: "founder-weekly-review-evidence/v2" as const,
            items: result.items,
            documentChangeAudit: result.audit,
        };
        ...
        expect(
            resolveDocumentChangeEvidenceAudit(parsedV2, result.items[0]!.sourceId)?.rawChanges
        ).toHaveLength(1);
        if (parsedV2.schemaVersion !== "founder-weekly-review-evidence/v2")
            throw new Error("Expected v2 snapshot");
        const baseDigest = buildFounderWeeklyReviewEvidenceDigest(parsedV2);
        const provenanceChanged = structuredClone(parsedV2);
        provenanceChanged.documentChangeAudit.rawChanges[0]!.previousExcerpt =
            "different copied provenance";
        ...
        expect(buildFounderWeeklyReviewPrompt(provenanceChanged)).toBe(
            buildFounderWeeklyReviewPrompt(parsedV2)
        );
    });
    ...
    it("keeps audit text out of the prompt and digests the complete snapshot", () => {
        ...
        const snapshot = FounderWeeklyReviewEvidenceSnapshotSchema.parse({
            ...
            documentChangeAudit: {
                ...
                rawChanges: [
                    {
                        rawChangeId: `raw_document_change:${"a".repeat(64)}`,
                        changeType: "removed",
                        alignmentMethod: "unmatched",
                        documentId: "1",
                        previousVersionId: 1,
                        ...
                        previousChunkId: 1,
                        currentChunkId: null,
                        previousExcerpt: auditSecret,
                        currentExcerpt: null,
                        previousHash: "b".repeat(64),
                        currentHash: null,
                        previousStructurePath: "/secret",
                        currentStructurePath: null,
                        previousStructureTitle: "Secret",
                        currentStructureTitle: null,
                ...
                groups: [
                    {
                        groupId: `document_change_group:${"c".repeat(64)}`,
                        evidenceSourceId: null,
                        documentId: "1",
                        previousVersionId: 1,
                        currentVersionId: 2,
                        structurePath: "/secret",
                        structureTitle: "Secret",
                        splitOrdinal: 0,
                        rawChangeIds: [`raw_document_change:${"a".repeat(64)}`],
                        category: "uncertain",
                        priority: 9,
                        confidence: "uncertain",
                        signals: ["no_strong_deterministic_signal"],
                        materialityMethod: "deterministic",
                        materialityVersion: "document-change-materiality/v1",
                    },
                ],
            },
        });
        ...
        const identicalDigest = buildFounderWeeklyReviewEvidenceDigest(
            FounderWeeklyReviewEvidenceSnapshotSchema.parse(JSON.parse(JSON.stringify(snapshot)))
        );
        const changed = structuredClone(snapshot);
        if (changed.schemaVersion !== "founder-weekly-review-evidence/v2")
            throw new Error("Expected v2 snapshot");
        changed.documentChangeAudit.groups[0]!.rawChangeIds = [
            `raw_document_change:${"d".repeat(64)}`,
        ];
    ...
    it("generates from v2 without a provider when prompt evidence is empty and records distinct digest metadata", async () => {
        ...
        expect(result.modelMetadata.evidenceSchemaVersion).toBe(
            "founder-weekly-review-evidence/v2"
        );
        expect(result.modelMetadata.attributes.evidenceDigest).toBe(
            buildFounderWeeklyReviewEvidenceDigest(snapshot)
        );
    });
});
