    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, background: "var(--bg)" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <h2 className="serif" style={{ fontSize: 24, margin: "3px 0 0", color: "var(--ink)", letterSpacing: "-0.02em" }}>
              Everything this workspace can reason over
            </h2>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 7, flexWrap: "wrap" }}>
              <Stat value={sources.length} label={sources.length === 1 ? "source" : "sources"} />
              <Divider />
              <Stat value={folders.length} label={folders.length === 1 ? "collection" : "collections"} />
              {indexing > 0 && (
                <>
                  <Divider />
                  <span style={{ fontSize: 11.5, color: "oklch(0.5 0.13 55)" }}>{indexing} indexing…</span>
                </>
              )}
            </div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
          ...
          >
            ...
            />
            {query && (
              <button onClick={() => setQuery("")} aria-label="Clear search" style={{ color: "var(--ink-3)", display: "flex" }}>
                <IconX size={11} />
              </button>
            )}
          </div>
          ...
          {selected.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button onClick={() => setSelected([])} style={{ fontSize: 11.5, color: "var(--ink-3)" }}>
                Clear
              </button>
            </div>
          )}
      ...
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "18px 24px 40px" }}>
        {loading && sources.length === 0 ? (
          <div style={{ fontSize: 13, color: "var(--ink-3)" }}>Loading your knowledge…</div>
        ) : filtered.length === 0 ? (
          <EmptyState
...
function MenuItem({
  ...
  return (
    ...
    >
      <span style={{ width: 12, display: "flex" }}>{selected && <IconCheck size={11} />}</span>
      ...
      {source.tags.length > 0 && (
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {source.tags.slice(0, 4).map((tag) => (
            <span key={tag} className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>
              #{tag}
            </span>
          ))}
        </div>
            ...
            {sources.map((source) => {
              ...
              return (
                ...
                >
                  ...
                  <Td>
                    <span style={{ fontSize: 11, color: domain.color, fontWeight: 600 }}>{source.domain}</span>
                  </Td>
...
function EmptyState({
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: "var(--ink-3)", maxWidth: 420 }}>
        {filtersActive
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {group.items.map((item) => (
              ...
              >
                ...
                <span style={{ fontSize: 10.5, color: "var(--ink-3)" }}>{item.desc}</span>
              </button>
            ))}
          </div>
        </div>
