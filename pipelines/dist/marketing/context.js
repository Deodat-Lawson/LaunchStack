...
export async function buildCompanyKnowledgeContext(args) {
    ...
    try {
        ...
        const kbResults = await getRag().companyEnsembleSearch(prompt, options);
 ...
 */
async function buildMetadataContext(companyId) {
    const db = getDb();
    const [row] = await db
...
// ============================================================================
async function buildRAGContext(companyId, prompt) {
    const db = getDb();
    const [companyRow, categoryRows] = await Promise.all([
        db.select().from(company).where(eq(company.id, companyId)).limit(1),
        db
    ...
    const rag = getRag();
    const options = { companyId, topK: 4, weights: [0.4, 0.6] };
    let generalSnippets = [];
    let differentiatorSnippets = [];
    try {
