...
/**
 * The complete Distribution client surface. It is deliberately independent of
 * route chrome so Studio can mount the same app without navigating away.
 */
export function DistributionApp({ embedded = false }: { embedded?: boolean }) {
    const state = useDistribution();
    const [programDialog, setProgramDialog] = useState<"closed" | "create" | "edit">("closed");
    const [importOpen, setImportOpen] = useState(false);
    const Root = embedded ? "div" : "main";
    const rootClassName = embedded
        ? "mx-auto flex h-full min-h-0 w-full max-w-7xl flex-col gap-5 overflow-y-auto px-4 pb-10 pt-6"
        : "mx-auto flex w-full max-w-7xl flex-1 flex-col gap-5 px-4 pb-10 pt-6";
            ...
            {!state.loading.programs && state.programs.length === 0 ? (
                <section className="border-line bg-panel rounded-2xl border p-8 text-center">
                    ...
                    <p className="text-ink-2 mx-auto mt-2 max-w-xl text-sm">
                        A program is the partner profile you want to recruit against: what you sell,
                        which territories, and which kinds of partner. Discovery runs, dossiers and
                        the pipeline all hang off it.
                    </p>
