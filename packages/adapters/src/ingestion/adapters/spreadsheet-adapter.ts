...
export class SpreadsheetAdapter implements SourceAdapter {
    ...
    async process(
        ...
        console.log(
            `[SpreadsheetAdapter] Processing: file=${options?.filename ?? "unknown"}, mime=${options?.mimeType ?? "none"}`
        );
        ...
        console.log(
            `[SpreadsheetAdapter] Workbook loaded: ${workbook.SheetNames.length} sheets (${workbook.SheetNames.join(", ")})`
        );
    ...
    private async resolveBuffer(input: string | Buffer): Promise<Buffer> {
        ...
        if (!res.ok) throw new Error(`SpreadsheetAdapter fetch failed: ${res.status}`);
     ...
     */
    private buildTables(data: string[][], rowLimit: number): ExtractedTable[] {
