...
export function summarizeToolInput(input) {
    ...
    if (record) {
        for (const key of TOOL_INPUT_SUMMARY_KEYS) {
            ...
            if (value)
                return truncateText(value.replace(/\s+/g, " ").trim(), TOOL_INPUT_MAX_CHARS);
        }
    }
    try {
...
export function extractToolResultText(content) {
    ...
    for (const block of content) {
        ...
        if (text)
            parts.push(text);
        else if (asString(record.type))
            parts.push(`[${String(record.type)}]`);
    }
    return parts.join("\n");
}
/**
...
export function parseClaudeSession(raw) {
    ...
    for (const line of raw.split("\n")) {
        ...
        if (type !== "user" && type !== "assistant") {
            if (type && METADATA_RECORD_TYPES.has(type))
                dropped.metadata += 1;
            else
                dropped.unknown += 1;
            continue;
        ...
        if (type === "user") {
            ...
            if (combined.length > 0)
                segments.push({ kind: "user", text: combined, at });
            continue;
        }
        // assistant
        if (!Array.isArray(content)) {
        ...
        for (const block of content) {
            ...
            if (blockType === "thinking" || blockType === "redacted_thinking") {
                ...
                if (text)
                    segments.push({ kind: "assistant", text, at });
            }
...
//# sourceMappingURL=parse-claude.js.map