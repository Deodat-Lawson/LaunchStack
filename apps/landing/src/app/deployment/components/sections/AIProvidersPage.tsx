...
import { BrainCircuit, Sparkles, ShieldAlert, Lightbulb, ExternalLink, Check } from "lucide-react";
...
const ENDPOINTS: Endpoint[] = [
    {
        ...
        envKey: "CHAT_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai\nCHAT_API_KEY=<your-google-ai-key>",
    },
    {
        id: "openrouter",
        name: "OpenRouter",
        ...
        <>
            ...
            >
                ...
                <p className={styles.heroSub}>
                    Launchstack talks to <span className={styles.serif}>one chat endpoint</span>{" "}
                    that speaks the OpenAI chat-completions protocol — and that endpoint can serve
                    many models. Point it anywhere; the orchestration, RAG, and analysis layers stay
                    the same.
                </p>
            </motion.div>
            ...
            >
                ...
                <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 14 }}>
                    <div className={styles.calloutInfo}>
                        <Lightbulb
                            size={18}
                            className={styles.calloutInfoIcon}
                            style={{ marginTop: 1 }}
                        />
                        <div>
                            <strong>A route is a job, not a model.</strong> Features ask for{" "}
                            <code className={styles.mono}>default</code>,{" "}
                            <code className={styles.mono}>fast</code>,{" "}
                            <code className={styles.mono}>reasoning</code>, or{" "}
                            <code className={styles.mono}>vision</code>; you decide which model
                            serves each. Unassigned routes inherit{" "}
                            <code className={styles.mono}>default</code> — but only when that model
                    ...
                    <div className={styles.calloutWarn}>
                        ...
                        />
                        <div>
                            <strong>Behavior is declared, never guessed.</strong> A model either
                            references a bundled preset or declares its own behavior in full —
                            nothing is inferred from the model id. Specialized routes fail closed:
                            with no vision-capable model configured, the image control disables
                            itself rather than sending an image to a model that will ignore it.
                        </div>
                    </div>
                    <div className={styles.calloutInfo}>
                        <Sparkles
                        ...
                        />
                        <div>
                            <strong>Mix and match.</strong> Chat, embeddings, and OCR are
                            independently configured, and none of them borrows another&apos;s
                            credential.
                        </div>
                    </div>
                </div>
            ...
            <Section title="Production notes">
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <div className={styles.calloutWarn}>
                        ...
                        />
                        <div>
                            <strong>Rate limits matter more than raw speed.</strong> Provider
                            throttling on the embedding route will surface as stuck document
                            uploads. Start with conservative batch sizes (
                            <code className={styles.mono}>EMBED_BATCH_SIZE=16</code>) when moving to
                            a new provider.
                        </div>
                    </div>
                    <div className={styles.calloutInfo}>
                        ...
                        />
                        <div>
                            <strong>Cost visibility.</strong> Enable LangSmith tracing (see the
                            LangChain Tracing page) to see per-run token usage across providers —
                            invaluable when deciding where to route each workload.
                        </div>
                    </div>
                </div>
            </Section>
...
const InlineRow: React.FC<{ title: string; badge: string; body: string }> = ({
    title,
    badge,
    body,
}) => (
