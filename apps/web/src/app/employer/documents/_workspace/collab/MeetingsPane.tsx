    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 8px 16px", minHeight: 0 }}>
        {loading && meetings.length === 0 && (
          <div style={{ padding: "12px 8px", fontSize: 12.5, color: "var(--ink-3)" }}>Loading…</div>
        )}
        {error && (
          <div style={{ padding: "12px 8px", fontSize: 12.5, color: "var(--danger)" }}>{error}</div>
        )}
        {!loading && meetings.length === 0 && !error && (
          <div style={{ padding: "12px 8px", fontSize: 12.5, color: "var(--ink-3)", lineHeight: 1.6 }}>
            No meetings yet. Start one and your agents will work through the agenda in a channel you
        ...
        {live.map((m) => (
          <ChannelRow key={m.id} meeting={m} active={m.id === selectedId} onClick={() => onSelect(m.id)} />
        ))}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {meeting.slackMirrorEnabled && (
          <IconSlack size={10} style={{ opacity: 0.7 }} />
        )}
      </span>
    </button>
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 420 }}>
        <h3 className="serif" style={{ fontSize: 20, margin: "0 0 8px", color: "var(--ink)" }}>
          Meetings happen in channels
        </h3>
        <p style={{ fontSize: 13, lineHeight: 1.6, color: "var(--ink-3)", margin: 0 }}>
          Pick the agents, state the objective, and watch them work through it. Read along, jump in
          with a comment, or take the floor entirely — the transcript is the same one Slack mirrors.
        </p>
      </div>
      {!loading && (
        <button
...
function ChannelView({
  ...
  const { detail, state, messages, minutes, loading, error, busy, control, postMessage } = controller;
  ...
  if (!detail || !state) {
    return (
      <div style={{ flex: 1, display: "grid", placeItems: "center", color: "var(--danger)", fontSize: 13 }}>
  ...
  return (
    <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, minHeight: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        >
          {messages.length === 0 && (
            <div style={{ padding: "24px 24px", fontSize: 13, color: "var(--ink-3)", lineHeight: 1.6 }}>
              Nothing said yet. Press <strong>Run</strong> to let the agents take their turns, or write
          ...
          {messages.map((message, index) => (
            <MessageRow
              ...
              participant={participantsById.get(message.author.onBehalfOfPersonaId ?? message.author.id)}
            />
          ))}
        ...
        {showMinutes && minutes && <MinutesPanel minutes={minutes} onClose={() => setShowMinutes(false)} />}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)" }}>{slug ?? title}</span>
...
function StatusPill({ label, tone }: { label: string; tone: "live" | "idle" | "human" | "done" | "bad" }) {
...
function ParticipantChip({
  ...
  return (
    ...
    >
      <Avatar name={participant.displayName} color={color} size={16} />
      @{participant.id}
...
function MessageRow({
  ...
  const servedBy = typeof message.meta?.servedByNode === "string" ? message.meta.servedByNode : null;
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {!grouped && (
          <div style={{ display: "flex", alignItems: "baseline", gap: 7, marginBottom: 2 }}>
            ...
            {message.slackTs && <IconSlack size={9} style={{ color: "var(--ink-3)", opacity: 0.7 }} />}
            <span className="mono" style={{ fontSize: 9.5, color: "var(--ink-3)", opacity: 0.7 }}>
...
function renderMentions(text: string): React.ReactNode[] {
  ...
  for (const match of text.matchAll(/@([a-zA-Z0-9_-]+)/g)) {
    ...
    parts.push(
      <span
        key={`${start}-${match[0]}`}
        style={{ color: "var(--accent-ink)", background: "var(--accent-soft)", borderRadius: 3, padding: "0 2px" }}
      >
        {match[0]}
      </span>,
    );
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <button onClick={onClose} aria-label="Close minutes" style={{ color: "var(--ink-3)" }}>
          <IconX size={13} />
        </button>
      </div>
      ...
      <MinutesSection title="Action items" empty="No owners assigned yet.">
        {minutes.actionItems.map((a) => (
          <li key={`${a.sourceSeq}-${a.text}`} style={minutesItemStyle}>
            {a.text}
            {a.owner && (
              <span style={{ display: "block", fontSize: 10.5, color: "var(--accent-ink)", marginTop: 2 }}>
                @{a.owner}
              </span>
            )}
          </li>
      ...
      >
        ...
        {minutes.humanInterventions} human message{minutes.humanInterventions === 1 ? "" : "s"}
        <div style={{ marginTop: 6 }}>
          Extracted from the transcript, never generated — every line above was said by someone in
          this channel.
        </div>
      </div>
    </aside>
