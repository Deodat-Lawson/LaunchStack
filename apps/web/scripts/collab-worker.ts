...
function buildRuntimes(nodeId: string): AgentRuntime[] {
  ...
  if (scripted) {
    return [new ScriptedAgentRuntime(JSON.parse(scripted) as Record<string, string[]>, { nodeId })];
  }
...
async function main(): Promise<void> {
  ...
  console.log(`[collab-worker] ready node=${env.nodeId} hub=${env.hubUrl} personas=${env.personaIds.join(",") || "*"}`);
