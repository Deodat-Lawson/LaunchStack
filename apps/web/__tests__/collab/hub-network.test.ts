...
describe("meeting hub over HTTP", () => {
  ...
  it("rejects a request signed with the wrong secret", async () => {
    const client = new HubClient({ hubUrl: rig.server.url, nodeId: "worker-b", secret: "wrong-secret" });
  ...
  it("rejects a node registering under someone else's id", async () => {
    ...
    await expect(
      client.request("POST", "/collab/v1/nodes/register", { nodeId: "worker-c", personaIds: [] }),
  ...
  it("reports connected nodes and the personas they claim", async () => {
    ...
    try {
      ...
      expect(nodes[0]).toMatchObject({ nodeId: "worker-b", personaIds: ["eng"], connected: true });
    } finally {
      await worker.stop();
    }
  ...
  it("times out a remote turn and keeps the meeting alive", async () => {
    ...
    const worker = new AgentWorker({ client, personaIds: ["eng"], pollWaitMs: 1_000, runtimes: [stalling] });
    await worker.start();
    ...
    try {
      const { orchestrator, config } = await createMeeting({
        ...
        runtimes: [new ScriptedAgentRuntime({ pm: ["Carrying on without Sam. MEETING_COMPLETE"] })],
        hub,
        maxTurns: 4,
      });
  ...
  it("recovers when a worker restarts mid-meeting", async () => {
    ...
    const first = new AgentWorker({
      ...
      personaIds: ["eng"],
      pollWaitMs: 500,
      runtimes: [new ScriptedAgentRuntime({ eng: ["From the first worker."] }, { nodeId: "worker-b" })],
    });
    await first.start();
    await first.stop();
    ...
    const second = new AgentWorker({
      ...
      personaIds: ["eng"],
      pollWaitMs: 500,
      runtimes: [new ScriptedAgentRuntime({ eng: ["From the replacement worker."] }, { nodeId: "worker-b" })],
    });
    await second.start();
