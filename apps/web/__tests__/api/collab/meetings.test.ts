...
interface Harness {
    ...
    meetings: Map<
        string,
        {
            ...
            config: Parameters<typeof mockBuildRow>[0];
            row: ReturnType<typeof mockBuildRow>;
        }
    >;
}
    ...
    return {
        ...
        getMeetingRuntime: (meetingId: string) => {
            ...
            return Promise.resolve(
                entry
                    ? { orchestrator: entry.orchestrator, config: entry.config, row: entry.row }
                    : null
            );
        },
...
describe("collab meeting routes", () => {
    ...
    describe("authentication", () => {
        it("rejects every route when signed out", async () => {
            ...
            expect((await control(jsonRequest({ action: "run" }), params("mtg_1"))).status).toBe(
                401
            );
            expect((await postMessage(jsonRequest({ text: "hi" }), params("mtg_1"))).status).toBe(
                401
            );
        });
    });
    ...
    describe("POST /api/collab/meetings/[id]/control", () => {
        it("advances one turn at a time with step", async () => {
            ...
            expect(
                ((await second.json()) as { state: { turnIndex: number } }).state.turnIndex
            ).toBe(2);
        });
        ...
        it("bounds run so a request cannot span the whole meeting", async () => {
            ...
            const response = await control(
                jsonRequest({ action: "run", limit: 2 }),
                params(meeting.id)
            );
            expect(
                ((await response.json()) as { state: { turnIndex: number } }).state.turnIndex
            ).toBe(2);
        });
        ...
        it("pauses, takes over, releases, and completes", async () => {
            ...
            expect(((await paused.json()) as { state: { status: string } }).state.status).toBe(
                "paused"
            );
            ...
            const takeoverState = (await takeover.json()) as {
                state: {
                    status: string;
                    controller?: { displayName: string; asPersonaId?: string };
                };
            };
            ...
            expect(((await released.json()) as { state: { status: string } }).state.status).toBe(
                "running"
            );

            const done = await control(jsonRequest({ action: "complete" }), params(meeting.id));
            expect(((await done.json()) as { state: { status: string } }).state.status).toBe(
                "completed"
            );
        });
        ...
        it("rejects an unknown action", async () => {
            const { meeting } = await startMeeting();
            expect(
                (await control(jsonRequest({ action: "detonate" }), params(meeting.id))).status
            ).toBe(400);
        });
    });
    ...
    describe("POST /api/collab/meetings/[id]/messages", () => {
        ...
        it("attributes the message to the held seat during a takeover", async () => {
            const { meeting } = await startMeeting();
            await control(
                jsonRequest({ action: "takeover", asPersonaId: "eng" }),
                params(meeting.id)
            );

            const response = await postMessage(
                jsonRequest({ text: "Engineering says two sprints." }),
                params(meeting.id)
            );
            const body = (await response.json()) as {
        ...
        it("refuses to post into a finished meeting", async () => {
            ...
            const response = await postMessage(
                jsonRequest({ text: "one more thing" }),
                params(meeting.id)
            );
            expect(response.status).toBe(400);
        ...
        it("rejects empty and oversized messages", async () => {
            const { meeting } = await startMeeting();
            expect((await postMessage(jsonRequest({ text: "" }), params(meeting.id))).status).toBe(
                400
            );
            expect(
                (await postMessage(jsonRequest({ text: "x".repeat(8_001) }), params(meeting.id)))
                    .status
            ).toBe(400);
        });
    });
    ...
    it("keeps a full human-in-the-loop meeting coherent end to end", async () => {
        ...
        await postMessage(
            jsonRequest({ text: "Hold — finance wants tier C considered." }),
            params(meeting.id)
        );
        ...
        expect(((await blocked.json()) as { state: { turnIndex: number } }).state.turnIndex).toBe(
            1
        );
        ...
        expect(
            detail.messages.some(m => m.kind === "system" && m.text.includes("took the floor"))
        ).toBe(true);
        expect(
            detail.messages.filter(m => m.author.kind === "agent" && m.kind === "chat").length
        ).toBeGreaterThan(1);
    });
});
