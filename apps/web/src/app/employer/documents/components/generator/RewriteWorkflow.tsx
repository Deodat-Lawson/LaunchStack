...
function sectionTitleOf(chunk: string, index: number): string {
    ...
    const stripped = firstLine.replace(/^#{1,6}\s*/, "").trim();
    const truncated = stripped.length > 56 ? `${stripped.slice(0, 56)}…` : stripped;
    return truncated.length > 0 ? `${index + 1}. ${truncated}` : `${index + 1}. Section`;
}
...
function splitIntoSections(text: string): Pick<Section, "id" | "index" | "title" | "original">[] {
...
function mapResponseToProposals(baseSections: Section[], response: string): Section[] {
...
export function RewriteWorkflow({
    ...
    const [text, setText] = useState<string>(persistedState?.text ?? initialText);
    const [customPrompt, setCustomPrompt] = useState<string>(
        persistedState?.options?.customPrompt ?? ""
    );
    const [activePresets, setActivePresets] = useState<Set<string>>(() => new Set<string>());
    const [guardrailState, setGuardrailState] = useState<Record<string, boolean>>({
        structure: true,
        voice: true,
        conflict: true,
    });
    const [proposedText, setProposedText] = useState<string>(persistedState?.rewrittenText ?? "");
    ...
    const [currentSectionId, setCurrentSectionId] = useState<string>(baseDescriptors[0]?.id ?? "");
    ...
    const currentSection = sections.find(sec => sec.id === currentSectionId) ?? sections[0];
    ...
    const acceptedPct = totalReviewable > 0 ? (counts.acc / totalReviewable) * 100 : 0;
    const rejectedPct = totalReviewable > 0 ? (counts.rej / totalReviewable) * 100 : 0;
    ...
    const composedPrompt = useMemo(() => {
        ...
        if (guardrailState.structure) lines.push("Preserve paragraph structure and breaks.");
    ...
    const toggleSectionSelected = useCallback((id: string) => {
        setSections(prev =>
            prev.map(sec => (sec.id === id ? { ...sec, selected: !sec.selected } : sec))
        );
    }, []);

    const setSectionStatus = useCallback((id: string, status: SectionStatus) => {
        setSections(prev => prev.map(sec => (sec.id === id ? { ...sec, status } : sec)));
    }, []);
        ...
        <aside className={s.rwsCfg}>
            <div className={s.rwsCfgHead}>
                ...
                <p className={s.rwsCfgHeadSub}>
                    Drift will rewrite only the sections you choose, keeping defined terms and
                    citations intact.
                </p>
            </div>
            ...
            <div className={s.rwsCfgSection}>
                ...
                <div className={s.rwsSourceItem}>
                    ...
                    <div className={s.rwsSourceUsed}>{sections.length} §</div>
                </div>
                <textarea
            ...
            <div className={s.rwsCta}>
                ...
                <div className={s.rwsCtaMeta}>~10s · {wordCount.toLocaleString()} words</div>
            </div>
        </aside>
    );
}
        ...
        <>
            <div className={s.rwsPipeHead}>
                <div style={{ minWidth: 0 }}>
                    <h1 className={s.rwsPipeTitle}>
                        Section {index + 1} · <em>{sectionShortName(section.title)}</em>
                    </h1>
            ...
            <div className={s.rwsPipeBody}>
                ...
                <div className={`${s.rwsPane} ${s.rwsPaneProposed}`}>
                    <div className={`${s.rwsPaneEyebrow} ${s.rwsPaneEyebrowProposed}`}>
                        Proposed · v2
                    </div>
...
function DiffProse({ parts, mode }: { parts: DiffPart[]; mode: "original" | "proposed" }) {
    const isOriginal = mode === "original";
    const visibleParts = parts.filter(p => (isOriginal ? !p.added : !p.removed));
...
function ReviewRail({
    ...
    const finalCta =
        counts.acc === 0 ? "Apply 0 accepted → v2" : `Apply ${counts.acc} accepted → v2`;
    ...
    return (
        <aside className={s.rwsOut}>
            ...
            <div className={s.rwsProgress}>
                <div className={s.rwsProgressAccepted} style={{ width: `${acceptedPct}%` }} />
                <div className={s.rwsProgressRejected} style={{ width: `${rejectedPct}%` }} />
            </div>
