    ...
    it("returns only company-owned current-version embedded chunks with deterministic ranking", async () => {
        ...
        try {
            ...
            const [v2] = await test.db
                ...
                .returning();
            const [noneVersion] = await test.db
                .insert(documentVersions)
                .values({
                    documentId: BigInt(noCurrent!.id),
                    versionNumber: 1,
                    url: "local://n",
                    mimeType: "text/plain",
                })
                .returning();
            const [foreignVersion] = await test.db
                .insert(documentVersions)
                .values({
            ...
            await test.db
                .update(document)
                .set({ currentVersionId: BigInt(v2!.id) })
                .where(sql`${document.id} = ${doc!.id}`);
            await test.db.insert(documentContextChunks).values([
                ...
                {
                    documentId: BigInt(noCurrent!.id),
                    versionId: BigInt(noneVersion!.id),
                    content: "no current",
                    tokenCount: 1,
                    charCount: 10,
                    embedding: vectorSql(0),
            ...
            const store = new StrictCurrentWorkspaceDocumentStore(test.db, {
                embedQuery: jest.fn().mockResolvedValue(vector(0)),
            });
            const result = await store.retrieveRelevantCurrentDocumentChunks({
                companyId: BigInt(owner!.id),
                founderContext: "current",
                topK: 1,
            });
    ...
    it("collects a computed v1-to-v2 diff plus only a current relevant workspace document", async () => {
        ...
        try {
            ...
            await test.db
                ...
                .where(sql`${document.id} = ${a!.id}`);
            await test.db
                .update(document)
                .set({ currentVersionId: BigInt(b1!.id) })
                .where(sql`${document.id} = ${b!.id}`);
            await test.db
                .update(document)
                .set({ currentVersionId: BigInt(foreignVersion!.id) })
                .where(sql`${document.id} = ${foreign!.id}`);
            await test.db.insert(documentContextChunks).values([
                {
                    documentId: BigInt(a!.id),
                    versionId: BigInt(a1!.id),
                    content: "product release before",
                ...
                },
                {
                    ...
                    content: "current blocker context",
            ...
            const service = new FounderWeeklyReviewEvidenceService(
                ...
                { kind: "computed", store: new FounderWeeklyReviewDocumentVersionStore(test.db) },
                new StrictCurrentWorkspaceDocumentStore(test.db, {
                    embedQuery: jest.fn().mockResolvedValue(vector(0)),
                })
            );
            const input = {
                companyId: BigInt(owner!.id),
                reportingPeriod: { start: "2026-02-01", end: "2026-02-07" },
                workspaceTimezone: "UTC",
                founderContext: "blocker",
                actor: { externalUserId: "u" },
                requestKey: "computed-integration",
            };
            ...
            expect(first.items.filter(item => item.sourceType === "document_change")).toHaveLength(
                1
            );
                ...
                expect.arrayContaining([
                    expect.objectContaining({
                        ...
                        metadata: expect.objectContaining({
                            previousVersionId: a1!.id,
                            currentVersionId: a2!.id,
                        }),
                    }),
                ])
            );
            const workspaceItem = first.items.find(
            ...
            expect(
                first.items.some(
                    item =>
                        item.excerpt.includes("v3 must not diff") ||
                        item.excerpt.includes("legacy relevant") ||
                        item.excerpt.includes("foreign relevant")
                )
            ).toBe(false);
        } finally {
            await test.close();
