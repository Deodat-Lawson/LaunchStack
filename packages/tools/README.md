...
| **investor-search**    | `@launchstack/tools/investor-search`    | Venture funds raising now, from SEC Form D filings (EDGAR full-text search + each filing's primary document): managers, office, phone, target size, amount closed. Keyless; paced under SEC's 10 req/s, retries EFTS's passing 500s, hides single-deal SPVs. User-Agent from `SEC_EDGAR_USER_AGENT`. | Investor relations (Studio) |
...
| **image-generation**    | `@launchstack/tools/image-generation`    | Generate an image and persist it through an injected store, returning a reference rather than bytes. Endpoint resolves IMAGE_* → CHAT_* → AI_*, so an OpenRouter chat credential already serves images. Spend is reported only after a successful write. | agent image tool (planned); marketing assets (planned) |
