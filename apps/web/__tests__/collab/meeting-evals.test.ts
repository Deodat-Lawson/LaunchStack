...
function state(overrides: Partial<MeetingState> = {}): MeetingState {
  return { meetingId: "mtg_eval", status: "completed", turnIndex: 4, nextSpeakerId: null, ...overrides };
}
...
describe("evaluateMeeting", () => {
  ...
  it("scores participation as zero for a pure monologue and high for an even split", () => {
    const monologue = evaluateMeeting(
      config(),
      state(),
      [msg("pm", "one"), msg("pm", "two"), msg("pm", "three"), msg("pm", "four")],
    );
    const balanced = evaluateMeeting(
      config(),
      state(),
      [msg("pm", "one"), msg("eng", "two"), msg("pm", "three"), msg("eng", "four")],
    );

    const score = (r: typeof monologue) => r.dimensions.find((d) => d.id === "participation")!.score;
  ...
  it("marks a direct question that was never answered", () => {
    ...
    const score = (r: typeof ignored) => r.dimensions.find((d) => d.id === "responsiveness")!.score;
  ...
  it("rates ending on the objective above running out of turns", () => {
    const clean = evaluateMeeting(config({ maxTurns: 6 }), state({ turnIndex: 3 }), [msg("pm", "Done.")]);
    const capped = evaluateMeeting(config({ maxTurns: 6 }), state({ turnIndex: 6 }), [msg("pm", "Done.")]);
