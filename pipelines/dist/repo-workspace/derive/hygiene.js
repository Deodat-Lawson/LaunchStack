...
export function isDeniedPath(filePath) {
    ...
    if (isDeniedFilename(basename))
        return true;
    if (REPO_DENIED_PATTERNS.some(pattern => pattern.test(basename)))
        return true;
    return REPO_DENIED_PATTERNS.some(pattern => pattern.test(filePath));
}
export function buildHygieneManifest(files) {
...
export function makeDeniedSet(manifest) {
