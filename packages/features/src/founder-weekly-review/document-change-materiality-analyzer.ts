...
export const DocumentChangeMaterialityAnalysisResultSchema = z
    .object({
        disposition: z
            .enum(["material", "non_material", "uncertain"])
            .describe("Whether the underlying business state materially changed."),
        category: z
            .enum(DOCUMENT_CHANGE_CATEGORIES)
            .describe(
                "The primary changed business-state dimension, editorial_rewrite, or uncertain."
            ),
        summary: z
            .string()
            .trim()
            .min(1)
            .max(DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.summaryCharacters)
            .describe(
                "One concise sentence of at most 320 characters. Do not include reasoning or repeat the full source."
            ),
        beforeKeyPoint: z
            .string()
            .trim()
            .min(1)
            .max(DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.keyPointCharacters)
            .describe("Optional verbatim copied before span of at most 240 characters.")
            .optional(),
        afterKeyPoint: z
            .string()
            .trim()
            .min(1)
            .max(DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.keyPointCharacters)
            .describe("Optional verbatim copied after span of at most 240 characters.")
            .optional(),
    ...
    .superRefine((result, context) => {
        if (result.disposition === "uncertain" && result.category !== "uncertain") {
            context.addIssue({
                ...
                message: "Uncertain disposition requires uncertain category.",
                path: ["category"],
            });
        }
        if (result.disposition === "material" && result.category === "editorial_rewrite") {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Material disposition cannot use editorial_rewrite.",
                path: ["category"],
            });
        }
    });
...
function changedCharacters(group: DocumentChangeGroup): number {
    return group.rawChanges.reduce(
        (total, change) =>
            total +
            (change.previousNormalizedContent?.length ?? 0) +
            (change.currentNormalizedContent?.length ?? 0),
        0
    );
}
...
function semanticRisk(
    ...
    if (assessment.confirmedFactualDeltas.length > 0 && assessment.possibleSignals.length === 0)
        return "confirmed_factual_delta";
    if (assessment.possibleSignals.length > 0) return "ambiguous_business_signal";
    return "paraphrase_possible";
}

export function hasStrongConfirmedFactualDeltaBypass(
    analyzed: AnalyzedDocumentChangeGroup
): boolean {
    const { group, materiality } = analyzed;
    if (group.rawChanges.length !== 1 || group.rawChanges[0]!.changeType !== "modified")
        return false;
    if (
        changedCharacters(group) > 700 ||
        materiality.category === "uncertain" ||
        materiality.category === "editorial_rewrite"
    )
        return false;
    const assessment = analyzeDocumentChangeFactualDeltas(group, materiality.signals);
    return (
        assessment.confirmedFactualDeltas.length > 0 &&
...
export function shouldAnalyzeDocumentChangeGroup(
    ...
    if (hasStrongConfirmedFactualDeltaBypass(analyzed))
    ...
    if (
        factualAssessment.possibleSignals.length > 0 &&
        factualAssessment.confirmedFactualDeltas.length === 0
    ) {
    ...
    if (
        only?.changeType === "modified" &&
        changedCharacters(group) >= 240 &&
        lexicalSimilarity(only) < 0.45
    ) {
...
function stableCandidateOrder(a: AnalysisCandidate, b: AnalysisCandidate): number {
    return (
        a.eligibility.priority - b.eligibility.priority ||
        b.analyzed.pair.currentCreatedAt.getTime() - a.analyzed.pair.currentCreatedAt.getTime() ||
        compareOrdinal(
            a.analyzed.group.structurePath ?? a.analyzed.group.structureTitle ?? "",
            b.analyzed.group.structurePath ?? b.analyzed.group.structureTitle ?? ""
        ) ||
...
export function selectDocumentChangeGroupsForMaterialityAnalysis(
    ...
    for (const priority of [
        ...new Set(candidates.map(candidate => candidate.eligibility.priority)),
    ].sort((a, b) => a - b)) {
        ...
        const documents = [...byDocument.keys()].sort((a, b) =>
            BigInt(a) < BigInt(b) ? -1 : BigInt(a) > BigInt(b) ? 1 : 0
        );
        let progress = true;
        while (
            progress &&
            selected.length < DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.callsPerReview
        ) {
            progress = false;
            for (const documentId of documents) {
                const candidate = byDocument
                    .get(documentId)!
                    .find(value => !selectedIds.has(value.analyzed.group.groupId));
                if (!candidate) continue;
                selected.push(candidate.analyzed);
                selectedIds.add(candidate.analyzed.group.groupId);
                progress = true;
                if (selected.length >= DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.callsPerReview)
                    break;
            }
        }
    ...
    return {
        selected,
        skipped: candidates
            .filter(candidate => !selectedIds.has(candidate.analyzed.group.groupId))
            .map(candidate => candidate.analyzed),
    };
}
...
export function buildDocumentChangeMaterialityAnalysisInput(
    analyzed: AnalyzedDocumentChangeGroup
): {
    input: DocumentChangeMaterialityAnalysisInput;
    inputDigest: string;
    ...
    const changes = analyzed.group.rawChanges
        ...
        .map(change => ({
            ...
            ...(compactExcerpt(change.previousChunk?.content)
                ? { previousExcerpt: compactExcerpt(change.previousChunk?.content) }
                : {}),
            ...(compactExcerpt(change.currentChunk?.content)
                ? { currentExcerpt: compactExcerpt(change.currentChunk?.content) }
                : {}),
            alignmentMethod: change.alignmentMethod,
        }));
    const factualAssessment = analyzeDocumentChangeFactualDeltas(
        analyzed.group,
        analyzed.materiality.signals
    );
    ...
    return {
        ...
        truncated:
            analyzed.group.rawChanges.length > changes.length ||
            analyzed.group.rawChanges.some(
                change =>
                    (change.previousChunk?.content.replace(/\s+/g, " ").trim().length ?? 0) >
                        DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.excerptCharactersPerSide ||
                    (change.currentChunk?.content.replace(/\s+/g, " ").trim().length ?? 0) >
                        DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.excerptCharactersPerSide
            ),
    };
}
...
function effectiveAnalyzedGroup(success: SuccessfulAnalysis): AnalyzedDocumentChangeGroup | null {
    ...
    const category =
        success.result.disposition === "uncertain" ? "uncertain" : success.result.category;
    return {
        ...success.analyzed,
        materiality: {
            category,
            ...
            confidence:
                success.result.disposition === "uncertain"
                    ? "uncertain"
                    : confidenceFromAnalysis(success.result.confidence),
            signals:
                success.result.disposition === "uncertain"
...
function verifiedKeyPoint(
    ...
    return changes.some(change => normalizedCopied(change[side]?.content ?? "").includes(candidate))
        ? value
        : undefined;
}

function firstCopiedSpan(
    changes: readonly RawDocumentChange[],
    side: "previousChunk" | "currentChunk"
): string | undefined {
...
function buildAnalyzerEvidence(
    success: SuccessfulAnalysis,
    effective: AnalyzedDocumentChangeGroup
): FounderWeeklyReviewEvidenceItem {
    const deterministic = buildCondensedDocumentChangeEvidence(
        effective.pair,
        effective.group,
    ...
    const before =
        verifiedKeyPoint(
            success.result.beforeKeyPoint,
            effective.group.rawChanges,
            "previousChunk"
        ) ?? firstCopiedSpan(effective.group.rawChanges, "previousChunk");
    const after =
        verifiedKeyPoint(
            success.result.afterKeyPoint,
            effective.group.rawChanges,
            "currentChunk"
...
function analysisSnapshot(success: SuccessfulAnalysis): DocumentChangeMaterialityAnalysisSnapshot {
    return {
        ...
        ...(success.result.beforeKeyPoint
            ? {
                  beforeKeyPoint: verifiedKeyPoint(
                      success.result.beforeKeyPoint,
                      success.analyzed.group.rawChanges,
                      "previousChunk"
                  ),
              }
            : {}),
        ...(success.result.afterKeyPoint
            ? {
                  afterKeyPoint: verifiedKeyPoint(
                      success.result.afterKeyPoint,
                      success.analyzed.group.rawChanges,
                      "currentChunk"
                  ),
        ...
        analysisPromptVersion:
            success.metadata.promptVersion ?? DOCUMENT_CHANGE_MATERIALITY_ANALYZER_PROMPT_VERSION,
        ...(success.metadata.provider
            ? { analysisProvider: success.metadata.provider.slice(0, 128) }
            : {}),
...
function warning(
    code: DocumentChangeProcessingWarning["code"],
    message: string
): DocumentChangeProcessingWarning {
    return { code, message };
}

async function executeBounded<T, R>(
    values: readonly T[],
    concurrency: number,
    task: (value: T) => Promise<R>
): Promise<PromiseSettledResult<R>[]> {
    const results: Array<PromiseSettledResult<R> | undefined> = [];
    ...
    return results.map((result, index) => {
        if (!result) throw new Error(`Bounded task ${index} did not settle.`);
        return result;
    });
}
...
export async function materializeDocumentChangesWithAnalyzer(
    ...
    const eligible = deterministic.analyzedGroups.filter(
        group => shouldAnalyzeDocumentChangeGroup(group).eligible
    );
    const analysisSelection = selectDocumentChangeGroupsForMaterialityAnalysis(
        deterministic.analyzedGroups
    );
    const prepared = analysisSelection.selected.map(analyzed => ({
        analyzed,
        ...buildDocumentChangeMaterialityAnalysisInput(analyzed),
    }));
    ...
    if (analysisSelection.skipped.length > 0 || prepared.some(value => value.truncated)) {
        warnings.push(
            warning(
                ...
                "Optional materiality analysis used bounded call or input capacity; remaining groups used deterministic fallback."
            )
        );
    }
    const settled = await executeBounded(
        prepared,
        DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.concurrency,
        async preparedInput => {
    ...
    for (const result of settled) {
        if (result.status === "fulfilled") successes.push(result.value);
            ...
            result.reason instanceof DocumentChangeMaterialityAnalyzerError &&
            ...
            result.reason instanceof DocumentChangeMaterialityAnalyzerError &&
            result.reason.code === "unavailable"
        )
            unavailable = true;
    ...
    if (invalid)
        warnings.push(
            warning(
                ...
                "An optional materiality result failed strict validation; deterministic evidence was retained."
            )
        );
    if (timeout)
        warnings.push(
            warning(
                ...
                "Optional materiality analysis timed out; deterministic evidence was retained."
            )
        );
    if (unavailable)
        warnings.push(
            warning(
                ...
                "Optional materiality analysis was unavailable; deterministic evidence was retained."
            )
        );
    if (settled.some(result => result.status === "rejected")) {
        warnings.push(
            warning(
                ...
                "Optional materiality analysis was partial; failed groups used deterministic fallback."
            )
        );
    }

    const successByGroup = new Map(
        successes.map(success => [success.analyzed.group.groupId, success])
    );
    ...
    const audit: DocumentChangeAuditSnapshot = {
        ...
        groups: deterministic.audit.groups.map(group => ({
            ...
            evidenceSourceId: selectedIds.has(group.groupId)
                ? documentChangeGroupSourceId(
                      deterministic.analyzedGroups.find(
                          candidate => candidate.group.groupId === group.groupId
                      )!.group
                  )
                : null,
            ...(successByGroup.has(group.groupId)
                ? { analysis: analysisSnapshot(successByGroup.get(group.groupId)!) }
                : {}),
        })),
    };
    ...
    const materialCount = successes.filter(
        success => success.result.disposition === "material"
    ).length;
    const nonMaterialCount = successes.filter(
        success => success.result.disposition === "non_material"
    ).length;
    const uncertainCount = successes.filter(
        success => success.result.disposition === "uncertain"
    ).length;
    return {
        ...deterministic,
        selectedGroups,
        ...
        diagnostics: {
            ...
            selectedGroupsByCategory: Object.fromEntries(
                DOCUMENT_CHANGE_CATEGORIES.map(category => [
                    category,
                    selectedGroups.filter(group => group.materiality.category === category).length,
                ])
            ) as Record<DocumentChangeCategory, number>,
            truncatedGroupsByCategory: Object.fromEntries(
                DOCUMENT_CHANGE_CATEGORIES.map(category => [
                    category,
                    effectiveGroups.filter(
                        group =>
                            !selectedIds.has(group.group.groupId) &&
                            group.materiality.category === category
                    ).length,
                ])
            ) as Record<DocumentChangeCategory, number>,
