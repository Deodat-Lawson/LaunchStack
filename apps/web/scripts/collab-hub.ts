...
async function main(): Promise<void> {
    ...
    const spec = process.env.COLLAB_MEETING
        ? (JSON.parse(process.env.COLLAB_MEETING) as MeetingSpec)
        : null;
    if (spec) {
        ...
        console.log(
            `[collab-hub] meeting ${JSON.stringify({ meetingId: config.id, channelId: channel.id })}`
        );
    }
