...
describeDb("explicit document version chunks", () => {
    it("loads only owned explicit-version chunks in deterministic order", async () => {
        ...
        try {
            ...
            expect(loaded.get(key(v2!.id))).toMatchObject({ state: "partial", chunks: [{ content: "page one" }, { content: "page two" }] });
            expect(loaded.get(key(v1!.id))).toMatchObject({ state: "partial", chunks: [{ content: "old" }] });
            expect(loaded.get(key(v3!.id))).toEqual({ state: "missing", chunks: [], warnings: ["version_chunks_missing"] });
            expect(loaded.get(key(otherVersion!.id))).toEqual({ state: "missing", chunks: [], warnings: ["document_version_not_accessible"] });
            expect(loaded.get(key(999_999))).toEqual({ state: "missing", chunks: [], warnings: ["document_version_not_accessible"] });
            ...
            expect(otherCompanyView.get(key(v2!.id))).toEqual({ state: "missing", chunks: [], warnings: ["document_version_not_accessible"] });
    ...
    it("loads in-period versions plus exactly one predecessor per document", async () => {
        ...
        try {
            ...
            const [doc] = await test.db.insert(document).values({ companyId: BigInt(owner!.id), url: "local://doc", category: "Product", title: "Plan" }).returning();
            const [untouched] = await test.db.insert(document).values({ companyId: BigInt(owner!.id), url: "local://quiet", category: "Product", title: "Quiet" }).returning();
            ...
            const insertVersion = (documentId: bigint, versionNumber: number, iso: string) =>
                test.db.insert(documentVersions).values({
                    documentId, versionNumber, url: `local://v${versionNumber}`, mimeType: "text/plain", createdAt: at(iso),
                }).returning();

            // Deep history before the window: only the newest may be loaded.
            await insertVersion(BigInt(doc!.id), 1, "2026-01-01T00:00:00.000Z");
            const [predecessor] = await insertVersion(BigInt(doc!.id), 2, "2026-01-20T00:00:00.000Z");
