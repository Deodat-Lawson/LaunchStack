...
const STOPWORDS = new Set([
  ...
  "we", "i", "you", "it", "is", "are", "be", "this", "that", "our", "your",
  "at", "as", "by", "from", "if", "so", "not", "can", "will", "would", "should",
  "have", "has", "had", "do", "does", "did", "was", "were", "been", "they",
]);
...
function scoreRedundancy(chat: ChannelMessage[]): MeetingEvalDimension {
  if (chat.length < 2) {
    return { id: "redundancy", label: "Non-repetition", score: 1, weight: 1, detail: "too short to repeat" };
  }
...
function scoreTermination(config: MeetingConfig, state: MeetingState): MeetingEvalDimension {
  if (state.status === "failed") {
    return { id: "termination", label: "Clean ending", score: 0, weight: 1.5, detail: state.error ?? "failed" };
  }
  ...
  return {
    ...
    score: hitCap ? 0.4 : 1,
    weight: 1.5,
    detail: hitCap ? `ran to the ${config.maxTurns}-turn cap` : `closed after ${state.turnIndex} turns`,
  };
}
...
function scoreHandoff(transcript: ChannelMessage[]): MeetingEvalDimension {
  ...
  for (const message of transcript) {
    ...
    if (underHumanControl && message.author.kind === "agent" && message.author.id !== "system") {
      violations++;
    }
  }
...
export function summarizeMeetingEvals(results: MeetingEvalResult[]): MeetingEvalSuiteReport {
  ...
  return {
    ...
    meanScore:
      results.length === 0
        ? 0
        : Math.round((results.reduce((s, r) => s + r.overall, 0) / results.length) * 1000) / 1000,
