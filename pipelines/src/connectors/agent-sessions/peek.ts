...
import type { DiscoveredKnowledgeItem } from "../types";
import { isHarnessUserText } from "./parse-claude";
import { extractCodexText } from "./parse-codex";
import type { SessionToolId } from "./types";
...
const EMPTY_PEEK: SessionPeek = { title: null, preview: null, projectPath: null, gitBranch: null };
 ...
 */
async function readWindows(absolutePath: string): Promise<{ head: string; tail: string } | null> {
    let handle;
    try {
...
function windowLines(chunk: string, edge: "head" | "tail"): string[] {
    if (chunk.length === 0) return [];
    const lines = chunk.split("\n");
    if (edge === "head") lines.pop();
    else lines.shift();
...
function claudeUserText(record: Record<string, unknown>): string | null {
    ...
    for (const block of content) {
        ...
        if (asString(blockRecord?.type) !== "text") continue;
        const text = asString(blockRecord?.text);
        if (text && !isHarnessUserText(text)) return text.trim();
    }
    return null;
}

function peekClaude(head: string, tail: string): SessionPeek {
...
function peekCodex(head: string): SessionPeek {
    let preview: string | null = null;
    let projectPath: string | null = null;
...
export async function peekSessionFile(
    absolutePath: string,
    tool: SessionToolId
): Promise<SessionPeek> {
...
export async function peekSessions(
    items: readonly DiscoveredKnowledgeItem[],
