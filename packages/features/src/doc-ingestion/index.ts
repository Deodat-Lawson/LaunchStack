...
import type { DocumentChunk, PageContent, PipelineResult } from "@launchstack/core/ocr/types";
...
async function maybeSyncToNeo4j(
    ...
    await runStep("step-g-neo4j-sync", async () => {
        ...
        const { isNeo4jConfigured, checkNeo4jHealth } = await import("@launchstack/core/graph");
...
export async function runDocIngestionTool(
    ...
    try {
        ...
        // -----------------------------------------------------------------------
        await maybeExtractEntities(sidecarUrl, storedSections, documentId, companyId, runStep);
