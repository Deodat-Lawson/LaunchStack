...
describe("collab request signing", () => {
  ...
  it("is case-insensitive about header casing", () => {
    ...
    expect(verifyRequest({ secret: SECRET, method: "POST", path: PATH, body: BODY, headers: upper }).ok).toBe(true);
  });
  ...
  it("rejects a tampered path, including its query string", () => {
    ...
    const headers = signRequest({ secret: SECRET, method: "GET", path: signedPath, body: "", nodeId: "worker-a" });
  ...
  it("rejects a tampered method", () => {
    expect(
      verifyRequest({ secret: SECRET, method: "DELETE", path: PATH, body: BODY, headers: sign() }).reason,
    ).toBe("bad_signature");
  });

  it("rejects a stale timestamp outside the skew window", () => {
    const headers = sign({ timestamp: Date.now() - 10 * 60_000 });
    expect(verifyRequest({ secret: SECRET, method: "POST", path: PATH, body: BODY, headers }).reason).toBe(
      "stale_timestamp",
    );
  });
  ...
  it("does not consume a nonce when the signature fails", () => {
    ...
    expect(
      verifyRequest({ secret: "wrong", method: "POST", path: PATH, body: BODY, headers, nonces }).reason,
    ).toBe("bad_signature");
    // The nonce is still available to the legitimate holder.
    expect(verifyRequest({ secret: SECRET, method: "POST", path: PATH, body: BODY, headers, nonces }).ok).toBe(true);
  });
  ...
  it("rejects missing headers", () => {
    ...
    expect(verifyRequest({ secret: SECRET, method: "POST", path: PATH, body: BODY, headers: partial }).reason).toBe(
      "missing_headers",
    );
  });
