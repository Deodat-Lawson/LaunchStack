...
import { FounderWeeklyReviewEvidenceSnapshotSchema, FounderWeeklyReviewRepository, FounderWeeklyReviewWorkerService } from "@launchstack/features/founder-weekly-review";
...
describeDb("Founder Weekly Review async evidence workflow", () => {
  it("creates without a snapshot, attaches it once, and then permits generation claim", async () => {
    ...
    try {
      const [companyRow] = await test.db.insert(company).values({ name: "Workflow", numberOfEmployees: "1" }).returning();
      const actor = { externalUserId: "u", internalUserId: 1n, companyId: BigInt(companyRow!.id), role: "owner" };
      const created = await createFounderWeeklyReviewDispatchService(test.db).createRunWithDispatch({ actor, requestKey: "workflow", reportingPeriod: snapshot.reportingPeriod, collectionInput: { workspaceTimezone: "UTC", founderContext: "bounded", actorExternalUserId: "u" } });
      ...
      const context = { companyId: actor.companyId, runId: created.run.id, collectionClaimId: "collection-1" };
      ...
      await expect(worker.attachEvidenceSnapshotIfAbsent(context, snapshot)).resolves.toMatchObject({ id: created.run.id });
      const generating = await worker.claimQueuedRun({ companyId: actor.companyId, runId: created.run.id, generationClaimId: "generation-1", generationJobId: "job-1" });
  ...
  it("retries collection failures without creating a snapshot or another run", async () => {
    const test = await createFounderWeeklyReviewTestDatabase();
    try {
      const [companyRow] = await test.db.insert(company).values({ name: "Recovery", numberOfEmployees: "1" }).returning();
      const actor = { externalUserId: "u", internalUserId: 1n, companyId: BigInt(companyRow!.id), role: "owner" };
      const service = createFounderWeeklyReviewDispatchService(test.db);
      const created = await service.createRunWithDispatch({ actor, requestKey: "collection-failure", reportingPeriod: snapshot.reportingPeriod, collectionInput: { workspaceTimezone: "UTC", actorExternalUserId: "u" } });
      const worker = new FounderWeeklyReviewWorkerService(new FounderWeeklyReviewRepository(test.db));
      const collecting = await worker.claimEvidenceCollection({ companyId: actor.companyId, runId: created.run.id, collectionClaimId: "collection-failure-claim" });
      expect(collecting.status).toBe("collecting");
      const failed = await worker.markCollectionFailed({ companyId: actor.companyId, runId: created.run.id, collectionClaimId: "collection-failure-claim" }, { errorCode: "evidence_collection_failed" });
      expect(failed.status).toBe("failed"); expect(failed.evidenceSnapshot).toBeNull();
