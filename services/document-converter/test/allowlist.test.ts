...
import { postJson, forbiddenFetch, startApp, testConfig, type TestServer } from "./helpers.js";
...
describe("URL origin allowlist (SSRF hardening)", () => {
    ...
    async function expectUrlNotAllowed(route: "/route" | "/convert", url: string): Promise<void> {
        const body =
            route === "/route" ? { schemaVersion: 1, documentUrl: url } : { schemaVersion: 1, url };
    ...
    it("rejects the cloud metadata endpoint", async () => {
        await expectUrlNotAllowed("/route", "http://169.254.169.254/latest/meta-data/");
        await expectUrlNotAllowed("/convert", "http://169.254.169.254/latest/meta-data/");
    });
