...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const [newMemory] = await db.insert(agentAiChatbotMemory).values(insertValues).returning();
    ...
    try {
        ...
        const whereConditions = memoryType
            ? and(
                  ...
                  eq(
                      agentAiChatbotMemory.memoryType,
                      memoryType as "short_term" | "long_term" | "working" | "episodic"
                  )
              )
            : eq(agentAiChatbotMemory.chatId, chatId);
        ...
        const memories = await db
            ...
            .orderBy(desc(agentAiChatbotMemory.importance), desc(agentAiChatbotMemory.accessedAt));
