...
describe("detectConflicts", () => {
    ...
    it("detects a cross-source conflict on distinct normalized values", () => {
        ...
        expect(conflicts[0]!.values.map(v => v.value)).toEqual(["berlin", "munich"]);
...
describe("resolveByRecency", () => {
    const conflictOf = (...assertions: EvidenceAssertion[]) =>
        detectConflicts(assertions, {
            currentVersionIds: new Set(assertions.map(a => a.anchor.sourceVersionId)),
        })[0]!;
    ...
    it("picks the newest observedAt and preserves all alternatives", () => {
        ...
        const { winner, alternatives } = resolveByRecency(conflictOf(older, newest, middle));
    ...
    it("breaks observedAt ties by higher sourceVersionId", () => {
        ...
        expect(resolveByRecency(conflictOf(lowVersion, highVersion)).winner).toBe(highVersion);
        expect(resolveByRecency(conflictOf(highVersion, lowVersion)).winner).toBe(highVersion);
    });
