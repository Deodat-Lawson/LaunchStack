...
export async function packDigest(view, bundle, maxChars) {
    ...
    const seen = new Set();
    const push = (path) => {
        if (seen.has(path) || denied.has(path) || !existing.has(path))
            return;
        seen.add(path);
        ordered.push(path);
    };
    for (const memory of bundle.memoryFiles)
        push(memory.path);
    for (const entry of bundle.map.entries)
        push(entry.path);
    ...
    for (const path of ordered) {
        ...
        if (content === null)
            continue; // binary/oversized — not truncation
