...
describe("POST /api/legal/apply-edits", () => {
    ...
    describe("Adeu Integration", () => {
        ...
        it("converts Blob result to base64", async () => {
            ...
            expect(json.success).toBe(true);
            expect(json.modifiedDocxBase64).toBe(Buffer.from(modifiedContent).toString("base64"));
        });
    ...
    describe("Error Handling", () => {
        ...
        it("returns 500 when Adeu service throws error", async () => {
            mockReadDocx.mockResolvedValue({ text: "a some document" } as any);
            mockProcessDocumentBatch.mockRejectedValueOnce(new Error("Adeu service unavailable"));
        ...
        it("handles network errors gracefully", async () => {
            mockReadDocx.mockResolvedValue({ text: "a some document" } as any);
            mockProcessDocumentBatch.mockRejectedValueOnce(new Error("ECONNREFUSED"));
