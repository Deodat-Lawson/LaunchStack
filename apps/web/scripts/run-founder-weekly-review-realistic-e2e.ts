...
function canonicalize(value: unknown): unknown {
    ...
    throw new Error("Cannot canonicalize snapshot.");
}
...
async function writeAtomic(path: string, body: string) {
    try {
        await access(path);
        throw new Error("Refusing to overwrite export.");
    } catch (error) {
...
function assertComputedSnapshot(
    snapshot: unknown,
    ids: Record<string, bigint | number>,
    texts: ComputedTexts
) {
    ...
    if (
        ...
        change.excerpt.includes(texts.v3) ||
        change.excerpt.startsWith("Version ")
    )
        throw new Error("Computed document-change assertions failed.");
    if (
        workspaceItem.metadata.documentVersionId !== String(ids.b2) ||
        workspaceItem.metadata.chunkId !== ids.bCurrentChunk ||
        workspaceItem.metadata.retrievalReason !== "founder_context_relevance" ||
        typeof workspaceItem.metadata.similarityScore !== "number" ||
        "sourceTimestamp" in workspaceItem
    )
        throw new Error("Strict workspace-document assertions failed.");
    if (
        items.some(item =>
            [texts.v3, texts.bHistorical, texts.nullVersion, texts.foreign, texts.unrelated].some(
                value => item.excerpt.includes(value)
            )
        )
    )
        throw new Error("Computed control evidence leaked into the snapshot.");
    if (new Set(items.map(item => item.sourceId)).size !== items.length)
        throw new Error("Computed snapshot contains duplicate source IDs.");
    return {
        parsed,
        counts: Object.fromEntries(
            ["document_change", "workspace_document", "customer_feedback", "founder_context"].map(
                type => [type, items.filter(item => item.sourceType === type).length]
            )
        ),
    };
}
function assertComputedReport(
    payload: any,
    snapshot: ReturnType<typeof FounderWeeklyReviewEvidenceSnapshotSchema.parse>
) {
    ...
    const citedTypes = (section: any) =>
        section.state === "evidence"
            ? section.items.flatMap((item: any) =>
                  item.sourceIds.map((id: string) => sourceTypeById.get(id))
              )
            : [];
    const changed = payload.sections.whatChanged;
    if (changed.state !== "evidence" || !citedTypes(changed).includes("document_change"))
        throw new Error(
            "Generated report did not cite the computed document change in whatChanged."
        );
    const shipped = payload.sections.whatShipped;
    if (
        shipped.state === "evidence" &&
        citedTypes(shipped).length > 0 &&
        citedTypes(shipped).every((type: unknown) => type === "workspace_document")
    )
        throw new Error("Generated report used workspace evidence alone for whatShipped.");
    if (
        citedTypes(payload.sections.whatCustomersSaid).some(
            (type: unknown) => type !== "customer_feedback"
        )
    )
        throw new Error("Generated report violated customer-feedback source semantics.");
    if (
        payload.sections.currentBlockers.state === "evidence" &&
        !citedTypes(payload.sections.currentBlockers).includes("workspace_document")
    )
        throw new Error("Generated report did not use current workspace context for blockers.");
}

if (process.env.SYNTHETIC_FWR_LOCAL !== "1" || process.env.NODE_ENV === "production")
    throw new Error("Refusing realistic E2E outside explicit local mode.");
...
try {
    ...
    const texts: ComputedTexts = {
        ...
        after: "Platform owns retry telemetry and recovery monitoring.",
        v3: "v3 future-only operational note",
        bHistorical: "historical workspace reliability text",
        nullVersion: "null-version workspace reliability text",
        foreign: "foreign workspace reliability text",
        unrelated: "unrelated current cafeteria menu",
    };
    ...
    if (mode === "legacy") {
        ...
        const [b] = await testDb.db
            ...
            .returning();
        const [feedbackDoc] = await testDb.db
            .insert(document)
            .values({
                companyId: actor.companyId,
                url: "local://feedback",
        ...
        const [a1] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(a!.id),
                versionNumber: 1,
                url: "local://a/v1",
                mimeType: "text/plain",
                changelog: "Initial ownership.",
        ...
        const [a2] = await testDb.db
            ...
            .values({
                ...
                url: "local://a/v2",
                mimeType: "text/plain",
                changelog: "Platform ownership was updated.",
                createdAt: new Date("2026-02-20T10:00:00.000Z"),
            })
            .returning();
        const [a3] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(a!.id),
                versionNumber: 3,
            ...
            .returning();
        const [b1] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(b!.id),
                versionNumber: 1,
                url: "local://b/v1",
                mimeType: "text/plain",
        ...
        const [b2] = await testDb.db
            ...
            .returning();
        const [feedbackVersion] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(feedbackDoc!.id),
                versionNumber: 1,
                url: "local://feedback/v1",
                mimeType: "text/plain",
                createdAt: new Date("2026-02-22T12:00:00.000Z"),
            })
            .returning();
        const [unrelatedVersion] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(unrelated!.id),
                versionNumber: 1,
                url: "local://unrelated/v1",
                mimeType: "text/plain",
                createdAt: new Date("2026-03-02T10:00:00.000Z"),
            })
            .returning();
        const [foreignVersion] = await testDb.db
            .insert(documentVersions)
            .values({
                documentId: BigInt(foreign!.id),
                versionNumber: 1,
                url: "local://foreign/v1",
                mimeType: "text/plain",
                createdAt: new Date("2026-03-02T10:00:00.000Z"),
            })
            .returning();
        ...
        await testDb.db
            .update(document)
            .set({ currentVersionId: BigInt(a3!.id) })
            .where(eq(document.id, a!.id));
        await testDb.db
            .update(document)
            .set({ currentVersionId: BigInt(b2!.id) })
            .where(eq(document.id, b!.id));
        await testDb.db
            .update(document)
            .set({ currentVersionId: BigInt(unrelatedVersion!.id) })
            .where(eq(document.id, unrelated!.id));
        await testDb.db
            .update(document)
            .set({ currentVersionId: BigInt(foreignVersion!.id) })
            .where(eq(document.id, foreign!.id));
            ...
            .values([
                {
                    documentId: BigInt(a!.id),
                    versionId: BigInt(a1!.id),
                    structureId: BigInt(s1!.id),
                    content: texts.before,
                    contentHash: "a".repeat(64),
                    tokenCount: 4,
                    charCount: texts.before.length,
                    pageNumber: 1,
                    lineStart: 1,
                    lineEnd: 1,
                },
                {
                    documentId: BigInt(a!.id),
                    versionId: BigInt(a2!.id),
                    structureId: BigInt(s2!.id),
                    content: texts.after,
                    contentHash: "b".repeat(64),
                    tokenCount: 7,
                    charCount: texts.after.length,
                    pageNumber: 1,
                    lineStart: 1,
                    lineEnd: 1,
                },
                {
                    documentId: BigInt(a!.id),
                    versionId: BigInt(a3!.id),
                    content: texts.v3,
                    contentHash: "c".repeat(64),
                    tokenCount: 4,
                    charCount: texts.v3.length,
                },
                {
                    documentId: BigInt(b!.id),
                    versionId: BigInt(b1!.id),
                    content: texts.bHistorical,
                    contentHash: "d".repeat(64),
                    tokenCount: 4,
                    charCount: texts.bHistorical.length,
                    embedding: vectorSql(0),
                },
                {
                    documentId: BigInt(b!.id),
                    versionId: BigInt(b2!.id),
                    content:
                        "Onboarding reliability and retry monitoring are blocking enterprise expansion.",
                ...
                },
                {
                    documentId: BigInt(b!.id),
                    versionId: null,
                    content: texts.nullVersion,
                    contentHash: "f".repeat(64),
                    tokenCount: 4,
                    charCount: texts.nullVersion.length,
                    embedding: vectorSql(0),
                },
                {
                    documentId: BigInt(feedbackDoc!.id),
                    versionId: BigInt(feedbackVersion!.id),
                    content:
                        "Enterprise buyers need reliable onboarding recovery before expansion.",
                ...
                {
                    documentId: BigInt(unrelated!.id),
                    versionId: BigInt(unrelatedVersion!.id),
                    content: texts.unrelated,
                    contentHash: "2".repeat(64),
                    tokenCount: 4,
                    charCount: texts.unrelated.length,
                },
                {
                    documentId: BigInt(foreign!.id),
                    versionId: BigInt(foreignVersion!.id),
                    content: texts.foreign,
                    contentHash: "3".repeat(64),
                    tokenCount: 4,
                    charCount: texts.foreign.length,
                    embedding: vectorSql(0),
                },
            ])
            .returning();
        const bCurrentChunk = inserted.find(
            chunk => chunk.documentId === BigInt(b!.id) && chunk.versionId === BigInt(b2!.id)
        )!;
        computedIds = {
            a: BigInt(a!.id),
            a1: a1!.id,
            ...
            b: BigInt(b!.id),
            b2: BigInt(b2!.id),
            bCurrentChunk: bCurrentChunk.id,
        };
        collector = new FounderWeeklyReviewEvidenceService(
            testDb.db,
    ...
    const created = await dispatchService.createRunWithDispatch({
        actor,
        requestKey: `realistic-${mode}-${randomUUID()}`,
        reportingPeriod: fixture.reportingPeriod,
        collectionInput: {
            workspaceTimezone: fixture.workspaceTimezone,
            founderContext:
                mode === "computed"
                    ? "Assess whether onboarding reliability and retry monitoring are blocking enterprise expansion."
                    : fixture.founderContext,
            actorExternalUserId: actor.externalUserId,
        },
    });
    if (created.run.evidenceSnapshot)
        throw new Error("Workflow run unexpectedly has an initial snapshot.");
    ...
    const checked =
        mode === "computed"
            ? assertComputedSnapshot(attached.evidenceSnapshot, computedIds!, texts)
            : {
                  parsed: attached.evidenceSnapshot!,
                  counts: Object.fromEntries(
                      ["document_change", "customer_feedback", "founder_context"].map(type => [
                          type,
                          attached.evidenceSnapshot!.items.filter(item => item.sourceType === type)
                              .length,
                      ])
                  ),
              };
    if (
        attached.status !== "queued" ||
        beforeDigest !== afterDigest ||
        !checked.counts.document_change ||
        !checked.counts.customer_feedback ||
        checked.counts.founder_context !== 1
    )
        throw new Error("Realistic collector assertions failed.");
    ...
    if (!generating.evidenceSnapshot) throw new Error("Generation began without snapshot.");
    let generationCalls = 0;
    const generated = await generateFounderWeeklyReview({
        evidenceSnapshot: generating.evidenceSnapshot,
        generate: async request => {
            generationCalls++;
            return generateFounderWeeklyReviewStructured(request);
        },
    });
    validateFounderWeeklyReviewV2Citations(
        generated.reviewPayload as never,
        generating.evidenceSnapshot
    );
    ...
    if (
        ...
        digest(readBack.evidenceSnapshot) !== beforeDigest
    )
    ...
    const dispatchRows = await testDb.db
        ...
        .where(eq(founderWeeklyReviewDispatches.runId, saved.id));
    ...
    if (process.env.SYNTHETIC_FWR_EXPORT_REPORT === "1") {
        const exportRoot = founderWeeklyReviewRealisticExportRoot(
            mode,
            process.env.SYNTHETIC_FWR_EXPORT_DIR
        );
        const directory = resolve(process.cwd(), exportRoot, saved.id);
            ...
            JSON.stringify(
                {
                    ...
                    evidenceCounts: checked.counts,
                    warningCodes: readBack.evidenceSnapshot.sourceWarnings.map(
    ...
    console.log(
        JSON.stringify({
            ...
            evidenceCounts: checked.counts,
            warningCodes: readBack.evidenceSnapshot.sourceWarnings.map(warning => warning.code),
            snapshotDigestUnchanged: beforeDigest === digest(readBack.evidenceSnapshot),
            validation: { canonicalSchema: true, citations: true, sourceSemantics: true },
            provider: generated.modelMetadata.provider,
            model: generated.modelMetadata.model,
            repairCount: generationCalls - 1,
