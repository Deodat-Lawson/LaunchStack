...
export class MeetingHub {
    ...
    async dispatchTurn(request: AgentTurnRequest): Promise<AgentTurnResult> {
        ...
        const promise = new Promise<AgentTurnResult>((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(
                    new Error(`Turn timed out after ${this.turnTimeoutMs}ms on node "${nodeId}"`)
                );
            }, this.turnTimeoutMs);
    ...
    async handle(request: HubHttpRequest): Promise<HubHttpResponse> {
        ...
        if (!verified.ok) {
            ...
            const code =
                verified.reason === "unsupported_version" ? "unsupported_version" : "unauthorized";
    ...
    private async route(
        ...
        const rest = path.startsWith(COLLAB_API_PREFIX)
            ? path.slice(COLLAB_API_PREFIX.length)
            : null;
        ...
        if (method === "POST" && segments[0] === "nodes" && segments[1] === "register") {
            ...
            if (payload.protocolVersion && payload.protocolVersion !== COLLAB_PROTOCOL_VERSION) {
                return errorResponse(
                    426,
                    "unsupported_version",
                    `Hub speaks v${COLLAB_PROTOCOL_VERSION}`
                );
            }
        ...
        if (method === "POST" && segments[0] === "nodes" && segments[1] === "poll") {
            ...
            if (events === null)
                return errorResponse(
                    404,
                    "not_found",
                    `Node "${payload.nodeId}" is not registered`
                );
        ...
        if (method === "POST" && segments[0] === "turns" && segments[2] === "result") {
            ...
            if (!payload || !requestId)
                return errorResponse(400, "bad_request", "requestId required");
        ...
        if (segments[0] === "channels" && segments[1] && segments[2] === "messages") {
            ...
            if (method === "POST") {
                ...
                if (!channel)
                    return errorResponse(404, "not_found", `Unknown channel ${channelId}`);
