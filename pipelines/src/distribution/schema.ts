...
import { PARTNER_KINDS, RELATIONSHIP_STAGES, RUN_STATUSES } from "./types";

// ─── Programs ────────────────────────────────────────────────────────────────

export const distributionPrograms = pgTable(
    "distribution_programs",
    {
        ...
        hsCodes: jsonb("hs_codes").$type<string[]>().notNull().default([]),
        targetTerritories: jsonb("target_territories").$type<Territory[]>().notNull().default([]),
        partnerKinds: jsonb("partner_kinds").$type<string[]>().notNull().default([]),
        constraints: text("constraints"),
        knownPartnerDomains: jsonb("known_partner_domains").$type<string[]>().notNull().default([]),
...
export const distributionRuns = pgTable(
    ...
    table => ({
        ...
        companyStatusIdx: index("distribution_runs_company_status_idx").on(
            table.companyId,
            table.status
        ),
    })
...
export const partnerOrgs = pgTable(
    ...
    table => ({
        resolveKeyUnique: uniqueIndex("partner_orgs_company_resolve_key_unique").on(
            table.companyId,
            table.resolveKey
        ),
...
export const partnerEvidence = pgTable(
    "partner_evidence",
    {
...
export const partnerRelationships = pgTable(
    "partner_relationships",
    {
        ...
        territory: jsonb("territory").$type<Territory>(),
        stage: varchar("stage", { length: 32, enum: RELATIONSHIP_STAGES })
            .notNull()
            .default("candidate"),
        fitScore: integer("fit_score"),
        fitRationale: text("fit_rationale"),
        fitBreakdown: jsonb("fit_breakdown").$type<FitBreakdown>(),
        riskFlags: jsonb("risk_flags").$type<string[]>().notNull().default([]),
        screening: jsonb("screening").$type<ScreeningState>(),
        dossier: jsonb("dossier").$type<Dossier>(),
    ...
    table => ({
        programOrgKindUnique: uniqueIndex("partner_relationships_program_org_kind_unique").on(
            table.companyId,
            table.programId,
            table.orgId,
            table.kind
...
export const distributionAgreements = pgTable(
...
export const marketEdges = pgTable(
    "market_edges",
    {
        id: bigserial("id", { mode: "number" }).primaryKey(),
        companyId: bigint("company_id", { mode: "bigint" })
