...
import { appendTemplateVersion, createCampaign, requireCampaign, setCampaignStatus, upsertRecipients, } from "./db.js";
...
export async function prepareEmailCampaign(args) {
    ...
    if (templateIssues.length > 0) {
        throw new CampaignLifecycleError(`Template rejected: ${templateIssues.map(i => i.message).join(" ")}`, "template_invalid", 422);
    }
    let review = null;
    if (!args.skipReview) {
    ...
    const status = review?.verdict === "revise" ? "needs_revision" : review ? "pending_approval" : "draft";
...
function requireName(name) {
    ...
    if (!trimmed) {
        throw new CampaignLifecycleError("A campaign name is required to create a campaign", "campaign_name_required", 400);
    }
    return trimmed;
}
/**
