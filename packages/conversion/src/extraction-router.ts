...
import { findAdapter } from "./document-converter/converters";
import {
    detectSourceType,
...
import type { NormalizedDocument } from "./ocr-processing/types";
...
export async function ingestDocument(
    ...
    if (isUrl && !adapter.needsUrl) {
        ...
        if (!res.ok) {
            throw new Error(
                `[IngestionRouter] Failed to fetch document: ${res.status} ${res.statusText}`
            );
        }
...
export async function ingestToNormalized(
    ...
    console.log(
        `[IngestionRouter] ingestToNormalized: converting to legacy NormalizedDocument format`
    );
