...
describe("applyEditsAsMarkdown", () => {
    ...
    it("throws on 422", async () => {
        ...
        await expect(applyEditsAsMarkdown(makeDocxBuffer(), params)).rejects.toThrow(
            AdeuServiceError
        );
    });
...
describe("diffDocxFiles", () => {
    ...
    it("returns has_differences: true for different documents", async () => {
        mockFetch.mockResolvedValueOnce(
            jsonResponse({ diff: "change here", has_differences: true })
        );

        const result = await diffDocxFiles(Buffer.from("original"), Buffer.from("modified"));
    ...
    it("works with Blob inputs", async () => {
        ...
        const result = await diffDocxFiles(new Blob(["original"]), new Blob(["modified"]));
        expect(result.has_differences).toBe(false);
    });
});
