...
const isMeaningful = (line) => line !== "" && !line.startsWith("%%");
/* ──────────────────────────────────────────────────────────────
...
function findImbalance(text) {
    ...
    const open = [];
    if (round > 0)
        open.push(`${round} unclosed "("`);
    if (square > 0)
        open.push(`${square} unclosed "["`);
    if (curly > 0)
        open.push(`${curly} unclosed "{"`);
...
const FLOW_SKIP = /^(?:subgraph\b|end\b|direction\b|classDef\b|class\b|style\b|linkStyle\b|click\b)/;
 ...
 */
function countFlowchartNodes(bodyLines) {
    const ids = new Set();
    for (const raw of bodyLines) {
        const line = raw.trim();
        if (!isMeaningful(line) || FLOW_SKIP.test(line))
            continue;
...
function countSequenceNodes(bodyLines) {
    const ids = new Set();
    for (const raw of bodyLines) {
        const line = raw.trim();
...
function countClassNodes(bodyLines) {
    const ids = new Set();
    for (const raw of bodyLines) {
        const line = raw.trim();
        if (!isMeaningful(line))
...
function countErNodes(bodyLines) {
    const ids = new Set();
    for (const raw of bodyLines) {
        const line = raw.trim();
        if (!isMeaningful(line))
...
export function lintMermaid(code) {
    ...
    if (kind === "unknown") {
        errors.push(`unknown-kind: "${kindLine.slice(0, 60)}" does not declare a supported diagram type ` +
    ...
    const balanceText = kind === "erDiagram" ? text.replace(/[|}o]{1,2}(?:--|\.\.)[|{o]{1,2}/g, "--") : text;
...
//# sourceMappingURL=mermaid-lint.js.map