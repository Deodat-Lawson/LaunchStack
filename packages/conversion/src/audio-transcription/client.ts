 ...
 */
import { transcribeResponseSchema, type TranscribeResponse } from "./wire";
import {
    videoTranscribeResponseSchema,
    type VideoTranscribeRequest,
    type VideoTranscribeResponse,
} from "../video-transcription/wire";
import type { TranscriptionPort } from "../ports";

import { ComputeServiceError, postJson, type ServiceClientConfig } from "../service-client";
...
export class HttpTranscriptionClient implements TranscriptionPort {
    ...
    async transcribeFile(input: {
        ...
        if (!response.ok) {
            throw new ComputeServiceError(`transcription /transcribe returned ${response.status}`, {
                service: "transcription",
                path: "/transcribe",
