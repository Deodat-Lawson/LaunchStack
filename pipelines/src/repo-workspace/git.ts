...
import type { EnsureMirrorResult, FetchResult, GitPort } from "./types";
...
export const defaultExec: ExecFn = (file, args, cwd) =>
    new Promise(resolve => {
        execFile(
            file,
            args,
            { cwd, timeout: EXEC_TIMEOUT_MS, maxBuffer: MAX_BUFFER },
            (error, stdout, stderr) => {
                let code = 0;
                if (error) {
...
function redact(text: string, token: string | null | undefined): string {
    if (!token) return text;
    return text.split(token).join("[redacted]");
}

function authArgs(token: string | null | undefined): string[] {
    if (!token) return [];
...
export function createGitPort(exec: ExecFn = defaultExec): GitPort {
    const run = async (
        operation: string,
        args: string[],
        options?: { cwd?: string; token?: string | null; allowFailure?: boolean }
    ): Promise<ExecResult> => {
    ...
    const resolveHead = async (mirrorPath: string): Promise<string> => {
    ...
    return {
        async ensureMirror({ remoteUrl, mirrorPath, token }): Promise<EnsureMirrorResult> {
        ...
        async fetchMirror({ mirrorPath, token }): Promise<FetchResult> {
            const previousSha = await resolveHead(mirrorPath).catch(() => null);
            await run(
                "fetch",
                [...authArgs(token), "--git-dir", mirrorPath, "fetch", "--prune", "origin"],
                { token }
            );
            ...
            if (advanced && previousSha) {
                const ancestry = await run(
                    "merge-base",
                    ["--git-dir", mirrorPath, "merge-base", "--is-ancestor", previousSha, headSha],
                    { allowFailure: true }
                );
        ...
        async addWorktree({ mirrorPath, sha, worktreePath, token }): Promise<void> {
            if (!/^[0-9a-f]{40,64}$/.test(sha)) {
        ...
        async removeWorktree({ mirrorPath, worktreePath }): Promise<void> {
