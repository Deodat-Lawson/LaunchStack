...
import { BrandVoiceSchema } from "./types.js";
export async function extractBrandVoice(args) {
    ...
    const results = await getRag().companyEnsembleSearch("company tone voice communication style brand personality writing examples", options);
    ...
    const toneHint = toneOverride
        ...
        : "";
    const response = await invokeMarketingStructured(BrandVoiceSchema, [
        new SystemMessage(`You are a brand voice analyst. Given text samples from a company's documents, synthesize a BrandVoice profile that captures how this company communicates.
