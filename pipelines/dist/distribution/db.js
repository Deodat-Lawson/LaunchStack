...
import { distributionAgreements, distributionPrograms, distributionRuns, partnerEvidence, partnerOrgs, partnerRelationships, relationshipEvents, } from "./schema.js";
import { assertTransition, isPastCandidate, STALE_AFTER_DAYS, STAGE_ORDER } from "./stages.js";
// ─── Mappers ─────────────────────────────────────────────────────────────────
...
export async function createProgram(args) {
    ...
    const [row] = await db
        ...
        .returning();
    if (!row)
        throw new Error("Failed to create distribution program");
    return toProgram(row);
}
export async function listPrograms(companyId) {
    const db = getDb();
    const rows = await db
        .select()
...
export async function getProgram(id, companyId) {
    const db = getDb();
    const [row] = await db
        .select()
        .from(distributionPrograms)
...
export async function updateProgram(id, companyId, patch) {
    const db = getDb();
    const values = {};
    if (patch.name !== undefined)
        values.name = patch.name;
    if (patch.offering !== undefined)
        values.offering = patch.offering;
    if (patch.categories !== undefined)
        values.categories = patch.categories;
    if (patch.hsCodes !== undefined)
        values.hsCodes = patch.hsCodes;
    if (patch.targetTerritories !== undefined)
        values.targetTerritories = patch.targetTerritories;
    if (patch.partnerKinds !== undefined)
        values.partnerKinds = patch.partnerKinds;
    if (patch.constraints !== undefined)
        values.constraints = patch.constraints ?? null;
    ...
    if (Object.keys(values).length === 0)
        return getProgram(id, companyId);
    const [row] = await db
...
export async function getRun(id, companyId) {
    const db = getDb();
    const [row] = await db
        .select()
...
export async function listRuns(companyId, options = {}) {
    const db = getDb();
    const conditions = [eq(distributionRuns.companyId, companyId)];
    if (options.programId)
        conditions.push(eq(distributionRuns.programId, options.programId));
    const rows = await db
        .select()
        .from(distributionRuns)
...
export async function updateRun(id, companyId, patch) {
    const db = getDb();
    const values = { ...patch };
...
export async function addRunCredits(id, companyId, amount) {
    if (amount <= 0)
        return;
    const db = getDb();
    await db
...
export async function upsertOrgs(args) {
    ...
    const existing = await db
        .select()
        .from(partnerOrgs)
        .where(and(eq(partnerOrgs.companyId, args.companyId), inArray(partnerOrgs.resolveKey, keys)));
    ...
    for (const org of args.orgs) {
        ...
        if (row) {
            ...
            const [winner] = await db
                .select()
                .from(partnerOrgs)
                .where(and(eq(partnerOrgs.companyId, args.companyId), eq(partnerOrgs.resolveKey, org.resolveKey)))
                .limit(1);
            if (winner)
                out.push(toOrg(winner));
        }
    }
    return out;
}
export async function getOrg(id, companyId) {
    const db = getDb();
    const [row] = await db
        .select()
        .from(partnerOrgs)
...
export async function listOrgsByIds(ids, companyId) {
    if (ids.length === 0)
        return [];
    const db = getDb();
    const rows = await db
...
export async function updateOrg(id, companyId, patch) {
    const db = getDb();
    const [row] = await db
        .update(partnerOrgs)
        .set(patch)
 ...
 */
export async function findKnownOrgEntity(companyId, name) {
    ...
    const [row] = await db
        ...
        .from(kgEntities)
        .where(and(eq(kgEntities.companyId, companyId), eq(kgEntities.label, "ORG"), eq(kgEntities.name, normalized)))
        .limit(1);
    return row ?? null;
}
// ─── Evidence ────────────────────────────────────────────────────────────────
...
export async function listEvidenceForOrg(companyId, orgId) {
    const db = getDb();
    const rows = await db
        .select()
...
export async function listEvidenceByIds(companyId, ids) {
    if (ids.length === 0)
        return [];
    const db = getDb();
    const rows = await db
        .select()
        .from(partnerEvidence)
        .where(and(eq(partnerEvidence.companyId, companyId), inArray(partnerEvidence.id, [...ids])));
    return rows.map(toEvidence);
}
// ─── Relationships ───────────────────────────────────────────────────────────
...
export async function upsertCandidateRelationships(args) {
    ...
    const rows = await db
        ...
        .where(and(eq(partnerRelationships.companyId, args.companyId), eq(partnerRelationships.programId, args.programId), inArray(partnerRelationships.orgId, orgIds)));
    return rows.map(toRelationship);
}
export async function getRelationship(id, companyId) {
    const db = getDb();
    const [row] = await db
        .select()
        .from(partnerRelationships)
...
function staleCutoff(stage, now) {
    const days = STALE_AFTER_DAYS[stage];
    if (days === null)
        return null;
    return new Date(now.getTime() - days * 86_400_000);
}
export function isStale(relationship, now = new Date()) {
...
export async function listPartners(companyId, filters = {}) {
    const db = getDb();
    const conditions = [eq(partnerRelationships.companyId, companyId)];
    if (filters.programId)
        conditions.push(eq(partnerRelationships.programId, filters.programId));
    if (filters.stage) {
        const stages = Array.isArray(filters.stage) ? filters.stage : [filters.stage];
        if (stages.length > 0)
            conditions.push(inArray(partnerRelationships.stage, stages));
    }
    if (filters.kind)
        conditions.push(eq(partnerRelationships.kind, filters.kind));
    if (filters.country)
        conditions.push(eq(partnerOrgs.country, filters.country.toUpperCase()));
    ...
    const order = filters.orderBy === "activity"
        ? [
            desc(sql `coalesce(${partnerRelationships.lastActivityAt}, ${partnerRelationships.stageChangedAt})`),
        ]
...
export async function updateRelationship(id, companyId, patch) {
    const db = getDb();
    const [row] = await db
...
export async function setResearchResult(id, companyId, result) {
    const db = getDb();
    const [row] = await db
        .update(partnerRelationships)
        .set({
...
export async function listExclusions(companyId, programId) {
    const db = getDb();
    ...
    const rows = await db
        ...
        .from(partnerRelationships)
        .innerJoin(partnerOrgs, eq(partnerOrgs.id, partnerRelationships.orgId))
        .where(and(eq(partnerRelationships.companyId, companyId), eq(partnerRelationships.programId, programId)));
    for (const row of rows) {
        const engaged = isPastCandidate(row.stage) || row.source === "import" || row.stage === "declined";
        if (!engaged)
            continue;
        if (row.domain)
...
export async function addEvent(args) {
    ...
    const [row] = await db
        ...
        .returning();
    if (!row)
        throw new Error("Failed to record relationship event");
    if (args.touch !== false) {
        await db
            .update(partnerRelationships)
            .set({ lastActivityAt: occurredAt })
            .where(and(eq(partnerRelationships.id, args.relationshipId), eq(partnerRelationships.companyId, args.companyId)));
    }
    return toEvent(row);
}
export async function listEvents(companyId, relationshipId) {
    const db = getDb();
    const rows = await db
        .select()
        .from(relationshipEvents)
        .where(and(eq(relationshipEvents.companyId, companyId), eq(relationshipEvents.relationshipId, relationshipId)))
...
export async function importPartners(args) {
    ...
    const resolved = resolveOrganizations(args.rows.map(r => ({
        name: r.name,
        domain: r.domain,
        country: r.country,
        source: "import",
    ...
    const domains = orgs.map(o => o.domain).filter((d) => Boolean(d));
    if (domains.length > 0) {
...
export async function updateAgreement(id, companyId, input) {
    const db = getDb();
    const values = {};
    if (input.territory !== undefined)
        values.territory = input.territory ?? null;
    if (input.exclusivity !== undefined)
        values.exclusivity = input.exclusivity;
    if (input.startsOn !== undefined)
        values.startsOn = input.startsOn ?? null;
    if (input.endsOn !== undefined)
        values.endsOn = input.endsOn ?? null;
    if (input.terms !== undefined)
        values.terms = input.terms;
    if (input.documentId !== undefined)
        values.documentId = input.documentId ?? null;
    ...
    const [row] = await db
        ...
        .set(values)
        .where(and(eq(distributionAgreements.id, id), eq(distributionAgreements.companyId, companyId)))
        .returning();
    return row ? toAgreement(row) : null;
}
export async function listAgreements(companyId, relationshipId) {
    const db = getDb();
    const rows = await db
        .select()
        .from(distributionAgreements)
        .where(and(eq(distributionAgreements.companyId, companyId), eq(distributionAgreements.relationshipId, relationshipId)))
...
export async function getDashboard(companyId, programId) {
    const db = getDb();
    const program = await getProgram(programId, companyId);
    const items = await listPartners(companyId, { programId, limit: 500, orderBy: "activity" });
    const counts = Object.fromEntries(STAGE_ORDER.concat(["declined", "dormant"]).map(s => [s, 0]));
    for (const item of items)
        counts[item.relationship.stage] += 1;
    const funnelStages = [
        "candidate",
        "contacted",
        "in_conversation",
    ...
    const inPipeline = items.filter(i => ["contacted", "in_conversation", "qualified", "negotiating"].includes(i.relationship.stage)).length;
    const stale = items.filter(i => i.stale).length;
    const dueThisWeek = items.filter(i => i.relationship.nextActionAt && i.relationship.nextActionAt <= weekAhead).length;
    const renewalRows = await db
        .select({ id: distributionAgreements.id })
        .from(distributionAgreements)
        .innerJoin(partnerRelationships, eq(partnerRelationships.id, distributionAgreements.relationshipId))
        .where(and(eq(distributionAgreements.companyId, companyId), eq(partnerRelationships.programId, programId), isNotNull(distributionAgreements.renewalReminderAt), lt(distributionAgreements.renewalReminderAt, weekAhead)));
    ...
    for (const item of items) {
        ...
        if (stage === "contracted" || stage === "active")
            cell.covered += 1;
        else if (["contacted", "in_conversation", "qualified", "negotiating"].includes(stage))
            cell.inPipeline += 1;
        else if (stage === "candidate" || stage === "researched")
            cell.candidates += 1;
    }
    const coverage = [...cellMap.values()].sort((a, b) => a.country.localeCompare(b.country) || a.kind.localeCompare(b.kind));
    ...
    if (relationshipIds.length > 0) {
        const events = await db
            .select()
            .from(relationshipEvents)
            .where(and(eq(relationshipEvents.companyId, companyId), eq(relationshipEvents.type, "stage_changed"), inArray(relationshipEvents.relationshipId, relationshipIds)))
        ...
        for (const [stage, list] of durations) {
            const m = median(list);
            if (m !== undefined)
                medianDaysInStage[stage] = Math.round(m * 10) / 10;
        }
    }
    const attention = items
        .filter(i => i.stale || (i.relationship.nextActionAt && i.relationship.nextActionAt <= weekAhead))
        .sort((a, b) => {
