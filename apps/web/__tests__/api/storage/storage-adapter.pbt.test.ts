...
jest.mock("~/server/storage/s3-client", () => ({
    ...
    deleteObject: (...args: unknown[]) =>
        (mockDeleteObject as (...a: unknown[]) => unknown)(...args),
...
const filenameArb = fc.string({ minLength: 1, maxLength: 60, unit: "grapheme" }).map(s => {
...
const dataArb = fc.uint8Array({ minLength: 1, maxLength: 200 }).map(arr => Buffer.from(arr));

const userIdArb = fc
    .string({ minLength: 1, maxLength: 40, unit: "grapheme" })
    .map(s => s.replace(/[^a-zA-Z0-9_-]/g, "u") || "user1");
...
beforeEach(() => {
    ...
    mockGetObjectUrl
        .mockClear()
        .mockImplementation((key: string) => `${TEST_ENDPOINT}/${TEST_BUCKET}/${key}`);
...
describe("Feature: storage unification, Property 5: Upload result shape consistency", () => {
...
describe("Feature: storage unification, Property 6: Upload error propagation", () => {
...
describe("Feature: storage unification, Property 9: URL resolution and fetching", () => {
        ...
        fc.assert(
            fc.property(
                fc
                    ...
                    .map(s => `documents/${s.replace(/[^a-zA-Z0-9_-]/g, "x") || "file"}`),
                key => {
    ...
    it("fetchFile routes S3 URLs to plain fetch and legacy private-blob URLs to fetchBlob", async () => {
        ...
        try {
            ...
            const privateBlobUrl =
                "https://store.private.blob.vercel-storage.com/documents/test.pdf";
...
describe("Feature: storage unification, Property 10: S3 retrieval error descriptiveness", () => {
    it("when S3 is unreachable, error includes endpoint and 'unavailable'", async () => {
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                fc
                    .string({ minLength: 1, maxLength: 80, unit: "grapheme" })
                    .map(s => s.replace(/[^a-zA-Z0-9_.-]/g, "x") || "file.pdf"),
...
describe("Feature: storage unification, Property 11: Backend resolution", () => {
