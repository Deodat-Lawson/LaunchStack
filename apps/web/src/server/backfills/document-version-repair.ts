...
/** Documents still missing a resolvable current version — the `--dry-run` estimate. */
export async function countUnrepairedDocuments(db: DbClient): Promise<number> {
    return resultCount(
...
export async function repairDocumentVersions(db: DbClient): Promise<void> {
