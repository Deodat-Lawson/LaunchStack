...
const reasoningUnion = z.discriminatedUnion("mode", [
  ...
  z
    .object({ mode: z.literal("always"), request: requestPatchSchema.optional() })
    .strict(),
  z
    .object({
...
function refineParameters(
  value: { maxOutputTokensField?: string },
  ctx: z.RefinementCtx,
): void {
  if (
...
const partialParametersSchema =
  parametersObjectSchema.partial().superRefine(refineParameters);
...
const fullBehaviorSchema = z
  .object({
    ...
    nativeStructuredOutput: z.array(
      z.enum(["json-object", "json-schema", "tool-calling"]),
    ),
...
function resolveModelBehavior(
  ...
  if (model.preset) {
    ...
    const merged = fullBehaviorSchema.safeParse(
      mergeBehavior(preset.behavior, override.data),
    );
    if (!merged.success) {
      throw new ChatConfigurationError(
 ...
 */
export function buildChatModelsConfig(
  input: BuildChatModelsConfigInput,
): ChatModelsConfig {
...
export function parseChatModelsYaml(text: string, sourceLabel = "chat model configuration"): unknown {
  try {
