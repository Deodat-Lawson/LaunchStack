...
export const partialWorkspaceFixture = fixture([
    ...
    },
    {
        ...
        sourceId: "context:1",
        title: "Founder context",
        excerpt: "A founder heard customers ask about audit logs.",
        metadata: {},
    },
]);
...
function payload(customerEvidence: boolean): FounderWeeklyReviewV2Payload {
    ...
    return {
        ...
        sections: {
            ...
            whatCustomersSaid: customerEvidence
                ? {
                      state: "evidence",
                      items: [
                          {
                              kind: "observed_fact",
                              text: "Audit logs were requested.",
...
function assertGrounded(
    ...
    for (const section of [
        review.sections.whatChanged,
        review.sections.whatShipped,
        review.sections.whatCustomersSaid,
...
describe("LAU-9 grounding evaluation fixtures", () => {
    it("grounds the complete fixture, including customer attribution", async () => {
        ...
        expect(result.reviewPayload.sections.whatCustomersSaid).toMatchObject({
            state: "evidence",
        });
    });
    ...
    it("completes the empty fixture without calling a provider or inventing facts", async () => {
        ...
        const result = await generateFounderWeeklyReview({
            evidenceSnapshot: emptyWorkspaceFixture,
            generate,
        });
