        ...
        it("returns validationError on 422 without throwing", async () => {
            ...
            const result = await step.run("modify-document", async () => {
                try {
                    ...
                    if (
                        err instanceof MockAdeuServiceError &&
                        ...
                        return {
                            summary: null,
                            fileBase64: null,
                            validationError: (err as { detail: string }).detail,
                        };
                    }
                    throw err;
                }
            ...
            expect((result as Record<string, unknown>).validationError).toBe(
                "Edit 1: target not found"
            );
        });
