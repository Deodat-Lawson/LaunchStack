...
describe("loadConfig", () => {
  ...
  it("refuses to start without CONVERTER_API_KEY, explaining why", () => {
    expect(() =>
      loadConfig({ ALLOWED_FETCH_ORIGINS: "http://app:3000" }),
    ).toThrowError(/CONVERTER_API_KEY/);
    expect(() =>
      loadConfig({ ALLOWED_FETCH_ORIGINS: "http://app:3000" }),
    ).toThrowError(/fails closed/);
  });

  it("treats an empty CONVERTER_API_KEY as unset (fail closed)", () => {
    expect(() =>
      loadConfig({ ...validEnv, CONVERTER_API_KEY: "  " }),
    ).toThrowError(/CONVERTER_API_KEY/);
  });

  it("requires ALLOWED_FETCH_ORIGINS", () => {
    expect(() => loadConfig({ CONVERTER_API_KEY: "k" })).toThrowError(
  ...
  it("normalizes allowlist entries to origins and drops duplicates", () => {
    ...
    expect(config.allowedFetchOrigins).toEqual([
      "http://app:3000",
      "https://s3.internal:9000",
    ]);
  });
  ...
  it("rejects OCR_DEFAULT_PROVIDER=MARKER with an actionable message naming DOCLING", () => {
    const attempt = () =>
      loadConfig({ ...validEnv, OCR_DEFAULT_PROVIDER: "MARKER" });
  ...
  it("rejects unknown providers, listing the valid ones", () => {
    expect(() =>
      loadConfig({ ...validEnv, OCR_DEFAULT_PROVIDER: "TESSERACT" }),
    ).toThrowError(/DOCLING, NATIVE_PDF, AZURE, LANDING_AI, DATALAB/);
  });
