import { GEMINI_BASE_URL, GEMINI_DEFAULT_MODEL, configureChatModels, createChatModelsConfig, } from "@launchstack/llm";
const trimmed = (value) => {
    const t = value?.trim();
    if (!t)
        return undefined;
...
export function configureJudgeFromEnv() {
    const baseUrl = trimmed(process.env.CHAT_BASE_URL) ?? trimmed(process.env.AI_BASE_URL) ?? GEMINI_BASE_URL;
    const apiKey = trimmed(process.env.CHAT_API_KEY) ??
        trimmed(process.env.AI_API_KEY) ??
        trimmed(process.env.GOOGLE_AI_API_KEY);
    if (!apiKey) {
        throw new Error("Set CHAT_API_KEY (or GOOGLE_AI_API_KEY for the default Gemini endpoint) before running the benchmark judge.");
    }
    const modelId = trimmed(process.env.BENCHMARK_JUDGE_MODEL) ?? GEMINI_DEFAULT_MODEL;
    configureChatModels(createChatModelsConfig({
        yaml: [
            "version: 1",
            "models:",
            "  judge:",
