...
#   - adeu-docs-editing (services/adeu-ai-docs-editing)  → /health, /adeu/read, /adeu/diff
#
# Prerequisites: both services running locally, e.g. via docker compose with
# service names `transcription` and `adeu-docs-editing`.
#
# This script:
#   1. Checks transcription /health returns ok and /transcribe fails closed (401)
#   2. Checks adeu-docs-editing /health returns adeu available + pinned version
...
# 8000 (the old sidecar port) and `adeu-docs-editing` on 8003.
TRANSCRIPTION_PORT="${TRANSCRIPTION_PORT:-8000}"
ADEU_SERVICE_PORT="${ADEU_SERVICE_PORT:-8003}"
TRANSCRIPTION_URL="${TRANSCRIPTION_URL:-http://localhost:$TRANSCRIPTION_PORT}"
ADEU_SERVICE_URL="${ADEU_SERVICE_URL:-http://localhost:$ADEU_SERVICE_PORT}"
...
ADEU_SERVICE_API_KEY="${ADEU_SERVICE_API_KEY:-${SIDECAR_API_KEY:-pdr_local_sidecar_key}}"
...
wait_for "adeu-docs-editing" "$ADEU_SERVICE_URL"
...
D_HEALTH=$(curl -sf "$ADEU_SERVICE_URL/health")
...
EXPECTED_ADEU_VER=$(grep -E '^adeu==' "$(dirname "$0")/../../services/adeu-ai-docs-editing/requirements.txt" | head -1 | cut -d'=' -f3)
if [ -z "$EXPECTED_ADEU_VER" ]; then
  red "Could not determine expected adeu version from services/adeu-ai-docs-editing/requirements.txt"
  FAIL=$((FAIL + 1))
else
...
READ_RESP=$(curl -sf -X POST "$ADEU_SERVICE_URL/adeu/read" \
  -H "X-API-Key: $ADEU_SERVICE_API_KEY" \
...
DIFF_RESP=$(curl -sf -X POST "$ADEU_SERVICE_URL/adeu/diff" \
  -H "X-API-Key: $ADEU_SERVICE_API_KEY" \
