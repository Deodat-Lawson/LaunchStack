 ...
 */
import { z } from "zod";
import { AgentBudgetExceededError, AgentStepLimitError, defineAgentTool, runAgent, } from "@launchstack/llm";
import { loadPlaybook } from "./skills.js";
import { DossierSchema, EVIDENCE_KINDS } from "./types.js";
...
export function validateDossierGrounding(dossier, evidence, fetched) {
    const errors = [];
    const ids = new Set();
    const collect = (rows) => rows.forEach(r => r.evidenceIds.forEach(id => ids.add(id)));
 ...
 */
export function makeDossierTools(ports, input) {
    ...
    const fetchPage = defineAgentTool({
        name: "fetch_page",
        description: "Fetch one public web page and return its readable text (scripts and markup removed). " +
        ...
        run: async ({ url }) => {
            ...
            if (cached)
                return { content: renderPage(cached, limits.maxCharsPerPage) };
            try {
    ...
    const tools = [fetchPage];
    if (ports.searchWeb) {
        const searchWeb = ports.searchWeb;
        tools.push(defineAgentTool({
            name: "search_web",
            description: "Web search. Returns up to 8 results as title, url and snippet. " +
            ...
            run: async ({ query }) => {
                try {
                    ...
                    return {
                        content: places
                            .slice(0, 10)
                            .map(p => `- ${p.name} — ${p.formattedAddress}${p.website ? ` — ${p.website}` : ""}`)
                            .join("\n"),
                    };
                }
                catch (error) {
    ...
    if (ports.tradeData) {
        ...
        tools.push(defineAgentTool({
            name: "lookup_trade",
            description: "Customs shipment records where this organisation is the importer. Returns shipper, HS code, description and date. Records count as fetched sources.",
            inputSchema: z.object({
                importerName: z.string().min(2).max(200),
                country: z.string().length(2),
            }),
            run: async ({ importerName, country }) => {
                try {
                    ...
                    const relevant = records.filter(r => r.consignee
                        .toLowerCase()
    ...
    tools.push(defineAgentTool({
        ...
        description: "Record one fact with its source. Returns an evidence id to cite in submit_result. " +
        ...
        run: async ({ kind, claim, sourceUrl, quote, confidence: rawConfidence, }) => {
...
function pageContainsQuote(text, quote) {
    ...
    for (let i = 0; i + 6 <= words.length; i++) {
        if (haystack.includes(words.slice(i, i + 6).join(" ")))
            return true;
    }
    return false;
}
function renderPage(page, maxChars) {
    const body = page.text.length > maxChars
...
export async function runDossierAgent(ports, input) {
    const limits = { ...DOSSIER_LIMITS, ...input.limits };
    ...
    if (errors.length === 0)
        return finish({ status: "ok", dossier: draft, repaired: false });
    // One repair with the structured errors and the list of valid ids.
    const validIds = [...toolset.getEvidence().values()].map(e => `${e.id}: ${e.kind} — ${e.claim} (${e.sourceUrl})`);
    try {
        const repair = await runAgent(ports.model, {
            system,
            user: `Your dossier failed validation. Fix every issue and call submit_result again.\n\nErrors:\n${errors.map(e => `- [${e.code}] ${e.message}`).join("\n")}\n\n` +
    ...
    if (errors.length === 0)
        return finish({ status: "ok", dossier: draft, repaired: true });
    return finish({ status: "gate_failed", dossier: null, errors });
}
...
//# sourceMappingURL=dossier-agent.js.map