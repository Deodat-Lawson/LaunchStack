...
import {
    ...
    DISPATCH_CLAIM_BATCH_SIZE,
    markDispatchDispatched,
    recordDispatchFailure,
} from "~/server/founder-weekly-review/dispatch-service";
import {
    founderWeeklyReviewCitationFailures,
    founderWeeklyReviewDispatchFailures,
    founderWeeklyReviewGenerationTotal,
    founderWeeklyReviewJobsEnqueued,
    founderWeeklyReviewRunsCompleted,
    founderWeeklyReviewRunsFailed,
...
const GenerationEventSchema = z.object({
    ...
    generationClaimId: z.string().min(1),
});
            ...
            await worker
                .markCollectionFailed(
                    ...
                    failure(
                        ...
                        "Evidence collection failed after retries."
                    )
                )
                .catch(() => undefined);
            return;
        ...
        case "queued":
            ...
            await worker
                .markQueuedRunFailed(
                    ...
                    failure(
                        "generation_failed",
                        "Generation failed before the run could be claimed."
                    )
                )
                .catch(() => undefined);
            return;
...
export const founderWeeklyReviewGenerationJob = inngest.createFunction(
    {
        id: "founder-weekly-review-generation",
        retries: 3,
    ...
    async ({ event, step }) => {
        ...
        if (!workflowRun.evidenceSnapshot) {
            ...
            try {
                ...
                await step.run("claim-evidence", () =>
                    worker.claimEvidenceCollection(collectionContext)
                );
                const collecting = await worker.getRun(context.companyId, context.runId);
                if (
                    !collecting.evidenceSnapshot &&
                    collecting.status === "collecting" &&
                    collecting.collectionClaimId === collectionContext.collectionClaimId
                ) {
                    logFounderWeeklyReview({
                        runId: collecting.id,
                        companyId: collecting.companyId.toString(),
                        stage: "evidence_collection_started",
                        status: collecting.status,
                        retryCount: collecting.retryCount,
                    });
                    const snapshot = await step.run("collect-evidence", () =>
                        canonicalFounderWeeklyReviewEvidenceCollector.collectFounderWeeklyReviewEvidence(
                            {
                                companyId: collecting.companyId,
                                reportingPeriod: collecting.reportingPeriod,
                                workspaceTimezone:
                                    collecting.collectionInput?.workspaceTimezone ?? "UTC",
                                founderContext: collecting.collectionInput?.founderContext,
                                actor: {
                                    externalUserId:
                                        collecting.collectionInput?.actorExternalUserId ??
                                        "unknown",
                                },
                    ...
                    await step.run("persist-evidence", () =>
                        worker.attachEvidenceSnapshotIfAbsent(collectionContext, snapshot)
                    );
                    workflowRun = await worker.getRun(context.companyId, context.runId);
                    logFounderWeeklyReview({
                        runId: workflowRun.id,
                        companyId: workflowRun.companyId.toString(),
                        stage: "evidence_collection_completed",
        ...
        if (claimed.status !== "generating" || claimed.generationClaimId !== data.generationClaimId)
            return { skipped: true };
        logFounderWeeklyReview({
            runId: claimed.id,
            companyId: claimed.companyId.toString(),
        ...
        try {
            ...
            if (!evidenceSnapshot)
                throw new Error("Evidence snapshot is required before generation.");
            const generated = await step.run("generate", () =>
                generateFounderWeeklyReview({
                    evidenceSnapshot,
                    generate: generateFounderWeeklyReviewStructured,
                })
            );
            await step.run("persist", () =>
                worker.saveGeneratedDraft(context, generated.reviewPayload, generated.modelMetadata)
            );
            ...
            founderWeeklyReviewStageDuration.observe(
                { stage: "generation", result: "success" },
                (performance.now() - generationStartedAt) / 1000
            );
            founderWeeklyReviewStageDuration.observe(
                { stage: "end_to_end", result: "success" },
                (Date.now() - saved.createdAt.getTime()) / 1000
            );
            logFounderWeeklyReview({
                runId: saved.id,
                ...
                generationAttempt: saved.generationAttempt,
            ...
            if (error instanceof FounderWeeklyReviewGenerationValidationError)
                founderWeeklyReviewCitationFailures.inc();
            founderWeeklyReviewGenerationTotal.inc({
                result: "failure",
                error_class:
                    error instanceof FounderWeeklyReviewGenerationValidationError
                        ? "citation_validation"
                        : "generation",
            });
            founderWeeklyReviewRunsFailed.inc({
                error_class:
                    error instanceof FounderWeeklyReviewGenerationValidationError
                        ? "citation_validation"
                        : "generation",
            ...
            if (error instanceof FounderWeeklyReviewGenerationValidationError) {
                throw new NonRetriableError(
                    "Founder Weekly Review validation failed after bounded semantic repair."
                );
            }
            throw error;
        }
