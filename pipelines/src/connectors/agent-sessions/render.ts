...
import { sessionToolLabel, type NormalizedSession, type TranscriptSegment } from "./types";
...
function describeDropped(session: NormalizedSession): string | null {
    ...
    if (sidechain > 0) parts.push(`${sidechain} subagent record${sidechain === 1 ? "" : "s"}`);
    if (metadata > 0) parts.push(`${metadata} harness record${metadata === 1 ? "" : "s"}`);
    if (unknown > 0) parts.push(`${unknown} unrecognized record${unknown === 1 ? "" : "s"}`);
    if (malformed > 0) parts.push(`${malformed} malformed line${malformed === 1 ? "" : "s"}`);
    return parts.length > 0 ? parts.join(", ") : null;
}

export function sessionDisplayTitle(session: NormalizedSession, fallbackId: string): string {
    if (session.title) return session.title;
    const firstUser = session.segments.find(
        (segment): segment is Extract<TranscriptSegment, { kind: "user" }> =>
            segment.kind === "user"
    );
    if (firstUser) {
...
export function renderSessionMarkdown(
    ...
    lines.push(
        `> Imported ${label} session${session.sessionId ? ` \`${session.sessionId}\`` : ""}.`
    );
    const where: string[] = [];
    if (session.projectPath) where.push(`project \`${session.projectPath}\``);
    if (session.gitBranch) where.push(`branch \`${session.gitBranch}\``);
    ...
    if (droppedNote) lines.push(`> Not imported: ${droppedNote}.`);
    lines.push("");
    ...
    for (const segment of session.segments) {
        if (segment.kind === "user") {
            ...
            if (lastSpeaker !== "assistant") lines.push("## Assistant", "");
