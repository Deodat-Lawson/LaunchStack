...
async function main(): Promise<void> {
    ...
    server.on("error", error => {
        logger.error({ error: error.stack ?? error.message }, "worker http server failed");
        process.exit(1);
    });
    ...
    const reclaimTimer = setInterval(() => {
        void outbox
            ...
            .catch(error => logger.error({ error: String(error) }, "stale-claim reclaim failed"));
    ...
    const loop = (async () => {
        while (!stopping) {
            ...
            if (claimed === 0) {
                await new Promise(resolve => setTimeout(resolve, workerEnv.OUTBOX_IDLE_POLL_MS));
            }
        }
    })();
