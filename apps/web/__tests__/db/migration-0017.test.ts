...
const FILENAME = "0017_reconcile_file_uploads_company_id.sql";
...
describe("migration 0017", () => {
  ...
  it("runs after the immutable 0016 migration", () => {
    const files = readdirSync(MIGRATIONS_DIR)
      .filter((file) => file.endsWith(".sql"))
      .sort();

    expect(files.indexOf("0016_file_uploads_company_id.sql")).toBeGreaterThanOrEqual(0);
    expect(files.indexOf(FILENAME)).toBeGreaterThan(
      files.indexOf("0016_file_uploads_company_id.sql"),
    );
