...
const docxBufferArb = fc.uint8Array({ minLength: 4, maxLength: 1024 }).map(arr => Buffer.from(arr));
...
const textContentArb = fc.string({ minLength: 1, maxLength: 500 }).filter(s => s.trim().length > 0);
...
const authorNameArb = fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0);
