...
flowchart TD
    subgraph EXT[External dependencies]
      ...
      OAI[OpenAI<br/>gpt-5-nano · gpt-4o]
      WEB[Tavily / Serper<br/>web search]
    end
    ...
    BAR --> S3[3 Company DNA<br/>metadata-first → RAG · gpt-5-nano]
    BAR --> S4[4 Competitors<br/>web search · gpt-5-nano]
    BAR --> S5[5 Platform trends<br/>web search]
    BAR --> S6[6 Brand voice<br/>RAG · gpt-5-nano]
    BAR --> S7{targetAudience set?}
    S7 -->|yes| S7a[7 Persona<br/>RAG · gpt-5-nano]
    ...
    MERGE --> STRAT[Stage 9 — Strategy synthesis<br/>gpt-5-nano · 3 fixed angles:<br/>thought-leadership · pain-point · proof-driven]
    STRAT --> GEN[Stage 10 — Content generation<br/>gpt-4o ×3 in parallel · quality gate OFF]
    GEN --> SEL[Selection: bestVariant = variants 0<br/>no ranking]
    SEL --> CLAIM[Stage 11 — Claim verification<br/>gpt-5-nano extract → RAG match ≤5 claims]
...
| 3 | Company DNA | `context.ts` `extractCompanyDNA` | parallel branch | companyId, prompt | `CompanyDNA` (mission, differentiators, proven results, human story, technical edge) | gpt-5-nano | `company_metadata` JSONB first; RAG fallback | none | uncaught → stops run | RAG when metadata missing/low-confidence (`MIN_CONFIDENCE = 0.5`) | strategy, generation |
| 4 | Competitor analysis | `competitor.ts` | parallel branch | companyIdentity, categories | `CompetitorAnalysis` | gpt-5-nano | web search + company identity | 24h in-memory cache (not durable) | LLM failure uncaught → can stop run | search fallback exists; final synth can still fail | strategy |
| 5 | Platform trends | `research.ts` | parallel branch | platform, company context, prompt | `MarketingResearchResult[]` | — | web search (Tavily/Serper via trend-search) | cache (trend-search) | caught | returns `[]` | strategy |
| 6 | Brand voice | `voice.ts` | parallel branch (always) | companyId, toneOverride | `BrandVoice` | gpt-5-nano | RAG (topK 6) | none | caught in `run.ts` | `brandVoice = undefined` | strategy, generation, refine |
| 7 | Target persona | `persona.ts` | only if `targetAudience` set | companyId, targetAudience | `TargetPersona` | gpt-5-nano | RAG (topK 6) | none | caught | `targetPersona = undefined`; skipped if no audience | strategy, generation |
...
| 9 | Strategy synthesis | `positioning.ts` `buildMultiStrategy` | after merge | DNA, competitors, trends, voice, persona, insights | 3 × `StrategyVariant` (thought-leadership, pain-point, proof-driven) | gpt-5-nano | in-memory | none | uncaught → stops run | none | generation |
| 10 | Content generation | `generator.ts` `generateVariants` | after strategies | 1 strategy each, shared research/voice/persona/type | 3 × `ContentVariant` | gpt-4o (×3 parallel) | in-memory | none | if one fails, whole `Promise.all` rejects → siblings discarded | none; quality gate OFF | selection |
| 11 | Claim verification | `claim-verifier.ts` | after selection | companyId, `bestVariant.message` | `ClaimSource[]` (≤5) | gpt-5-nano (extract) | RAG (topK 2) | none | caught | `[]` | SSE result, UI |
...
| 13 | Refine (dedicated) | `generator.ts` `refineContent` → `/api/.../refine` | POST to refine route | message, platform, feedback, company context, brand voice | refined message + `feedbackApplied` | gpt-4o | caller-supplied | none | 500 on error | — | **no UI caller** |
...
- `buildMessagingStrategy` (`positioning.ts`) — imported in `run.ts:4`, **never called**.
- `generateCampaignOutput` (`generator.ts`) — imported in `run.ts:2`, **never called**. This and above are superceded by the "make 3 posts not 1" function
- `validatePostQuality` — only runs when `enableQualityGate` is true; the live path passes `false`.
- `/api/marketing-pipeline/refine` and `/publish` — no component in the app fetches them.
...
- **Strategy model** → confirmed `MARKETING_MODELS.strategyBuilding` = **gpt-5-nano**
  (`positioning.ts:66`), matching the draft.

### 7.2 Verified accurate (code-checked)
- Models (`models.ts`): DNA / competitor / strategy / claims = **gpt-5-nano**;
  content generation + refine = **gpt-4o**. (Note: this differs from older branches that
  used `gpt-5-mini` throughout — the draft is correct for *this* branch.)
...
- **`?debug=true` still exists** (`route.ts:66`) — it was the OpenAI-key *diagnostic* that
...
3. **Generation fault-tolerance** — `Promise.all` over 3 gpt-4o calls loses all output if one fails; use `allSettled` + keep survivors.
...
*Generated against branch `marketing-pipeline-v2`. File/line anchors reflect that branch;
