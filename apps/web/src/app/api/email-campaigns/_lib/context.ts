 ...
 * `~/server/api/context`.
 *
 * Everything general — actor resolution, the permission gate, id parsing,
 * response envelopes, error mapping — now lives in the shared module. What
 * stays here is genuinely about email: the sender identity CAN-SPAM requires
 * on every message, and the base URL unsubscribe tokens are appended to.
 *
...
/**
 * Same as {@link resolveActor}, but additionally requires `campaigns.send`.
 *
 * Approving a template and delivering outbound email are workspace-wide
 * actions taken under the company's identity, so they are their own
 * permission rather than a side effect of holding any other. Members can
 * still draft campaigns and run dry-run previews.
 */
export async function resolveSendingActor(): Promise<ActorResult<CampaignActor>> {
    return requirePermission(await resolveActor(), "campaigns.send");
}
