...
import {
  ...
  retryDelayMs,
  runOutboxTick,
} from "../src/pipeline/outbox-tick";
import type { ClaimedEvent, OutboxStorePort } from "../src/ports";
import type { PipelineProcessor } from "../src/pipeline/process-event";
...
function claimedEvent(
  overrides: Partial<ClaimedEvent> = {},
): ClaimedEvent {
  return {
    outboxId: 1,
    attemptCount: 0,
...
describe("runOutboxTick", () => {
  ...
  it("keeps processing the batch after one event fails", async () => {
    const outbox = makeOutbox([
      claimedEvent({ outboxId: 1 }),
      claimedEvent({ outboxId: 2 }),
    ]);
