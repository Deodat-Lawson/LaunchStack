...
export class MeetingOrchestrator {
  ...
  private async stepInner(): Promise<MeetingStepResult> {
    ...
    try {
      ...
      this.state = { ...this.state, nextSpeakerId: this.peekNextSpeaker(await this.transcript())?.id ?? null };
   ...
   */
  async run(options: { limit?: number; onStep?: (r: MeetingStepResult) => void } = {}): Promise<MeetingState> {
