...
const RUN = ["1", "true"].includes(
  (process.env.RUN_LLM_BENCHMARK ?? "").toLowerCase(),
);
...
const BENCH_DIR = path.join(
  REPO_ROOT,
  "packages/features/src/marketing-pipeline/benchmark",
);
...
suite("campaign planner benchmark (LLM judge, raw scores)", () => {
  ...
  it("scores candidates and writes results", async () => {
    ...
    const runnable = candidates.filter(
      (c) => c.post && !c.post.trim().startsWith("<"),
    );
    ...
    for (const crit of JUDGE_CRITERIA) {
      byCriterion[crit] = mean(
        results.flatMap((r) =>
          r.scores.filter((s) => s.criterion === crit).map((s) => s.score),
        ),
      );
    }
    ...
    console.log(
      `  results → ${path.relative(REPO_ROOT, outDir)}/results.json\n`,
    );
