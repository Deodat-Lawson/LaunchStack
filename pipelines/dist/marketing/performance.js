...
export async function getPerformanceHistory(args) {
    ...
    const rows = await db
        ...
        .where(and(eq(marketingContentHistory.companyId, BigInt(args.companyId)), eq(marketingContentHistory.platform, args.platform)))
...
export function buildPerformanceInsights(history) {
    ...
    if (withEngagement.length > 0) {
        const avgRate = withEngagement.reduce((sum, h) => sum + h.engagements / Math.max(h.impressions, 1), 0) / withEngagement.length;
        insights.push(`Average engagement rate: ${(avgRate * 100).toFixed(1)}%`);
        const best = withEngagement.sort((a, b) => b.engagements / Math.max(b.impressions, 1) -
            a.engagements / Math.max(a.impressions, 1))[0];
        if (best?.angle) {
...
export async function markContentPublished(args) {
    ...
    const [row] = await db
        ...
        .from(marketingContentHistory)
        .where(and(eq(marketingContentHistory.companyId, BigInt(args.companyId)), eq(marketingContentHistory.platform, args.platform), eq(marketingContentHistory.message, args.message)))
        .orderBy(desc(marketingContentHistory.createdAt))
        .limit(1);
    if (!row)
        return;
