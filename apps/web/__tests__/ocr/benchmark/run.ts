...
async function main() {
    ...
    for (const f of fixtures) {
        try {
            const truth = await safeReadText(
                join(GROUND_TRUTH_DIR, `${basename(f.path, extOf(f.path))}.md`)
            );
...
async function runOnyx(baseUrl: string, fixturePath: string): Promise<TargetOutput> {
    ...
    const ingestRes = await fetch(
        `${baseUrl.replace(/\/$/, "")}/manage/admin/connector/file/upload`,
        {
            method: "POST",
            body: form,
        }
    ...
    if (!ingestRes.ok)
        throw new Error(`onyx upload ${ingestRes.status}: ${await ingestRes.text()}`);
