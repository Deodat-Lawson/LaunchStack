...
async function rig(
    options: { script?: Record<string, string[]>; useAgentIdentity?: boolean } = {}
) {
    ...
    const created = await createMeeting({
        ...
        slack: {
            ...
            config: {
                ...
                enabled: true,
                useAgentIdentity: options.useAgentIdentity,
            },
        },
    });
...
function messageEvent(
    overrides: Partial<NonNullable<SlackEventEnvelope["event"]>>
): SlackEventEnvelope {
    return {
        type: "event_callback",
        event: {
...
describe("Slack mirroring (channel → Slack)", () => {
    ...
    it("labels a human speaking through an agent's seat", async () => {
        ...
        await orchestrator.postHumanMessage({
            humanId: "u_1",
            displayName: "Alex",
            text: "Product says no.",
        });
        await slackBridge!.drain();

        expect(slack.posted.some(p => p.text.includes("*Alex (for @pm)*: Product says no."))).toBe(
            true
        );
    });
...
describe("Slack ingestion (Slack → channel)", () => {
    it("appends a human Slack message into the meeting", async () => {
        ...
        const result = await slackBridge!.handleEvent(
            messageEvent({ text: "Hold on — legal is reviewing." })
        );

        expect(result.ingested?.author).toMatchObject({
            kind: "human",
            id: "U_ALEX",
    ...
    it("drops a duplicate delivery of the same Slack ts", async () => {
        ...
        expect(
            (await store.read(config.channelId)).filter(m => m.text === "Ship it.")
        ).toHaveLength(1);
    });
    ...
    it("ignores messages from other channels and empty messages", async () => {
        ...
        expect(
            (await slackBridge!.handleEvent(messageEvent({ channel: "C_OTHER", text: "hi" })))
                .ignored
        ).toBe("other_channel");
        expect((await slackBridge!.handleEvent(messageEvent({ text: "   " }))).ignored).toBe(
            "empty"
        );
        expect(
            (
                await slackBridge!.handleEvent({
                    type: "event_callback",
...
describe("Slack meeting controls", () => {
    it("!takeover pauses the agents and !release resumes them", async () => {
        ...
        expect((await slackBridge!.handleEvent(messageEvent({ text: "!takeover" }))).command).toBe(
            "takeover"
        );
        ...
        expect(
            (await slackBridge!.handleEvent(messageEvent({ text: "!release", ts: "1.2" }))).command
        ).toBe("release");
    ...
    it("!takeover @persona occupies that agent's seat", async () => {
        ...
        expect(orchestrator.getState().controller).toMatchObject({
            asPersonaId: "eng",
            displayName: "Alex Chen",
        });
    });
