...
export function supportedLanguageForPath(path) {
 ...
 */
function stripCommentsAndStrings(source, syntax) {
    const out = [];
    ...
    scan: while (i < length) {
        ...
        for (const delimiter of syntax.stringDelimiters) {
            if (source.startsWith(delimiter, i)) {
                out.push(delimiter);
                i += delimiter.length;
                const multiline = delimiter.length > 1 ||
...
function countChar(line, target) {
    let count = 0;
...
function collectDefinitions(lines, rules) {
    const seen = new Set();
    const ordered = [];
    const add = (name) => {
...
// References
// ---------------------------------------------------------------------------
function collectReferences(stripped, definitions, stopwords) {
    const seen = new Set();
    const references = [];
 ...
 */
export const extractFileSymbols = (path, content) => {
