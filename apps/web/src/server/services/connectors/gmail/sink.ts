/**
 * Host wiring for the Gmail connector — the `KnowledgeSink` that puts synced
 * threads and attachments into the knowledge base: blob upload → document
 * lifecycle → the same ingestion pipeline every other upload goes through.
 * Mirrors the Drive sink; the differences are Gmail-shaped metadata and a
 * per-member folder chosen by the caller.
 */
...
const CREATION_KEY_PREFIX = "connector:gmail:";

export interface GmailSinkContext {
    readonly companyId: bigint;
    readonly connectionId: number;
    /** The owner's auth subject id — every synced document is theirs. */
    readonly userId: string;
    /** The owner's private folder (`Gmail/<address>`). */
...
interface StoredMetadata {
    ...
    readonly gmailKind: string;
    readonly gmailThreadId: unknown;
    readonly gmailMessageId: unknown;
 ...
 * the ingestion router picks its adapter from it. Threads carry `.md`;
 ...
 */
export async function createGmailSink(context: GmailSinkContext): Promise<KnowledgeSink> {
    getEngine();
    ...
    return {
        ...
        async store(item: KnowledgeItem): Promise<StoredKnowledgeItem> {
            ...
            const metadata: StoredMetadata = {
                ...
                gmailKind: item.kind === "gmail-attachment" ? "attachment" : "thread",
                gmailThreadId: item.metadata.gmailThreadId,
                gmailMessageId: item.metadata.gmailMessageId ?? null,
                gmailLink: item.location.origin,
                subject: item.metadata.subject ?? item.title,
                participants: item.metadata.participants ?? null,
                lastMessageAt: item.metadata.lastMessageAt ?? item.modifiedAt,
            ...
            const lifecycle = await createDocumentVersionLifecycle({
                ...
                // message set converges on the version it already created.
                creationKey: `${baseCreationKey}:v:${item.contentHash}`,
                mimeType: item.mimeType,
                fileSize: contentByteLength(item.content),
                changelog: `Synced from Gmail on ${syncedAt}`,
                originalFilename: filename,
                embeddingIndexKey,
            });
    ...
    const rows = await db
        ...
        .where(
            and(
                ...
                sql`${document.ocrMetadata} ->> 'connector' = 'gmail'`,
...
/**
 * Deletion policy: mark, don't destroy. A thread that left the selected
 * labels (or was deleted in Gmail) keeps its ingested document — the owner
 * may have cited it — but carries a flag the UI can surface.
 */
export async function markMissingDocuments(
    ...
    await db
        .update(document)
        .set({
            ocrMetadata: sql`${document.ocrMetadata} || '{"gmailDeleted": true}'::jsonb`,
        })
        .where(
            and(
                eq(document.companyId, companyId),
