...
export interface SlackClient {
  ...
  createChannel(name: string, isPrivate?: boolean): Promise<{ ok: boolean; channelId?: string; error?: string }>;
...
export class HttpSlackClient implements SlackClient {
  ...
  async postMessage(input: SlackPostMessageInput): Promise<SlackPostMessageResult> {
    const result = await this.call<{ ok: boolean; ts?: string; channel?: string; error?: string }>(
      "chat.postMessage",
      {
  ...
  async userInfo(userId: string): Promise<SlackUserProfile | null> {
    ...
    const profile = result.ok && result.user
      ? {
          id: result.user.id,
