...
import { bigint, index, integer, jsonb, text, timestamp, uniqueIndex, varchar, } from "drizzle-orm/pg-core";
import { company } from "@launchstack/store/schema";
import { pgTable } from "@launchstack/store/schema/helpers";
import { DIAGRAM_TYPES, REPO_EXPLAINER_JOB_STATUSES, REPO_WORKSPACE_STATUSES, SYNC_REASONS, SYNC_REQUEST_STATUSES, } from "./types.js";
export const repoWorkspaces = pgTable("repo_workspaces", {
    ...
    repoUnique: uniqueIndex("repo_workspaces_company_repo_unique").on(table.companyId, table.provider, table.owner, table.repo),
...
export const repoSyncRequests = pgTable("repo_sync_requests", {
    id: varchar("id", { length: 256 }).primaryKey(),
...
export const repoContextBundles = pgTable("repo_context_bundles", {
    id: varchar("id", { length: 256 }).primaryKey(),
    workspaceId: varchar("workspace_id", { length: 256 })
        .notNull()
        .references(() => repoWorkspaces.id, { onDelete: "cascade" }),
    sha: varchar("sha", { length: 64 }).notNull(),
    bundle: jsonb("bundle").$type().notNull(),
    computeMs: integer("compute_ms"),
    createdAt: timestamp("created_at", { withTimezone: true })
        .default(sql `CURRENT_TIMESTAMP`)
        .notNull(),
}, table => ({
    shaUnique: uniqueIndex("repo_context_bundles_workspace_sha_unique").on(table.workspaceId, table.sha),
    workspaceIdx: index("repo_context_bundles_workspace_idx").on(table.workspaceId),
}));
export const repoExplainerJobs = pgTable("repo_explainer_jobs", {
    id: varchar("id", { length: 256 }).primaryKey(),
    companyId: bigint("company_id", { mode: "bigint" })
        .notNull()
        .references(() => company.id, { onDelete: "cascade" }),
