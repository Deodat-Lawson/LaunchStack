  ...
  db: {
    select: () => ({
      from: () => ({
        where: () => ({
          orderBy: () => ({
            limit: () => Promise.resolve(mockCtx.meetingRow ? [mockCtx.meetingRow] : []),
          }),
        }),
      }),
    }),
...
describe("POST /api/collab/slack/events", () => {
  ...
  it("echoes the URL verification challenge", async () => {
    const response = await POST(slackRequest({ type: "url_verification", challenge: "abc123" }));
  ...
  it("answers 503 when Slack events are not configured", async () => {
    ...
    expect(((await response.json()) as { error: string }).error).toMatch(/SLACK_SIGNING_SECRET/);
  });
