...
export class LandingAIAdapter implements OCRAdapter {
    ...
    constructor(apiKey?: string) {
        ...
        if (!this.apiKey) {
            console.warn("Landing.AI API key not configured. Adapter will fail on use.");
        }
    }
    ...
    async uploadDocument(
        ...
        if (!response.ok) {
            ...
            throw new Error(`Landing.AI parse failed: ${response.status} - ${errorText}`);
        }
        ...
        return {
            ...
            metadata: {
                ...
                provider: "LANDING_AI",
                processingTimeMs,
                // No confidenceScore: Landing.AI does not report one, and fabricated
                // constants are gone (ADR-004).
            },
        };
    }
