...
export function buildTranscriptMessages(
  ...
  return transcript.map((m) => {
    if (m.author.kind === "agent" && m.author.id === persona.id && !m.author.onBehalfOfPersonaId) {
...
export class LlmAgentRuntime implements AgentRuntime {
  ...
  async takeTurn({ persona, context, transcript }: AgentTurnRequest): Promise<AgentTurnResult> {
    ...
    if (transcript.length === 0) {
      messages.push({
        role: "user",
        content: "System: You are opening the meeting. State the goal and your first substantive point.",
      });
    }
