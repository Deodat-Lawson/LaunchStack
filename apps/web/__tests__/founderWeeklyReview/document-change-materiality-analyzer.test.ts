...
const chunk = (id: number, versionId: bigint, documentId: bigint, content: string, path = "/plan"): VersionChunk => ({
    chunkId: id,
    versionId,
    documentId,
    content,
...
const modified = (id: number, documentId: bigint, before: string, after: string, path = "/plan"): ChunkAlignment => ({
    changeType: "modified",
...
function analyzed(documentId: bigint, alignments: ChunkAlignment[], day = 2): AnalyzedDocumentChangeGroup {
...
class FakeAnalyzer implements DocumentChangeMaterialityAnalyzer {
    ...
    constructor(private readonly implementation: (input: DocumentChangeMaterialityAnalysisInput) => unknown | Promise<unknown>) {}
...
describe("optional document-change materiality analyzer", () => {
    it("uses a conservative semantic-risk gate and bypasses simple factual deltas", () => {
        const uncertainSingle = analyzed(1n, [modified(1, 1n, "Telemetry is reliable.", "Reliability is a property of telemetry.")]);
        ...
        const replacement = analyzed(3n, [
            { changeType: "removed", previousChunk: chunk(1, 1n, 3n, "The pilot serves invited accounts."), alignmentMethod: "unmatched" },
            { changeType: "added", currentChunk: chunk(2, 2n, 3n, "The rollout continues with invited accounts."), alignmentMethod: "unmatched" },
        ]);
        const complexStrong = analyzed(4n, [
            modified(1, 4n, "Product owns telemetry.", "Platform owns telemetry."),
            modified(2, 4n, "Retries follow the established operating process.", "The operating process for retries remains established."),
        ]);
        expect(shouldAnalyzeDocumentChangeGroup(uncertainSingle)).toMatchObject({ eligible: true, reason: "uncertain_single_change" });
        expect(shouldAnalyzeDocumentChangeGroup(uncertainMulti)).toMatchObject({ eligible: true, reason: "uncertain_multi_change" });
        expect(shouldAnalyzeDocumentChangeGroup(replacement)).toMatchObject({ eligible: true });
        expect(shouldAnalyzeDocumentChangeGroup(complexStrong)).toMatchObject({ eligible: true, reason: "complex_multi_change" });
    ...
    it("selects at most four calls deterministically while retaining document diversity", () => {
        const groups = [1n, 2n, 3n, 4n, 5n].flatMap((documentId, index) => [
            analyzed(documentId, [modified(1, documentId, "The workflow remains reliable.", "Reliability remains part of the workflow.")], index + 2),
            analyzed(documentId, [modified(2, documentId, "The plan remains durable.", "Durability remains in the plan.", "/other")], index + 2),
        ]);
        ...
        expect(forward.selected).toHaveLength(DOCUMENT_CHANGE_MATERIALITY_ANALYZER_LIMITS.callsPerReview);
        expect(new Set(forward.selected.map(group => group.group.documentId.toString())).size).toBe(4);
        expect(reverse.selected.map(group => group.group.groupId)).toEqual(forward.selected.map(group => group.group.groupId));
    ...
    it("freezes material analysis, validates copied key points, and preserves provider-owned provenance boundaries", async () => {
        ...
        const result = await materializeDocumentChangesWithAnalyzer([
            input(1n, [modified(1, 1n, "The pilot is limited to invited accounts.", "Invited accounts remain the pilot audience.")]),
        ], analyzer);
        ...
        expect(result.items[0]).toMatchObject({
            ...
            metadata: expect.objectContaining({ materialityMethod: "llm", category: "scope_change" }),
        });
        expect(result.items[0]!.excerpt).toContain("The pilot is limited to invited accounts.");
        expect(result.audit.groups[0]!.analysis).toEqual(expect.objectContaining({
    ...
    it("keeps non-material groups audit-only and retains uncertain groups conservatively", async () => {
        const nonMaterial = new FakeAnalyzer(() => ({
            disposition: "non_material", category: "editorial_rewrite", summary: "Meaning is unchanged.", confidence: 0.95,
        }));
        const pairInput = input(1n, [modified(1, 1n, "Product owns telemetry.", "Telemetry is owned by Product.")]);
        ...
        expect(filtered.audit.groups[0]).toMatchObject({ evidenceSourceId: null, analysis: { disposition: "non_material" } });

        const uncertain = new FakeAnalyzer(() => ({
            disposition: "uncertain", category: "uncertain", summary: "Meaning may have changed.", confidence: 0.4,
        }));
        const retained = await materializeDocumentChangesWithAnalyzer([pairInput], uncertain);
        expect(retained.items).toHaveLength(1);
        expect(retained.items[0]!.metadata).toMatchObject({ materialityMethod: "deterministic", category: "uncertain" });
        expect(retained.audit.groups[0]!.analysis?.disposition).toBe("uncertain");
    });

    it.each([
        ["unavailable", new DocumentChangeMaterialityAnalyzerError("unavailable", "offline"), "materiality_analyzer_unavailable"],
        ["timeout", new DocumentChangeMaterialityAnalyzerError("timeout", "slow"), "materiality_analysis_timeout"],
        ["invalid confidence", { disposition: "material", category: "scope_change", summary: "x", confidence: 2 }, "materiality_result_invalid"],
        ...
        const pairInput = input(1n, [modified(1, 1n, "Telemetry is reliable.", "Reliability is a property of telemetry.")]);
    ...
    it("isolates partial failures and never exceeds concurrency or call budgets", async () => {
        ...
        const inputs = [1n, 2n, 3n, 4n, 5n].map((documentId, index) => input(documentId, [
            modified(1, documentId, "The workflow remains reliable.", "Reliability remains part of the workflow."),
        ], index + 2));
        const result = await materializeDocumentChangesWithAnalyzer(inputs, analyzer);
        expect(analyzer.calls).toHaveLength(4);
        expect(maximumActive).toBeLessThanOrEqual(2);
        expect(result.analyzerDiagnostics).toMatchObject({ selectedAnalysisCount: 4, skippedByCallBudgetCount: 1, failedAnalysisCount: 1 });
        expect(result.warnings.map(warning => warning.code)).toEqual(expect.arrayContaining([
            "materiality_analysis_budget_truncated", "materiality_analysis_partial", "materiality_analyzer_unavailable",
        ]));
    });
    ...
    it("keeps evidence digest distinct from the exact prompt consequence", async () => {
        const pairInput = input(1n, [modified(1, 1n, "Product owns telemetry.", "Telemetry is owned by Product.")]);
        const deterministic = materializeDocumentChanges([pairInput]);
        const analyzedResult = await materializeDocumentChangesWithAnalyzer([pairInput], new FakeAnalyzer(() => ({
            disposition: "non_material", category: "editorial_rewrite", summary: "Meaning is unchanged.", confidence: 0.94,
        })));
        const snapshot = (items: typeof deterministic.items, audit: typeof deterministic.audit) => FounderWeeklyReviewEvidenceSnapshotSchema.parse({
            schemaVersion: "founder-weekly-review-evidence/v2",
            capturedAt: "2026-02-28T00:00:00.000Z",
            reportingPeriod: { start: "2026-02-01", end: "2026-02-28" },
            workspaceTimezone: "UTC",
        ...
        expect(buildFounderWeeklyReviewEvidenceDigest(after)).not.toBe(buildFounderWeeklyReviewEvidenceDigest(before));
        const promptHash = (value: typeof before) => createHash("sha256").update(buildFounderWeeklyReviewPrompt(value)).digest("hex");
    ...
    it("keeps raw/group construction stable before semantic analysis", () => {
        ...
        expect(grouped.groups[0]!.rawChanges.map(change => change.rawChangeId)).toEqual(raw.rawChanges.map(change => change.rawChangeId));
    });
});
