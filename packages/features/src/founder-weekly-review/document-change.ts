...
const validContentHash = (value: string | null): value is string =>
    value !== null && /^[a-f0-9]{64}$/i.test(value);
...
export function selectVersionPairsForReportingPeriod(
    ...
    return pairs.sort(
        (a, b) =>
            a.currentCreatedAt.getTime() - b.currentCreatedAt.getTime() ||
...
function bestMatch(
    previous: VersionChunk,
    candidates: VersionChunk[],
    used: Set<string>,
    key: (chunk: VersionChunk) => string | null,
    requireUnique = false,
    predicate: (candidate: VersionChunk) => boolean = () => true
): VersionChunk | undefined {
    const value = key(previous);
    if (!value) return undefined;
    const matches = candidates
        .filter(
            candidate =>
                !used.has(candidate.chunkId.toString()) &&
                key(candidate) === value &&
                predicate(candidate)
        )
        .sort(compareChunks);
...
function textSimilarity(left: string, right: string): number {
    const tokens = (value: string) =>
        new Set(value.toLocaleLowerCase().match(/[\p{L}\p{N}_-]{2,}/gu) ?? []);
    const a = tokens(left);
    const b = tokens(right);
    if (
        left.trim().length < MIN_TEXT_SIMILARITY_CHARACTERS ||
...
function bestTextMatch(
    ...
    candidates: VersionChunk[],
    used: Set<string>
): { chunk: VersionChunk; score: number } | undefined {
...
export function alignVersionChunks(
    ...
    const results: ChunkAlignment[] = [];
    const strategies: Array<{
        method: ChunkAlignment["alignmentMethod"];
        key: (chunk: VersionChunk) => string | null;
        requireUnique?: boolean;
        predicate?: (previous: VersionChunk, current: VersionChunk) => boolean;
    }> = [
        {
            method: "content_hash",
    ...
    for (const oldChunk of previous) {
        let match: VersionChunk | undefined;
        let method: ChunkAlignment["alignmentMethod"] = "unmatched";
        for (const strategy of strategies) {
            match = bestMatch(
                ...
                used,
                strategy.key,
                strategy.requireUnique,
                candidate => strategy.predicate?.(oldChunk, candidate) ?? true
            );
            if (match) {
    ...
    return results.sort((a, b) =>
        compareChunks(a.currentChunk ?? a.previousChunk!, b.currentChunk ?? b.previousChunk!)
    );
}
...
export function buildDocumentChangeEvidence(
    pair: VersionPair,
    alignments: readonly ChunkAlignment[]
            ...
            return {
                ...
                metadata: {
                    ...
                    userChangelog: pair.currentChangelog
                        ? bound(
                              pair.currentChangelog.replace(/\s+/g, " ").trim(),
                              MAX_METADATA_TEXT
                          )
                        : null,
                },
