...
export const uploadDocument = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (!Number.isInteger(eventData.versionId) || eventData.versionId <= 0) {
            throw new Error(`[ProcessDocument] Invalid versionId: ${String(eventData.versionId)}`);
        }
        ...
        if (isZipFile(eventData.mimeType, routingName)) {
            ...
            await step.run("delete-zip-document", async () => {
                await db.delete(ocrJobs).where(eq(ocrJobs.id, eventData.jobId));
                await db.delete(document).where(eq(document.id, eventData.documentId));
                console.log(
