/**
 * Drizzle schema for the Email Outreach Pipeline — a staged campaign lifecycle.
 *
...
export const emailCampaigns = pgTable(
  "email_campaigns",
  {
     ...
     * automation policy) approves one — `/send` refuses while it is NULL.
     */
    approvedVersionId: integer("approved_version_id"),
    /**
     * Denormalized copy of the LATEST version's template. Kept so readers
     * predating the lifecycle tables still work; `email_template_versions`
     * is the source of truth and the only thing a send may read.
     */
...
export const emailTemplateVersions = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex("email_template_versions_campaign_version_uq").on(
      t.campaignId,
      t.version,
    ),
  ],
...
export const emailCampaignApprovals = pgTable(
  "email_campaign_approvals",
  {
    ...
    /** human | automation */
    approvedByKind: varchar("approved_by_kind", { length: 16 })
      .notNull()
      .default("human"),
...
export const emailSendAttempts = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex("email_send_attempts_campaign_key_uq").on(
      t.campaignId,
      t.idempotencyKey,
    ),
  ],
...
/** One row per recipient per attempt — the delivery audit trail. */
export const emailSends = pgTable(
  "email_sends",
  {
    ...
    campaignId: integer("campaign_id")
      .notNull()
      .references(() => emailCampaigns.id, { onDelete: "cascade" }),
    /** NULL only for rows written before send attempts existed. */
    attemptId: integer("attempt_id").references(() => emailSendAttempts.id, {
      onDelete: "cascade",
    }),
  ...
  (t) => [
    ...
    uniqueIndex("email_sends_attempt_recipient_uq").on(
      t.attemptId,
      t.recipientEmail,
    ),
  ],
