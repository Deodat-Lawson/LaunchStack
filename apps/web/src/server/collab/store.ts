...
export class PostgresChannelStore implements ChannelStore {
  ...
  async getChannel(channelId: string): Promise<Channel | null> {
    const [row] = await db.select().from(collabChannel).where(eq(collabChannel.id, channelId)).limit(1);
  ...
  async getChannelBySlug(workspaceId: string, slug: string): Promise<Channel | null> {
    const [row] = await db
      .select()
      .from(collabChannel)
      .where(and(eq(collabChannel.companyId, BigInt(workspaceId)), eq(collabChannel.slug, slug)))
      .limit(1);
    return row ? toChannel(row) : null;
  }
  ...
  async getMessage(messageId: string): Promise<ChannelMessage | null> {
    const [row] = await db.select().from(collabMessage).where(eq(collabMessage.id, messageId)).limit(1);
