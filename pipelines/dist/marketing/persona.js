...
import { TargetPersonaSchema } from "./types.js";
export async function extractTargetPersona(args) {
    const { companyId, targetAudience } = args;
    const options = { companyId, topK: 6, weights: [0.4, 0.6] };
    const results = await getRag().companyEnsembleSearch(`target audience customer persona ${targetAudience} pain points needs priorities`, options);
    ...
    const response = await invokeMarketingStructured(TargetPersonaSchema, [
        new SystemMessage(`You are an audience research analyst. Given a target audience description and company knowledge, synthesize a TargetPersona profile.
...
Ground everything in the provided context. Return valid JSON.`),
        new HumanMessage(`Target audience: ${targetAudience}\n\nCompany knowledge:\n\n${contextBlock}`),
    ], "target_persona");
