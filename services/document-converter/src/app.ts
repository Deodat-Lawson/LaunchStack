...
function parseBody<S extends z.ZodType<unknown>>(
  schema: S,
  body: unknown,
): z.infer<S> {
...
export function createApp(config: Config, deps: AppDeps = {}): Express {
  ...
  app.get("/readyz", async (_req, res) => {
    ...
    res
      ...
      .json({ status: state.ready ? "ready" : "unready", docling: state.docling });
  });
  ...
  app.post("/route", async (req, res) => {
    ...
    const routingDeps: RoutingDeps = {
      fetchDocument: (url) =>
        fetchDocument(config, fetchImpl, url, "documentUrl"),
      renderPages,
      classify,
    };
