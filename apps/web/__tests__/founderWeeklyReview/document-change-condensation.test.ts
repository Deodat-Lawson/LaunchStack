...
const pair = (documentId = 1n, previousVersionId = 1, currentVersionId = 2): VersionPair => ({
    ...
    previousCreatedAt: new Date(
        `2026-01-${String(Math.min(previousVersionId, 28)).padStart(2, "0")}T00:00:00.000Z`
    ),
    currentVersionId,
    currentVersionNumber: currentVersionId,
    currentCreatedAt: new Date(
        `2026-02-${String(Math.min(currentVersionId, 28)).padStart(2, "0")}T00:00:00.000Z`
    ),
...
describe("deterministic document-change condensation", () => {
    it("constructs stable, distinct raw IDs and preserves provenance independent of alignment order", () => {
        ...
        const alignments: ChunkAlignment[] = [
            ...
            {
                changeType: "removed",
                previousChunk: chunk(2, 1n, "removed", { structurePath: "/removed" }),
                alignmentMethod: "unmatched",
            },
            {
                changeType: "added",
                currentChunk: chunk(3, 2n, "added", { structurePath: "/added" }),
                alignmentMethod: "unmatched",
            },
        ];
        const first = buildRawDocumentChanges(versionPair, alignments);
        const shuffled = buildRawDocumentChanges(versionPair, [
            alignments[2]!,
            alignments[0]!,
            alignments[1]!,
        ]);
        ...
        expect(
            first.rawChanges.every(change =>
                /^raw_document_change:[a-f0-9]{64}$/.test(change.rawChangeId)
            )
        ).toBe(true);
        expect(first.rawChanges.find(change => change.changeType === "modified")).toEqual(
            expect.objectContaining({
                previousChunk: expect.objectContaining({ chunkId: 1 }),
                currentChunk: expect.objectContaining({ chunkId: 10_001 }),
            })
        );
        const changedContent = buildRawDocumentChanges(versionPair, [
            modified(1, "before", "different after"),
        ]);
        expect(changedContent.rawChanges[0]!.rawChangeId).not.toBe(
            first.rawChanges.find(change => change.changeType === "modified")!.rawChangeId
        );
    });
    ...
    it("groups adjacent mixed changes within a section and never crosses known section boundaries", () => {
        ...
        const alignments: ChunkAlignment[] = [
            ...
            {
                changeType: "removed",
                previousChunk: chunk(2, 1n, "old two", {
                    structurePath: "/alpha",
                    structureTitle: "Alpha",
                }),
                alignmentMethod: "unmatched",
            },
            {
                changeType: "added",
                currentChunk: chunk(3, 2n, "new two", {
                    structurePath: "/alpha",
                    structureTitle: "Alpha",
                }),
                alignmentMethod: "unmatched",
            },
            modified(4, "old beta", "new beta", {
                structurePath: "/beta",
                structureTitle: "Beta",
                pageNumber: 2,
            }),
        ...
        expect(
            grouped.groups
                .find(group => group.structurePath === "/alpha")
                ?.rawChanges.map(change => change.changeType)
                .sort()
    ...
    it("keeps distinct known titles separate when paths are unavailable", () => {
        ...
        const raw = buildRawDocumentChanges(versionPair, [
            modified(1, "old alpha", "new alpha", {
                structurePath: null,
                structureTitle: "Alpha",
                pageNumber: 1,
            }),
            modified(2, "old beta", "new beta", {
                structurePath: null,
                structureTitle: "Beta",
                pageNumber: 1,
            }),
    ...
    it("does not merge conflicting known paths solely because their titles match", () => {
        const versionPair = pair();
        const raw = buildRawDocumentChanges(versionPair, [
            modified(1, "old one", "new one", {
                structurePath: "/one",
                structureTitle: "Overview",
                pageNumber: 1,
            }),
            modified(2, "old two", "new two", {
                structurePath: "/two",
                structureTitle: "Overview",
                pageNumber: 1,
            }),
    ...
    it("groups renamed paths through connected aligned chunks and supports one-to-many and many-to-one boundaries", () => {
        ...
        const renamed: ChunkAlignment[] = [1, 2].map(id => ({
            ...
            previousChunk: chunk(id, 1n, `old ${id}`, {
                structurePath: "/old",
                structureTitle: "Plan",
            }),
            currentChunk: chunk(100 + id, 2n, `new ${id}`, {
                structurePath: "/new",
                structureTitle: "Plan",
            }),
        ...
        const oneToMany: ChunkAlignment[] = [
            ...
            {
                changeType: "added",
                currentChunk: chunk(104, 2n, "part two", {
                    structurePath: "/split",
                    structureTitle: "Split",
                }),
                alignmentMethod: "unmatched",
            },
        ];
        const manyToOne: ChunkAlignment[] = [
            ...
            {
                changeType: "removed",
                previousChunk: chunk(6, 1n, "part two", {
                    structurePath: "/merge",
                    structureTitle: "Merge",
                }),
                alignmentMethod: "unmatched",
            },
        ];
        const groups = groupRawDocumentChanges(
            versionPair,
            buildRawDocumentChanges(versionPair, [...renamed, ...oneToMany, ...manyToOne])
                .rawChanges
        ).groups;
    ...
    it("produces identical groups and IDs for shuffled raw input", () => {
        ...
        expect(shuffled).toEqual(first);
        expect(
            first.groups.every(group => /^document_change_group:[a-f0-9]{64}$/.test(group.groupId))
        ).toBe(true);
        const changedSupport = buildRawDocumentChanges(versionPair, [
            modified(1, "a", "different", { structurePath: "/a", structureTitle: "A" }),
            modified(2, "b", "bb", { structurePath: "/a", structureTitle: "A" }),
        ]).rawChanges;
        expect(groupRawDocumentChanges(versionPair, changedSupport).groups[0]!.groupId).not.toBe(
            first.groups[0]!.groupId
        );
    });

    it("keeps 16 records together and splits 17 into stable bounded windows with one warning", () => {
        const versionPair = pair();
        const sixteen = buildRawDocumentChanges(
            versionPair,
            Array.from({ length: 16 }, (_, index) =>
                modified(index + 1, `before ${index}`, `after ${index}`, {
                    structurePath: "/large",
                    structureTitle: "Large",
                })
            )
        ).rawChanges;
        ...
        const seventeen = buildRawDocumentChanges(versionPair, [
            ...
            modified(17, "before 17", "after 17", {
                structurePath: "/large",
                structureTitle: "Large",
            }),
        ]).rawChanges;
        ...
        expect(
            first.groups.every(
                group => group.rawChanges.length <= DOCUMENT_CHANGE_GROUP_BUDGET.rawChangesPerGroup
            )
        ).toBe(true);
        expect(first.warnings).toEqual([
            expect.objectContaining({ code: "materiality_group_too_large" }),
        ]);
        expect(shuffled).toEqual(first);
    });
    ...
    it("enforces pair, document, and review limits with deterministic round-robin diversity", () => {
        const inputs = [1n, 2n, 3n, 4n].map(documentId => {
            ...
            return {
                pair: versionPair,
                alignments: Array.from({ length: 12 }, (_, index) =>
                    modified(
                        ...
                        {
                            documentId,
                            structurePath: `/section-${index}`,
                            structureTitle: `Section ${index}`,
                        }
                    )
                ),
            };
        ...
        const shuffled = condenseDocumentChanges(
            [...inputs]
                .reverse()
                .map(input => ({ ...input, alignments: [...input.alignments].reverse() }))
        );
        ...
        expect(shuffled.selectedGroups.map(group => group.groupId)).toEqual(
            first.selectedGroups.map(group => group.groupId)
        );
    });
    ...
    it("round-robins version pairs within a document", () => {
        const inputs = [pair(1n, 1, 2), pair(1n, 2, 3)].map((versionPair, pairIndex) => ({
            pair: versionPair,
            alignments: Array.from({ length: 6 }, (_, index) =>
                modified(
                    pairIndex * 100 + index + 1,
                    `before ${pairIndex}-${index}`,
                    `after ${pairIndex}-${index}`,
                    {
                        structurePath: `/pair-${pairIndex}-section-${index}`,
                        structureTitle: `Pair ${pairIndex} Section ${index}`,
                    }
                )
            ),
        }));
    ...
    it("reports the actual deterministic no-op count used by the collector", () => {
        const result = condenseDocumentChanges([
            { pair: pair(), alignments: [modified(7, "  same\r\nvalue\u00a0 ", "same\nvalue ")] },
        ]);
    ...
    it("condenses a deterministic 20-page, 40-chunk enterprise fixture before the prompt envelope", () => {
        ...
        for (let index = 0; index < 20; index++) {
            ...
            alignments.push({
                ...
                previousChunk: chunk(index + 1, 1n, exact, {
                    documentId: 44n,
                    pageNumber: index + 1,
                    structurePath: `/unchanged-${index}`,
                    structureTitle: `Unchanged ${index}`,
                }),
                currentChunk: chunk(1_001 + index, 2n, exact, {
                    documentId: 44n,
                    pageNumber: index + 1,
                    structurePath: `/unchanged-${index}`,
                    structureTitle: `Unchanged ${index}`,
            ...
            alignments.push(
                modified(
                    ...
                    `  wrapped\r\n line\u00a0${index}  `,
                    {
                        documentId: 44n,
                        pageNumber: index + 1,
                        structurePath: `/noop-${index}`,
                        structureTitle: `No-op ${index}`,
                    }
                )
            );
        for (let index = 0; index < 4; index++)
            alignments.push(
                modified(200 + index, `planned metric ${index}`, `launched metric ${index + 1}`, {
                    documentId: 44n,
                    pageNumber: 6 + index,
                    structurePath: `/material-${index}`,
                    structureTitle: `Material ${index}`,
                })
            );
        for (let index = 0; index < 6; index++)
            alignments.push(
                modified(300 + index, `old rewritten ${index}`, `new rewritten ${index}`, {
                    documentId: 44n,
                    pageNumber: 10 + Math.floor(index / 2),
                    structurePath: "/rewritten",
                    structureTitle: "Rewritten section",
                })
            );
        for (let index = 0; index < 5; index++)
            alignments.push(
                modified(400 + index, `editorial wording ${index}`, `rephrased wording ${index}`, {
                    documentId: 44n,
                    pageNumber: 15 + index,
                    structurePath: "/editorial",
                    structureTitle: "Editorial section",
                })
            );
        ...
        const evidence = first.selectedGroups.map(group =>
            buildDocumentChangeGroupEvidence(versionPair, group)
        );
        const snapshot = {
        ...
        expect(first.selectedGroups.map(group => group.groupId)).toEqual(
            shuffled.selectedGroups.map(group => group.groupId)
        );
