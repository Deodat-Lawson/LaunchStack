...
import type { FileSymbols, RepoMap, RepoMapEntry } from "../types";
...
export function buildRepoMap(files: FileSymbols[], options?: BuildRepoMapOptions): RepoMap {
    ...
    const entries: RepoMapEntry[] = files
        .map(file => {
            ...
            const symbols = [...file.definitions]
                .sort((a, b) => {
                    const diff =
                        (graph.referenceCounts.get(b) ?? 0) - (graph.referenceCounts.get(a) ?? 0);
...
export function renderRepoMap(entries: RepoMapEntry[], maxChars: number): string {
    const lines: string[] = [];
    let used = 0;
    for (const entry of entries) {
        ...
        if (used + block.length > maxChars) {
            if (lines.length > 0) lines.push("… (map truncated at budget)");
            break;
        }
