/**
 * Orchestration's ports — the interfaces durable work depends on
 * (ADR-003). The outbox store implementation lives in `outbox-store.ts`;
 * the source lifecycle implementation in `source-lifecycle/`; the
 * extraction/indexing pipeline is implemented by @launchstack/indexing and
 * injected by the worker's composition root.
 */
import type { PipelineEvent } from "./pipeline-events";
export type { ClockPort, LoggerPort } from "@launchstack/runtime";
