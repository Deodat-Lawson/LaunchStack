...
export const DocumentChangeMaterialityAnalysisSnapshotSchema = z
    .object({
    ...
    .superRefine((result, context) => {
        if (result.disposition === "uncertain" && result.category !== "uncertain") {
            context.addIssue({
                ...
                message: "Uncertain analysis requires uncertain category.",
                path: ["category"],
            });
        }
        if (result.disposition === "material" && result.category === "editorial_rewrite") {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Material analysis cannot use editorial_rewrite.",
                path: ["category"],
            });
        }
    });
...
export const DocumentChangeGroupSnapshotSchema = z
    .object({
...
export const DocumentChangeAuditSnapshotSchema = z
    .object({
...
export const FounderWeeklyReviewEvidenceSnapshotV1Schema =
    FounderWeeklyReviewEvidenceSnapshotBaseSchema.extend({
        schemaVersion: z.literal(FOUNDER_WEEKLY_REVIEW_EVIDENCE_V1_SCHEMA_VERSION),
    }).strict();
export const FounderWeeklyReviewEvidenceSnapshotV2Schema =
    FounderWeeklyReviewEvidenceSnapshotBaseSchema.extend({
...
export const FounderWeeklyReviewEvidenceSnapshotSchema = z
    ...
    .superRefine((snapshot, context) => {
        ...
        if (rawIds.size !== snapshot.documentChangeAudit.rawChanges.length) {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Document-change audit raw IDs must be unique.",
                path: ["documentChangeAudit", "rawChanges"],
            });
        }
        for (const [index, group] of snapshot.documentChangeAudit.groups.entries()) {
            if (seenGroups.has(group.groupId)) {
                context.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: "Document-change audit group IDs must be unique.",
                    path: ["documentChangeAudit", "groups", index, "groupId"],
                });
            }
            seenGroups.add(group.groupId);
            if (group.rawChangeIds.some(rawChangeId => !rawIds.has(rawChangeId))) {
                context.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: "Document-change audit group references an unknown raw change.",
                    path: ["documentChangeAudit", "groups", index, "rawChangeIds"],
                });
            }
            if (group.evidenceSourceId) {
                ...
                if (
                    ...
                    item.metadata.groupId !== group.groupId
                ) {
                    context.addIssue({
                        code: z.ZodIssueCode.custom,
                        message:
                            "Condensed document-change evidence must resolve to its audit group.",
                        path: ["documentChangeAudit", "groups", index, "evidenceSourceId"],
                    });
                }
            }
        }
        for (const [index, item] of snapshot.items.entries()) {
            if (
                item.sourceType === "document_change" &&
                item.sourceId.startsWith("document_change:group:") &&
                !snapshot.documentChangeAudit.groups.some(
                    group => group.evidenceSourceId === item.sourceId
                )
            ) {
                context.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: "Condensed document-change evidence has no audit group.",
...
>;
export type FounderWeeklyReviewEvidenceSchemaVersion =
    FounderWeeklyReviewEvidenceSnapshot["schemaVersion"];
...
const FounderWeeklyReviewV2FactualSectionSchema = z.union([
    z
        .object({
            ...
            items: z.array(FounderWeeklyReviewV2FactualItemSchema).min(1).max(3),
        })
        .strict(),
...
const FounderWeeklyReviewV2PrioritySectionSchema = z.union([
    z
        .object({
            state: z.literal("evidence"),
            items: z.array(FounderWeeklyReviewV2RecommendationSchema).min(1).max(3),
        })
        .strict(),
    FounderWeeklyReviewV2NoEvidenceSectionSchema,
]);
