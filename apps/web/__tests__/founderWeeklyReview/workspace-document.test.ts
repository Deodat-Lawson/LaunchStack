...
describe("workspace document evidence", () => {
    it("normalizes bounded founder context and skips blank queries", () => {
        expect(normalizeFounderContextRetrievalQuery("  founder   priority ")).toBe(
            "founder priority"
        );
    ...
    it("selects diverse stable hits and normalizes JSON-safe evidence", () => {
        ...
        expect(selected.map(item => [item.documentId, item.contextChunkId])).toEqual([
            [1n, 1],
            [1n, 2],
            [2n, 3],
        ]);
        ...
        expect(evidence[0]).toMatchObject({
            ...
            sourceId: "workspace_document:doc:1:version:9:chunk:1",
            metadata: {
                documentId: "1",
                documentVersionId: "9",
    ...
    it("uses exact BigInt ordering to break equal-score ties", () => {
        ...
        expect(selected.map(item => item.documentId)).toEqual([
            9007199254740992n,
            9007199254740993n,
        ]);
    });
