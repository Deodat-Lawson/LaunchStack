...
import { eventOutbox } from "@launchstack/store/schema";
import type { DbClient } from "@launchstack/store/client";
import { pipelineEventSchema, type PipelineEvent } from "./pipeline-events";
import type { ClaimedEvent, LoggerPort, OutboxStorePort } from "./ports";
...
export interface DrizzleOutboxStoreOptions {
     ...
     * never thrown.
     */
    onValidationDead?: (outboxId: number, eventId: string) => void | Promise<void>;
}
...
export class DrizzleOutboxStore implements OutboxStorePort {
        ...
        for (const row of rows as unknown as Array<{
            ...
            if (!parsed.success) {
                ...
                if (this.opts.onValidationDead) {
                    try {
                        await this.opts.onValidationDead(Number(row.id), String(row.event_id));
                    } catch (hookError) {
                        this.logger.error(
     ...
     */
    async markProcessed(outboxId: number, followUps: PipelineEvent[] = []): Promise<void> {
    ...
    async markFailed(
        ...
        const rows =
          ...
          `);
        if ((rows as unknown as unknown[]).length === 0) {
            this.logger.warn({ outboxId }, "stale claimant outcome discarded; row was reclaimed");
        }
    }
