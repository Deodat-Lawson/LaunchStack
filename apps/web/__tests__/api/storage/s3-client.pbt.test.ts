...
const s3KeyArb = fc
    .tuple(
        fc
            ...
            .map(s => s.replace(/[^a-z0-9_-]/gi, "a") || "a"),
...
const endpointArb = fc
    .tuple(
        ...
        fc
            .string({ minLength: 3, maxLength: 20, unit: "grapheme" })
            .map(s => s.replace(/[^a-z0-9]/gi, "x") || "xxx"),
...
describe("Feature: local-s3-migration, Property 3: Presigned URL structure", () => {
...
describe("Feature: local-s3-migration, Property 4: S3 client connection error descriptiveness", () => {
