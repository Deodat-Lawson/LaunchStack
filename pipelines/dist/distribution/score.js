...
function tokens(values) {
    ...
    for (const value of values) {
        for (const t of value.toLowerCase().split(/[^\p{L}\p{N}]+/u))
            if (t.length > 2)
                out.add(t);
    }
    return out;
...
export function computeFit(input) {
    ...
    const categoryOverlap = Math.round(WEIGHTS.categoryOverlap * Math.min(1, overlapRatio(programTerms, orgTerms) * 1.5));
    ...
    const dossierTerritories = (input.dossier?.territories ?? []).map(t => t.territory.toLowerCase());
    let territoryMatch = 0;
    if (wanted && input.org.country === wanted)
        territoryMatch = WEIGHTS.territoryMatch;
    ...
    else if (input.org.country === null)
        territoryMatch = Math.round(WEIGHTS.territoryMatch * 0.3);
    // Role: dossier roles (agent-established) beat mention-guessed roles.
    const roles = new Set([...(input.dossier?.roles ?? []), ...input.org.roles]);
...
export function deriveRiskFlags(input) {
    const flags = [];
    if (input.breakdown.excludedBecause)
        flags.push(`excluded: ${input.breakdown.excludedBecause}`);
    if (input.evidenceCount === 0)
        flags.push("no evidence recorded");
    else if (input.evidenceCount < 3)
        flags.push("thin evidence");
    if (input.budgetExhausted)
        flags.push("research budget exhausted");
    for (const risk of input.dossier?.risks ?? [])
        flags.push(risk.risk);
    if (input.breakdown.roleMatch === 0)
        flags.push("role not confirmed");
...
export function templateRationale(args) {
    const b = args.breakdown;
    if (b.excludedBecause)
        return `${args.orgName} scores 0: ${b.excludedBecause}`;
    const parts = [
