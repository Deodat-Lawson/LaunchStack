...
import { uploadBatches, uploadBatchFiles } from "@launchstack/store/schema";
    ...
    if (withFiles) {
        return db.query.uploadBatches.findFirst({
            where: (table, { eq: eqFn, and: andFn }) =>
                andFn(eqFn(table.id, batchId), eqFn(table.createdByUserId, userId)),
            with: { files: true },
...
export async function updateBatchStatus(
    ...
    extra?: Partial<
        Pick<
            UploadBatchRecord,
            "committedAt" | "processingStartedAt" | "completedAt" | "failedAt" | "errorMessage"
        >
    >
): Promise<void> {
    ...
    if (extra) {
        ...
        if (extra.processingStartedAt !== undefined)
            updateData.processingStartedAt = extra.processingStartedAt;
