...
export function makeExplainerTools(view: WorkspaceView, bundle: ContextBundle): ExplainerToolset {
    ...
    for (const memory of bundle.memoryFiles) readPaths.add(memory.path);
    ...
    const repoMap = defineAgentTool({
        name: "repo_map",
        description:
            "Query the ranked repo map (files other files depend on most). " +
        ...
        run: ({ query }): AgentToolResult => {
            if (!query) return { content: bundle.map.rendered || "(empty map)" };
    ...
    const repoTree = defineAgentTool({
        name: "repo_tree",
        description:
            "Render the directory tree. Optionally scope it to a subdirectory " +
            "and limit the depth.",
        inputSchema: z.object({
            path: z
                .string()
        ...
        run: async ({ path: scope, depth }): Promise<AgentToolResult> => {
    ...
    const searchCode = defineAgentTool({
        name: "search_code",
        description:
            "Search file contents (regex or literal). Returns matching lines as " +
        ...
        run: async ({ pattern, glob, maxResults }): Promise<AgentToolResult> => {
    ...
    const readFiles = defineAgentTool({
        name: "read_files",
        description:
            `Read up to ${READ_BUDGET.maxPathsPerCall} files by exact repo-relative path. ` +
        ...
        run: async ({ paths }): Promise<AgentToolResult> => {
            const files = await view.listFiles();
            ...
            sections.push(
                `--- budget: ${READ_BUDGET.maxFilesPerRun - filesReadCount} files, ` +
