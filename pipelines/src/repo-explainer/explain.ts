...
import { validateExplanation } from "./gate";
import { packDigest } from "./pack";
import { loadExplainerSkills } from "./skills";
import { makeExplainerTools } from "./workspace-tools";
...
export interface ExplainRunInput {
    ...
    signal?: AbortSignal;
    onTurn?: (info: { turn: number; toolCalls: string[] }) => void;
...
export const DEFAULT_EXPLAIN_LIMITS: Required<ExplainLimits> = {
    maxTurns: 12,
    tokenBudget: 250_000,
    fastPathMaxChars: 300_000,
};

function memoryFilesBlock(bundle: ContextBundle): string {
    if (bundle.memoryFiles.length === 0) return "(none found)";
...
function taskPrompt(
    ...
    const instructionsSection = trimmed
        ...
        : "";
    return (
        `${instructionsSection}Explain the repository ${repoName}. ` +
        `Produce the summary and one ${diagramType} diagram, then call submit_result.`
    );
}

export async function runRepoExplanation(input: ExplainRunInput): Promise<ExplanationRunResult> {
    ...
    let readPaths: ReadonlySet<string>;
    let turns: number;
    ...
    if (!gate.ok) {
        ...
        const repairRun = await runAgent(input.port, {
            system: systemBase,
            user:
                `Your previous result failed validation. Fix every issue and call ` +
