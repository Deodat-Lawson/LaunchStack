...
import { EXTRACTION_SYSTEM_PROMPT, buildChunkExtractionPrompt } from "./prompts.js";
// ============================================================================
// Configuration
// ============================================================================
 ...
 */
export async function extractCompanyFacts(input) {
 ...
 */
async function callLLM(generate, documentName, batchContent, batchIndex, totalBatches) {
    try {
...
// ============================================================================
function aggregateResults(batchResults, documentId, documentName, extractedAt, source) {
    ...
    for (const field of companyFields) {
        ...
        if (candidates.length > 0) {
            ...
            company[field] = hydrate(best, boosted, extractedAt, source);
        }
    }
...
// ============================================================================
function hydrate(fact, confidence, extractedAt, source) {
    return {
        value: fact.value,
...
function pickHighestConfidence(candidates) {
...
function deduplicateByValue(facts) {
    const map = new Map();
 ...
 */
function mergeNamedEntries(entries, extractedAt, source, hydrateEntry) {
...
function hydratePersonEntry(group, extractedAt, source) {
...
function hydrateServiceEntry(group, extractedAt, source) {
    const count = group.length;
    const bestName = pickHighestConfidence(group.map(g => g.name));
    const entry = {
        name: hydrate(bestName, boostConfidence(bestName.confidence, count), extractedAt, source),
...
function hydrateProjectEntry(group, extractedAt, source) {
    const count = group.length;
    const bestName = pickHighestConfidence(group.map(g => g.name));
    const entry = {
        name: hydrate(bestName, boostConfidence(bestName.confidence, count), extractedAt, source),
...
function hydrateSubprojectEntry(group, extractedAt, source) {
    const count = group.length;
    const bestName = pickHighestConfidence(group.map(g => g.name));
    const entry = {
        name: hydrate(bestName, boostConfidence(bestName.confidence, count), extractedAt, source),
...
function hydrateLegalEntry(group, extractedAt, source) {
    const count = group.length;
    const bestName = pickHighestConfidence(group.map(g => g.name));
    const entry = {
        name: hydrate(bestName, boostConfidence(bestName.confidence, count), extractedAt, source),
