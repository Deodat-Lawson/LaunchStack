...
/**
 * Returns true if the Neo4j backend should serve graph retrieval. The
 * enable/disable decision for graph retrieval as a whole lives in the
 * ensemble config (set by the composition root, which also configures
 * Neo4j itself before any retrieval runs) — this only picks the backend.
 */
