 ...
 */
export function mergeCompanyMetadata(existing, extracted) {
    const now = new Date().toISOString();
    const diff = { added: [], updated: [], deprecated: [] };
 ...
 */
function mergeNamedArray(existing, incoming, section, diff) {
...
// Markets merge
// ============================================================================
function mergeMarkets(existing, incoming, diff) {
    const result = { ...existing };
...
// Policies merge
// ============================================================================
function mergePolicies(existing, incoming, diff) {
    const result = { ...existing };
    for (const [key, incomingFact] of Object.entries(incoming)) {
        const existingFact = existing[key];
        if (!existingFact) {
