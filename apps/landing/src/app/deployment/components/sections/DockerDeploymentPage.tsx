...
import { Server, Database, RefreshCw, CheckCircle2, ShieldAlert } from "lucide-react";
...
const Divider: React.FC = () => <hr className="my-12 border-t border-gray-200" />;
    ...
    return (
        <>
            ...
            >
                ...
                >
                    Self-host Launchstack with Docker Compose. The stack includes PostgreSQL with
                    pgvector, automatic schema migrations, S3-compatible object storage, the Next.js
                    app, the background worker, and the compute services.
                </p>
            </motion.div>
            ...
            <Section
                title="What runs"
                subtitle="The default profile starts everything Local mode requires."
            >
                <div className="space-y-3">
                    <StepCard icon={<Database className="h-5 w-5" />} title="db">
                        PostgreSQL 16 with pgvector pre-installed (host port 5433). Data is
                        persisted in a named volume.
                    </StepCard>
                    <StepCard icon={<RefreshCw className="h-5 w-5" />} title="migrate">
                        Runs{" "}
                        <code
                        ...
                        once after the database is healthy, then exits.
                    </StepCard>
                    <StepCard icon={<Database className="h-5 w-5" />} title="seaweedfs">
                        S3-compatible object storage for uploaded files — no Vercel Blob token
                        needed when self-hosting.
                    </StepCard>
                    <StepCard
                        icon={<Server className="h-5 w-5" />}
                        title="transcription / document-editor / document-converter"
                    >
                        The compute services: Whisper transcription and yt-dlp download (port 8000),
                        Adeu DOCX redlining (port 8003), and OCR routing, vision classification, and
                        docling-backed parsing (port 8002). Each authenticates with a fail-closed
                        API key.
                    </StepCard>
                    <StepCard icon={<RefreshCw className="h-5 w-5" />} title="worker">
                        The sole durable workflow coordinator (port 8020): consumes the
                        transactional outbox, runs the ingestion pipeline, and hosts the Inngest
                        serve endpoint at{" "}
                        <code
                            className={`${darkMode ? "bg-gray-900" : "bg-gray-100"} rounded px-1 py-0.5 text-xs`}
                        >
                    ...
                    <StepCard icon={<Server className="h-5 w-5" />} title="app">
                        Production Next.js server on port 3000 — command acceptance and reads.
                        Durable work happens in the worker.
                    </StepCard>
                    <StepCard icon={<Server className="h-5 w-5" />} title="inngest-dev">
                        Inngest dev server (dashboard at port 8288), polling the worker&apos;s{" "}
                        <code
                            className={`${darkMode ? "bg-gray-900" : "bg-gray-100"} rounded px-1 py-0.5 text-xs`}
                        >
                            /api/inngest
                        </code>{" "}
                        endpoint — not the app.
                    </StepCard>
                </div>
            </Section>
...
open http://localhost:8288`}
                    ...
                    <table className="w-full text-sm">
                        ...
                        >
                            <tr className={darkMode ? "bg-gray-800/40" : "bg-white"}>
                                ...
                                >
                                    <code
                                        className={`${darkMode ? "bg-gray-900" : "bg-gray-100"} rounded px-1.5 py-0.5 text-xs`}
                                    >
                                        default
                                    </code>
                                </td>
                                ...
                                >
                                    db, migrate, seaweedfs, transcription, document-editor,
                                    document-converter, worker, app, inngest-dev
                                </td>
                                <td
                                    className={`px-4 py-3 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                                >
                                    Everything Local mode requires
                                </td>
                            </tr>
                            <tr className={darkMode ? "bg-gray-800/40" : "bg-white"}>
                                <td
                                ...
                                <td
                                    className={`px-4 py-3 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                                >
                                    PDF/Office parsing engine (~800MB RAM). Without it, /convert
                                    returns a typed 503 and text-file ingestion still works
                                </td>
                            </tr>
                            <tr className={darkMode ? "bg-gray-800/40" : "bg-white"}>
                                <td
            ...
            <div className="mb-16 space-y-4">
                <Callout icon={<CheckCircle2 className="h-5 w-5" />}>
                    ...
                    to confirm <em>migrate</em> exited successfully and <em>db</em>, <em>app</em>,{" "}
                    <em>worker</em>, and the compute services are healthy.
                </Callout>
                ...
                <Callout icon={<ShieldAlert className="h-5 w-5" />} variant="warning">
                    ...
                    <code
                        className={`${darkMode ? "bg-gray-800" : "bg-yellow-100"} rounded px-1.5 py-0.5 text-xs`}
                    >
                        docker compose --env-file .env build --no-cache migrate && docker compose
                        --env-file .env up
                    </code>
                </Callout>
            </div>
