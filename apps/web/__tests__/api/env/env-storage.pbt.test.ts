...
const invalidProviderArb = fc.string().filter(s => s !== "s3" && s !== "database");

/** Non-empty, non-whitespace string suitable as an S3 config value */
const s3ValueArb = fc.string({ minLength: 1, maxLength: 200 }).filter(s => s.trim().length > 0);
...
describe("Feature: storage unification, Property 1: Storage provider enum validation", () => {
...
describe("Feature: storage unification, Property 2: Conditional S3 variable requirement", () => {
