...
| **grounded-retrieval** | `@launchstack/tools/grounded-retrieval` | Company-scoped RAG retrieval with named snippet policies (topK / weights / caps) and a declared empty-vs-throw failure policy.                                                                                            | company-context; brand-voice + persona (planned, PR-2); claim-evidence (planned, PR-4) |

Planned (design doc, Phase 1): `brand-voice`, `persona`, `web-research`,
`claim-evidence`, `content-scoring`, `social-publish`, `platform-profiles`.
