...
export class SlackChannelBridge {
    ...
    private async mirrorOutbound(message: ChannelMessage): Promise<void> {
        ...
        const result = await this.options.slack.postMessage({
            ...
            text:
                this.options.useAgentIdentity && !isSystem
                    ? message.text
                    : `*${label}*: ${message.text}`,
            username: this.options.useAgentIdentity && !isSystem ? label : undefined,
            iconEmoji:
                this.options.useAgentIdentity && author.kind === "agent"
                    ? ":robot_face:"
                    : undefined,
    ...
    async handleEvent(envelope: SlackEventEnvelope): Promise<{
        ...
        const displayName =
            (await this.options.resolveDisplayName?.(humanId)) ?? (await this.lookupName(humanId));
        ...
        const message = await this.appendHuman({
            humanId,
            displayName,
            text,
            slackTs: event.ts,
    ...
    async backfill(sinceTs?: string): Promise<ChannelMessage[]> {
        ...
        for (const entry of history) {
            if (entry.bot_id || this.ownTimestamps.has(entry.ts) || this.ingested.has(entry.ts))
                continue;
