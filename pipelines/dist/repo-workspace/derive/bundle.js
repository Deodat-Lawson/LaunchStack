 ...
 */
import { CONTEXT_BUNDLE_SCHEMA_VERSION } from "../types.js";
import { buildHygieneManifest, makeDeniedSet } from "./hygiene.js";
import { collectMemoryFiles } from "./memory-files.js";
import { buildRepoMap } from "./repo-map.js";
import { computeRepoStats } from "./stats.js";
import { extractFileSymbols, supportedLanguageForPath } from "./symbols.js";
...
export async function deriveContextBundle(view, options) {
    ...
    const symbolCandidates = visible
        .filter(file => file.size <= MAX_SYMBOL_FILE_BYTES && supportedLanguageForPath(file.path) !== null)
    ...
    const tree = renderTree(visible.map(file => file.path), { maxChars: options?.treeMaxChars, maxDepth: options?.treeMaxDepth });
    return {
