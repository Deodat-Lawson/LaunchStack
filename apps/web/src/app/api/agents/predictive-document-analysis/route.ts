...
async function storeAnalysisResult(
    ...
    analysisType: string,
    includeRelatedDocs: boolean,
    resultJson: PredictiveAnalysisOutput
) {
...
async function getDocumentDetails(documentId: number): Promise<DocumentDetails | null> {
    ...
    return results[0]
        ? {
              ...results[0],
              companyId: results[0].companyId.toString(),
          }
        : null;
}
...
export async function POST(request: Request) {
    ...
    return withRateLimit(request, RateLimitPresets.strict, async () => {
        ...
        try {
            ...
            if (
                timeoutMs &&
                (timeoutMs < TIMEOUT_LIMITS.MIN_MS || timeoutMs > TIMEOUT_LIMITS.MAX_MS)
            ) {
                recordResult("error");
                return NextResponse.json(
                    {
            ...
            const rlmChunks = await db
                ...
                .where(
                    and(
                        ...
                        eq(document.id, documentId),
                        eq(document.currentVersionId, docDetails.currentVersionId),
                        eq(documentContextChunks.versionId, docDetails.currentVersionId)
                    )
                )
                .orderBy(documentContextChunks.id);

            const chunks: PdfChunk[] = rlmChunks.map(c => ({
                id: c.id,
                content: c.content,
                page: c.page ?? 1,
            ...
            if (error instanceof Error) {
                if (error.message.includes("timed out") || error.message.includes("timeout")) {
                    ...
                    message =
                        "The AI analysis took too long to complete. Please try again or reduce the document size.";
                    ...
                    message =
                        "External service error. Analysis completed with limited functionality.";
