...
function isPercentageField(key) {
    return PERCENTAGE_FIELD_KEYS.has(key) || key.endsWith("_pct");
}
function isPositiveIntField(key) {
    return (
...
function normalizeMarkedFieldText(value) {
    return value
        .replace(/<[^>]*>/g, "")
        .replace(/\u200B/g, "")
        .trim();
}
export function validateFieldValue(key, value, field) {
    ...
    if (field.type === "number") {
        ...
        if (isPositiveIntField(key) && num <= 0) return `${field.label} must be greater than 0`;
    }
    if (field.type === "select" && field.options && !field.options.includes(cleaned)) {
        return `Invalid value for ${field.label}`;
    }
    return null;
}
export function extractFieldValuesFromSections(sectionContents) {
    const values = {};
    const regex = /<mark[^>]*data-field-key=(["'])([^"']+)\1[^>]*>([\s\S]*?)<\/mark>/gi;
...
export function validateDocument(fieldValues, templateFields) {
    const errors = [];
    const seen = new Set();
 ...
 */
export function buildTemplateFieldDataForDocx(templateId, contentHtml, fallback) {
...
//# sourceMappingURL=legal-document-validation.js.map
