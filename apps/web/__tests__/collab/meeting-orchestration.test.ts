...
describe("meeting orchestration", () => {
  it("runs a round-robin meeting and ends on the completion marker", async () => {
    const h = harness({
      script: {
        pm: ["Margin is 42%. @eng can we ship a tiered plan?", "Decision: we'll go with tier B."],
  ...
  it("returns the floor to the moderator when no one is nominated", async () => {
    const h = harness({
      ...
      script: {
        pm: ["Opening. @eng start us off.", "Thanks. @fin your read?", "Wrapping. MEETING_COMPLETE"],
  ...
  describe("human takeover", () => {
    it("suspends agent turns until control is released", async () => {
      ...
      expect(transcript.some((m) => m.kind === "system" && m.text.includes("took the floor"))).toBe(true);
      expect(transcript.some((m) => m.kind === "system" && m.text.includes("handed control back"))).toBe(true);
    });
    ...
    it("consumes the agent's turn when a human speaks through its seat", async () => {
      ...
      const human = (await h.store.read(config.channelId)).find((m) => m.author.kind === "human");
    ...
    it("lets a human chime in without taking control", async () => {
      ...
      await orchestrator.start();
      await orchestrator.postHumanMessage({ humanId: "u_2", displayName: "Jo", text: "FYI churn is up 2%." });
...
describe("selectNextSpeaker", () => {
  ...
  it("round-robins independent of the transcript", () => {
    for (let i = 0; i < 6; i++) {
      expect(selectNextSpeaker({ participants, transcript: [], turnIndex: i, policy: { kind: "round_robin" } })!.id)
  ...
  it("returns null with no participants", () => {
    expect(
      selectNextSpeaker({ participants: [], transcript: [], turnIndex: 0, policy: { kind: "reactive" } }),
    ).toBeNull();
  });
  ...
  it("never hands a reactive turn straight back to the last speaker", () => {
    ...
    const next = selectNextSpeaker({ participants, transcript, turnIndex: 1, policy: { kind: "reactive" } });
...
describe("minutes", () => {
  it("extracts decisions and action items from the transcript only", async () => {
    ...
    const minutes = buildMinutes(config, orchestrator.getState(), await h.store.read(config.channelId));
