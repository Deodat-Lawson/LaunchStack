...
const PROMPT_METADATA_ALLOWLIST = {
    document_change: [
        "changeType",
...
function metadataText(item, key) {
...
function allowlistedMetadata(item) {
    const metadata = {};
...
export function buildFounderWeeklyReviewPromptEvidenceItem(item) {
    return {
...
function serializedItemsLength(items) {
    return JSON.stringify(items).length;
}
function canAppendWithin(selected, candidate, limit) {
...
export function buildGenerationEvidenceEnvelope(snapshot) {
...
export function assertGenerationEvidenceEnvelopeWithinBudget(envelope) {
