...
const BLOCK_START_PATTERNS: Record<string, RegExp[]> = {
    python: [/^(async\s+)?def\s+\w+/, /^class\s+\w+/, /^@\w+/],
    javascript: [
    ...
    rust: [/^(pub\s+)?(fn|struct|enum|trait|impl|mod|type)\s+/],
    css: [/^[.#@]\w/, /^\w[\w-]*\s*\{/],
    generic: [/^(export\s+)?(function|class|def|fn|func|sub|proc)\s+/],
};
...
export function chunkCodeFile(pages: PageContent[], filename?: string): DocumentChunk[] {
    ...
    for (const block of blocks) {
        if (
            parentCharCount + block.content.length > CODE_PARENT_MAX_CHARS &&
            parentBuffer.length > 0
        ) {
            flushParent();
        }
