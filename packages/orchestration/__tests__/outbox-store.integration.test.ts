...
import { eventIds, PROTOCOL_VERSION } from "../src/pipeline-events";
import type { PipelineEvent } from "../src/pipeline-events";

import { DrizzleOutboxStore } from "../src/outbox-store";
...
function noteEvent(noteId: number, companyId: number, fingerprint = "fp1"): PipelineEvent {
    return {
...
describe.skipIf(!url)("DrizzleOutboxStore (integration)", () => {
    ...
    it("revives a processed stable-id event on re-enqueue", async () => {
        ...
        const [claimed] = (await store.claimBatch(10)).filter(c => c.event.companyId === companyId);
    ...
    `;
        const reclaimed = await store.reclaimStale(new Date(Date.now() - 60 * 60_000), 8);
    ...
    `;
        ...
        expect(rows[0]!.last_error).toBe("reclaimed after stale claim; attempts exhausted");
    });
    ...
    it("cascade replay: replaying an upstream row revives its dead downstream row", async () => {
        ...
        const [a] = (await store.claimBatch(10)).filter(c => c.event.companyId === companyId);
        await store.markProcessed(a!.outboxId, [noteEvent(31, companyId)]);
        const [b] = (await store.claimBatch(10)).filter(c => c.event.companyId === companyId);
        ...
        const replayed = await store.replay(eventIds.noteEmbeddingRequested(30, "fp1"));
        expect(replayed).toBe(true);

        const [aAgain] = (await store.claimBatch(10)).filter(c => c.event.companyId === companyId);
        expect(aAgain!.event.eventId).toBe(eventIds.noteEmbeddingRequested(30, "fp1"));
    ...
    `);
        const claimed = await store.claimBatch(10);
        expect(claimed.filter(c => c.event.eventId === "corrupt:1")).toHaveLength(0);
        const rows = await handle.client`
