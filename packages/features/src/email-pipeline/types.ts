...
export type SendStatus = "dry_run" | "sent" | "failed" | "suppressed" | "skipped";

export interface SendResult {
    recipientEmail: string;
    status: SendStatus;
    /**
     * The rendered subject this recipient received (or would have, on a dry run).
     * Absent for `skipped` / `suppressed`, which are decided before the template
     * is merged and so never produce one.
     */
    subject?: string;
