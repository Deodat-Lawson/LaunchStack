...
const syntheticProvider = process.env.SYNTHETIC_FWR_PROVIDER;
const deterministicMode = (process.argv[2]?.startsWith("negative-") ?? false) || process.argv[2] === "retry-snapshot-immutability";
if (!deterministicMode && syntheticProvider !== "ollama" && syntheticProvider !== "kimi") throw new Error("Refusing synthetic baseline: set SYNTHETIC_FWR_PROVIDER=ollama or kimi.");
...
if (syntheticProvider === "ollama") {
  process.env.OLLAMA_BASE_URL = process.env.OLLAMA_BASE_URL ?? "http://127.0.0.1:11434/v1";
  process.env.LLM_PROVIDER_PRIORITY = "ollama";
  process.env.LLM_MODEL_FOUNDERWEEKLYREVIEW_OLLAMA = process.env.SYNTHETIC_FWR_MODEL ?? "llama3.1:8b";
}
if (syntheticProvider === "kimi" && !process.env.MOONSHOT_API_KEY?.trim()) throw new Error("Refusing Kimi synthetic baseline: MOONSHOT_API_KEY is required.");

async function generateWithLocalOllama<TSchema extends import("zod").ZodType>(input: { system?: string; prompt: string; schema: TSchema; schemaName?: string }) {
  console.log(JSON.stringify({ stage: "ollama_request_constructed", result: "pass" }));
  const response = await fetch(`${process.env.OLLAMA_BASE_URL!.replace(/\/$/, "")}/chat/completions`, {
    method: "POST", headers: { "content-type": "application/json", authorization: "Bearer ollama" },
    body: JSON.stringify({ model: process.env.SYNTHETIC_FWR_MODEL ?? "llama3.1:8b", temperature: 0, stream: false, messages: [{ role: "system", content: input.system ?? "" }, { role: "user", content: input.prompt }], response_format: { type: "json_schema", json_schema: { name: input.schemaName ?? "founder_weekly_review", strict: true, schema: zodSchema(input.schema).jsonSchema } } }),
  });
  console.log(JSON.stringify({ stage: "ollama_http_response", result: response.ok ? "pass" : "fail", httpStatus: response.status }));
  if (!response.ok) throw new Error(`ollama_http_error:${response.status}`);
  const body = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
  const content = body.choices?.[0]?.message?.content;
  if (!content?.trim()) throw new Error("ollama_missing_content");
  console.log(JSON.stringify({ stage: "ollama_content_extracted", result: "pass" }));
  let parsed: unknown;
  try { parsed = JSON.parse(content); console.log(JSON.stringify({ stage: "ollama_json_parsed", result: "pass" })); }
  catch { throw new Error("ollama_json_parse_failed"); }
  try { const object = input.schema.parse(parsed); console.log(JSON.stringify({ stage: "review_schema_valid", result: "pass" })); return { object, metadata: { provider: "ollama", model: process.env.SYNTHETIC_FWR_MODEL ?? "llama3.1:8b", capability: "founderWeeklyReview", temperature: 0 } }; }
...
async function generateWithKimi<TSchema extends import("zod").ZodType>(input: { system?: string; prompt: string; schema: TSchema; schemaName?: string }) {
  ...
  const content = body.choices?.[0]?.message?.content;
  if (!content?.trim()) throw new Error("kimi_missing_content");
  console.log(JSON.stringify({ stage: "kimi_content_extracted", result: "pass" }));
  let parsed: unknown; try { parsed = JSON.parse(content); console.log(JSON.stringify({ stage: "kimi_json_parsed", result: "pass" })); } catch { throw new Error("kimi_json_parse_failed"); }
  return { object: parsed as ReturnType<TSchema["parse"]>, metadata: { provider: "kimi", model: body.model ?? (process.env.SYNTHETIC_FWR_MODEL ?? "kimi-k2.6"), capability: "founderWeeklyReview", temperature: undefined as unknown as number, ...(body.choices?.[0]?.finish_reason ? { finishReason: body.choices[0].finish_reason } : {}), ...(body.usage ? { usage: body.usage } : {}) } };
}
