...
export class JsonExportAdapter implements SourceAdapter {
    ...
    private processGenericJson(raw: string, options?: SourceAdapterOptions): StandardizedDocument {
        ...
        const pages: {
            pageNumber: number;
            textBlocks: string[];
            tables: import("../../ocr/types").ExtractedTable[];
        }[] = [];
        let cursor = 0;
        let pageNum = 1;
    ...
    private processSlackExport(
        ...
        const startTime = Date.now();
        console.log(`[JsonExportAdapter/Slack] Processing: file=${options?.filename ?? "unknown"}`);
    ...
    private async resolveInput(input: string | Buffer): Promise<string> {
        ...
        if (input.startsWith("http://") || input.startsWith("https://") || input.startsWith("/")) {
            ...
            if (!res.ok) throw new Error(`JsonExportAdapter fetch failed: ${res.status}`);
            return res.text();
        }
        return input;
    }
    ...
    private groupByDate(messages: SlackMessage[]): Record<string, SlackMessage[]> {
