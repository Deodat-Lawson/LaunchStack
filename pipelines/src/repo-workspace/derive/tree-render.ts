...
export function renderTree(paths: string[], options?: RenderTreeOptions): string {
    const maxChars = options?.maxChars ?? DEFAULT_MAX_CHARS;
    const maxDepth = options?.maxDepth ?? DEFAULT_MAX_DEPTH;

    const root: TreeNode = { name: options?.rootLabel ?? ".", children: new Map(), isFile: false };
    ...
    const visit = (node: TreeNode, prefix: string, depth: number): void => {
        if (truncated) return;
    ...
    visit(root, "", 0);
    if (truncated) lines.push("… (tree truncated at budget)");
    return lines.join("\n");
}
