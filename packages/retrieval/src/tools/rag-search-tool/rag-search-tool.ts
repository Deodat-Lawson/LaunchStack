 ...
 *
 * Access validation is injected: which documents a user may read is an
 * app-side question (it lives in product schema this package cannot see), so
 * the composition root supplies an AccessValidator and this tool enforces its
 * answer before any retrieval runs.
 */

import { tool } from "@langchain/core/tools";
import { z } from "zod";
import { multiDocEnsembleSearch } from "../../algorithms/ensemble";
import { formatResultsForPrompt } from "./format";
import type { RAGSearchResult, RAGSearchInput } from "../../search-types";

/**
 * Resolves which of the requested documents the user may search, plus their
 * titles. Implemented by the app (workspace membership is product schema).
 */
...
export async function executeRAGSearch(
    ...
    userId: string,
    validateAccess: AccessValidator
): Promise<{
    ...
    try {
        ...
        const { validDocIds, documentTitles } = await validateAccess(userId, input.documentIds);
...
/**
 * Build the LangChain rag_search tool around an app-supplied access
 * validator. The userId travels in the run's `configurable` bag, as before.
 */
export function createRagSearchTool(validateAccess: AccessValidator) {
    return tool(
