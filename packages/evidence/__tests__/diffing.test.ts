...
describe("diffVersions", () => {
  ...
  it("is independent of input array order", () => {
    ...
    expect(
      diffVersions([...before].reverse(), [...after].reverse()),
    ).toEqual(expected);
    expect(
      diffVersions(
  ...
  it("throws on duplicate ordinals within one version", () => {
    expect(() =>
      diffVersions([chunk(0, "a"), chunk(0, "b")], []),
    ).toThrow(RangeError);
    expect(() =>
      diffVersions([], [chunk(2, "a"), chunk(2, "a")]),
    ).toThrow(RangeError);
  });
  ...
  it("does not mutate its inputs", () => {
    const before = Object.freeze([
      Object.freeze(chunk(1, "b")),
      Object.freeze(chunk(0, "a")),
    ]);
