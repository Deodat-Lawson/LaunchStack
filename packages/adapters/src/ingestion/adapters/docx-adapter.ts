...
export class DocxAdapter implements SourceAdapter {
    ...
    async process(
        ...
        console.log(
            `[DocxAdapter] Processing: file=${options?.filename ?? "unknown"}, mime=${options?.mimeType ?? "none"}`
        );
        ...
        console.log(
            `[DocxAdapter] Mammoth HTML output: ${html.length} chars, warnings=${result.messages.length}`
        );
