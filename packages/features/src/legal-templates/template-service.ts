...
export function validateData(templateId: string, data: Record<string, string>): ValidationResult {
...
export function fillTemplate(templatePath: string, data: Record<string, string>): Buffer {
...
export function generateDocument(templateId: string, data: Record<string, string>): GenerateResult {
    const template = TEMPLATE_REGISTRY[templateId];
    if (!template) {
        return {
            success: false,
