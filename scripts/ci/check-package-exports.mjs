/**
 * Verify every `exports` subpath of @launchstack/core is actually loadable from
 * the built output, the way an npm consumer would load it.
 *
 ...
 *
 * Run after `pnpm --filter @launchstack/core build`:
...
if (!existsSync(DIST)) {
  console.error(
    "[check-package-exports] dist/ not found — run `pnpm --filter @launchstack/core build` first",
  );
  process.exit(2);
}
...
if (failures.length > 0) {
  ...
  console.error(
    "\nIf these are ERR_MODULE_NOT_FOUND on a relative path, the build did not\n" +
      "rewrite specifiers for Node ESM — check that packages/core's build runs\n" +
      "scripts/fix-esm-specifiers.mjs after tsc.\n",
  );
  process.exit(1);
}

console.log(
  `\n[check-package-exports] ok — ${ok} export(s) load under Node ESM, ` +
    `${peerGated.length} peer-gated, ${skipped} wildcard`,
);
